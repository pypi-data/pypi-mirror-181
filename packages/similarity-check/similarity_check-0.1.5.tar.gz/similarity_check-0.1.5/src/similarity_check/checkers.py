...
class sentence_tranformer():
    def __init__(
        ...
        if isinstance(targets, pd.DataFrame):
            ...
            self.target_df = target.reset_index(drop=True)
            self.target_names = target[target_col].tolist()
        else:
            if isinstance(target_group, list):
                self.group_ids = target_group
            else:
                raise TypeError('if target are a list entered groups must also be a text')
    ...
    def clean_data(self, remove_punct: bool=True, remove_stop_words: bool=True, stemm: bool=False, lang: str='en', encode_target = False) -> None: 
    ...
    def match(
        ...
        if isinstance(source, pd.DataFrame):
            ...
            self.source_names = source[source_col].tolist()
        else:
            if not source:
        ...
        for i in tqdm(range(0, len(self.source_names), batch_size)):
            ...
            batch_targets, batch_top_cosine, batch_match_idxs = self.max_cosine_sim(encoded_inputs, topn, threshold)
            start = time()
            targets[i:i+batch_size, :] = batch_targets
            top_cosine[i:i+batch_size, :] = batch_top_cosine
            match_idxs[i:i+batch_size, :] = batch_match_idxs
            print(f'time taken to concat batch results is {time()-start:.2f}')
    ...
    def max_cosine_sim(self, encoded_inputs,  topn, threshold):
        if len(encoded_inputs.shape) == 1:
            a = a.unsqueeze(0)

        if len(self.encoded_targets.shape) == 1:
            b = b.unsqueeze(0)

        scores = np.matmul(encoded_inputs, self.encoded_targets.T)
