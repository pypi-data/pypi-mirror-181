...
class MeowerAPI:
    ...
    def get_page(self, page=1, chatid="home"):
        if chatid == "home":
          return self.session.get(urljoin(self.base_uri, "/home?autoget&page={0}".format(page))).json()
        else:
          return self.session.get(urljoin(self.base_uri, "/posts/{0}?autoget&page={1}".format(chatid, page))).json()

    def get_user(self, username):
        return self.session.get(urljoin(self.base_uri, "/users/{0}".format(username))).json()
