...
import blackLager.node
from blackLager import portnums_pb2, mesh_pb2
from blackLager.util import stripnl, Timeout, our_exit, remove_keys_from_dict, convert_mac_addr
from blackLager.__init__ import LOCAL_ADDR, BROADCAST_NUM, BROADCAST_ADDR, ResponseHandler, publishingThread, OUR_APP_VERSION, protocols

class MeshInterface:
    """Interface class for blackLager devices
    ...
    def __init__(self, debugOut=None, noProto=False):
        ...
        self.localNode = blackLager.node.Node(self, -1)  # We fixup nodenum later
    ...
    def getNode(self, nodeId):
        ...
        if nodeId in (LOCAL_ADDR, BROADCAST_ADDR):
            ...
            n = blackLager.node.Node(self, nodeId)
    ...
    def _disconnected(self):
        ...
        publishingThread.queueWork(lambda: pub.sendMessage("blackLager.connection.lost", interface=self))
    ...
    def _connected(self):
        ...
        if not self.isConnected.is_set():
            ...
            publishingThread.queueWork(lambda: pub.sendMessage("blackLager.connection.established", interface=self))
    ...
    def _handleFromRadio(self, fromRadioBytes):
        ...
        if fromRadio.HasField("my_info"):
            ...
            if self.myInfo.min_app_version > OUR_APP_VERSION:
                failmsg = "This device needs a newer python client, run 'pip install --upgrade blackLager'."\
                          "For more information see https://tinyurl.com/5bjsxu32"

            # check for firmware too old
            if self.myInfo.max_channels == 0:
                failmsg = "This version of blackLager-python requires device firmware version 1.2 or later. "\
                          "For more information see https://tinyurl.com/5bjsxu32"
            ...
            publishingThread.queueWork(lambda: pub.sendMessage("blackLager.node.updated",
    ...
    def _handlePacketFromRadio(self, meshPacket, hack=False):
        ...
        - blackLager.receive.text(packet = MeshPacket dictionary)
        - blackLager.receive.position(packet = MeshPacket dictionary)
        - blackLager.receive.user(packet = MeshPacket dictionary)
        - blackLager.receive.data(packet = MeshPacket dictionary)
        """
        ...
        # asObj = DotMap(asDict)
        topic = "blackLager.receive"  # Generic unknown packet type
        ...
        topic = f"blackLager.receive.data.{portnum}"
        ...
        if handler is not None:
            topic = f"blackLager.receive.{handler.name}"
