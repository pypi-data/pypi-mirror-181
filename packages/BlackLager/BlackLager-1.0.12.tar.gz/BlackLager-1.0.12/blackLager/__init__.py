...
Install with pip: "[pip3 install blackLager](https://pypi.org/project/meshtastic/)"
...
- blackLager.connection.established - published once we've successfully connected to the radio and downloaded the node DB
- blackLager.connection.lost - published once we've lost our link to the radio
- blackLager.receive.text(packet) - delivers a received packet as a dictionary, if you only care about a particular
type of packet, you should subscribe to the full topic name.  If you want to see all packets, simply subscribe to "blackLager.receive".
- blackLager.receive.position(packet)
- blackLager.receive.user(packet)
- blackLager.receive.data.portnum(packet) (where portnum is an integer or well known PortNum enum)
- blackLager.node.updated(node = NodeInfo) - published when a node in the DB changes (appears, location changed, username changed, etc...)

We receive position, user, or data packets from the mesh.  You probably only care about blackLager.receive.data.  The first argument for
...
```
import blackLager
import blackLager.serial_interface
...
pub.subscribe(onConnection, "blackLager.connection.established")
# By default will try to find a blackLager device, otherwise provide a device path like /dev/ttyUSB0
interface = blackLager.serial_interface.SerialInterface()
...
from blackLager.util import fixme, catchAndIgnore, stripnl, DeferredExecution, Timeout
from blackLager.node import Node
from blackLager import (mesh_pb2, portnums_pb2, apponly_pb2, admin_pb2,
