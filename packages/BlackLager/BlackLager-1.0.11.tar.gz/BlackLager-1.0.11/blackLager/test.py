...
from blackLager.__init__ import BROADCAST_NUM
from blackLager.serial_interface import SerialInterface
from blackLager.tcp_interface import TCPInterface
...
def testAll(numTests=5):
    ...
    """
    ports = blackLager.util.findPorts(True)
    if len(ports) < 2:
        blackLager.util.our_exit("Warning: Must have at least two devices connected to USB.")
...
def testSimulator():
    """
    Assume that someone has launched blackLager-native as a simulated node.
    ...
    Run with
    python3 -c 'from blackLager.test import testSimulator; testSimulator()'
    """
