...
from blackLager.mesh_interface import MeshInterface
from blackLager.util import stripnl, is_windows11
...
class StreamInterface(MeshInterface):
    """Interface class for blackLager devices over a stream link (serial, TCP, etc)"""
    ...
    def __reader(self):
        ...
        try:
            ...
            if not self._wantExit:  # We might intentionally get an exception during shutdown
                logging.error(f"Unexpected OSError, terminating blackLager reader... {ex}")
        except Exception as ex:
            logging.error(f"Unexpected exception, terminating blackLager reader... {ex}")
        finally:
