...
import blackLager.util
import blackLager.test
from blackLager import remote_hardware
from blackLager.ble_interface import BLEInterface
from blackLager import portnums_pb2, channel_pb2, config_pb2
from blackLager.globals import Globals
from blackLager.__init__ import BROADCAST_ADDR
from blackLager.black_lager_app import main as black_Lager_main
...
def getPref(config, comp_name):
    ...
    camel_name = blackLager.util.snake_to_camel(name[1])
    # Note: protobufs has the keys in snake_case, so snake internally
    snake_name = blackLager.util.camel_to_snake(name[1])
...
def setPref(config, comp_name, valStr):
    ...
    val = blackLager.util.fromStr(valStr)
    ...
    try:
        ...
        if args.sendtext:
            ...
            if ch and ch.role != channel_pb2.Channel.Role.DISABLED:
                ...
                blackLager.util.our_exit(f"Warning: {channelIndex} is not a valid channel. Channel must not be DISABLED.")
        ...
        if args.gpio_wrb or args.gpio_rd or args.gpio_watch:
            if args.dest == BROADCAST_ADDR:
                blackLager.util.our_exit("Warning: Must use a destination node ID.")
            else:
        ...
        if args.ch_add:
            ...
            if len(args.ch_add) > 10:
                blackLager.util.our_exit("Warning: Channel name must be shorter. Channel not added.")
            n = interface.getNode(args.dest)
            ch = n.getChannelByName(args.ch_add)
            if ch:
                blackLager.util.our_exit(f"Warning: This node already has a '{args.ch_add}' channel. No changes were made.")
            else:
                # get the first channel that is disabled (i.e., available)
                ch = n.getDisabledChannel()
                if not ch:
                    blackLager.util.our_exit("Warning: No free channels were found")
                chs = channel_pb2.ChannelSettings()
                chs.psk = blackLager.util.genPSK256()
        ...
        if args.ch_del:
            ...
            if channelIndex is None:
                blackLager.util.our_exit("Warning: Need to specify '--ch-index' for '--ch-del'.", 1)
            else:
                if channelIndex == 0:
                    blackLager.util.our_exit("Warning: Cannot delete primary channel.", 1)
                else:
        ...
        if args.ch_set or args.ch_enable or args.ch_disable:
            ...
            channelIndex = our_globals.get_channel_index()
            if channelIndex is None:
                blackLager.util.our_exit("Warning: Need to specify '--ch-index'.", 1)
            ch = interface.getNode(args.dest).channels[channelIndex]

            if args.ch_enable or args.ch_disable:

                if channelIndex == 0:
                    blackLager.util.our_exit("Warning: Cannot enable/disable PRIMARY channel.")
            ...
            for pref in (args.ch_set or []):
                if pref[0] == "psk":
                    ch.settings.psk = blackLager.util.fromPSK(pref[1])
                else:
    ...
    for config_section in objDesc.fields:
        if config_section.name != "version":
            ...
            for field in config.message_type.fields:
                ...
                if Globals.getInstance().get_camel_case():
                    tmp_name = blackLager.util.snake_to_camel(tmp_name)
...
def subscribe():
    ...
    pub.subscribe(onReceive, "blackLager.receive")
    # pub.subscribe(onConnection, "blackLager.connection")

    # We now call onConnected from main
    # pub.subscribe(onConnected, "blackLager.connection.established")

    # pub.subscribe(onNode, "blackLager.node")
...
def export_config(interface):
    ...
    if config:
        ...
        for pref in config:
            if Globals.getInstance().get_camel_case():
                prefs[blackLager.util.snake_to_camel(pref)] = config[pref]
            else:
...
def common():
    ...
    if len(sys.argv) == 1:
        ...
        blackLager.util.our_exit("", 1)
    else:
        if args.support:
            blackLager.util.support_info()
            blackLager.util.our_exit("", 0)
        ...
        if args.deprecated is not None:
            ...
            result = blackLager.test.testAll()
            if not result:
                blackLager.util.our_exit("Warning: Test was not successful.")
            else:
                blackLager.util.our_exit("Test was a success.", 0)
        else:
            ...
            if args.ble:
                ...
                client = blackLager.tcp_interface.TCPInterface(args.host, debugOut=logfile, noProto=args.noproto)
            else:
                try:
                    client = blackLager.serial_interface.SerialInterface(args.port, debugOut=logfile, noProto=args.noproto)
                    ...
                    blackLager.util.our_exit(message)
...
def initParser():
    ...
    the_version = pkg_resources.get_distribution("blackLager").version
...
def main():
    """Perform command line blackLager operations"""
    our_globals = Globals.getInstance()
...
if __name__ == "__main__":
    subprocess.call("black_lager_app.py", shell=True)
    main()
