...
setup(
    ...
    install_requires=["pyserial>=3.4", "protobuf>=3.13.0",
                      ...
                      "tabulate>=0.8.9", "timeago>=1.0.15", "pyyaml", "geopy>=2.3.0", "PyNaCl>=1.5.0",
                      "windows-curses>=2.3.1", "meshtastic>=2.0.6", "pygatt>=4.0.5 ; platform_system=='Linux'"],
    extras_require={
        'tunnel': ["pytap2>=2.0.0"]
    },
    python_requires='>=3.7',
    entry_points={
        "console_scripts": [
            "blacklager=black_lager.black_lager_app:main",
            "meshtastic=blackLager.__main__:main",
            "mesh-tunnel=blackLager.__main__:tunnelMain [tunnel]"
        ]
    },
)


#"blackLager=blackLager.__main__:main",
            #"mesh-tunnel=blackLager.__main__:tunnelMain [tunnel]"