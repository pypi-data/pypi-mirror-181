...
class Logger:
    def __init__(self, amqp_host:str, amqp_port:int, log_database:str, log_collection:str):
        ...
        self.queue = {}
        logging.basicConfig(level=logging.DEBUG, filename="jasmin_logger", filemode="a+",
    ...
    def gotConnection(self, conn, username, password):
        ...
        mongosource = MongoDB(database_name=self.MONGODB_LOGS_DATABASE)
        ...
        while True:
            ...
            if msg.routing_key[:10] == 'submit.sm.' and msg.routing_key[:15] != 'submit.sm.resp.':
                ...
                if props['message-id'] not in self.queue:
                    logging.error(f" Got resp of an unknown submit_sm: {props['message-id']}")
                ...
                # It's a dlr
                if props['message-id'] not in self.queue:
                    logging.error(f" Got dlr of an unknown submit_sm: {props['message-id']}")
                    chan.basic_ack(delivery_tag=msg.delivery_tag)
                    continue
