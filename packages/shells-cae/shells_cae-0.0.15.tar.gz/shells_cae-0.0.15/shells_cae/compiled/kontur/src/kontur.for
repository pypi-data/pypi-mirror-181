...
C      real*8 RES(12)
     ...
     <DCML2(60),DCMX2(60)
      ...
      data tx/'''  M(1-N3)=''','''CXЭ(1-N3)=''','''CYA(1-N3)=''',
     ...
     *,0.744,0.748,0.753,0.757,0.761,0.765,.744,.782,.79,.798,.806/
      data jj/11/,nos/'(цилиндр)     ','(конус)       ',
     <'(оживало обр.)','(оживало прм.)','(степень)     ',
     <'(парабола)    '/
      OPEN(UNIT=1,FILE='InPut.DAT',STATUS='OLD')
      OPEN(UNIT=2,FILE='Out.DAT')
 106  FORMAT(1X,9HИзделие N,I2,5X,7HОбраз N,I2,5X,9HВариант N,I2,5X,3
     *HAl=,F9.4,5X,2HD=,F6.4)
 107  FORMAT(' Mo=',F7.3,2X,'dM =',F7.3,2X,'al =',F7.3,2X,'dal=',F
     *7.3,2X,'Yint =',F7.3)
 1000 FORMAT(6E9.5)
 1001 FORMAT(15x,'Геометрические характеристики :'/(I6,3X,4f8.4,2f8.4))
 1002 FORMAT(1X,4HAnl=F8.5,2X,4HAla=F8.5,2X,3HBl=F8.5,2X,4HDln=F8.5)
 2000 FORMAT(1X,3HNг=,I2,2X,3HNн=,I2,2X,3HNк=,I2,2X,4HNFL=,I1,2X,
     *7HIPrint=,I1,2X,4HMal=,I2,2X,
     *4HMax=,I2,2X,4HIpr=,I2,2X,3HC2=,F6.4,2X,
     *2Hf=,F5.3,2X,3HRr=,F8.6)
 1005 FORMAT(10F7.4)
      sim(72:78)='      '
C      write(*,'(1x, "Начинается расчет аэродинамики по КОНТУРу.")')
C      write(*,'(1x,a15)') 'Показать? (y/n)'
C      read(*,'(a1)') sc
      SC='y'
C      RJA1=0.
C      RJA2=0.
...
C  55  END DO
C
C      Переменная MVAR нужна была для расчета нескольких форм снарядов
C      Судя по оператору rewind 1 это не работало (вообще)
C      Каждый раз считалась одна и та же форма! //Y.Pavlov
C
C      READ(1,*) MVAR
C      RREF=0.5
c      PI=3.1415926536
...
C         read(1,'(a72,7x)') sim
C         write(2,*) sim
C         sim(48:78)='(Вх. данные из файла InPuT.dat)'
C         write(*,*) sim
C      end if
      NGOL=NG+1
      NHBS=NH+1
...
c
      ...
      if(sc.ne.'n') then
         jj=jj+1
         if(jj.gt.10) then
            write(*,'(1x,''_'',''MAX'',2(''_''),''Cxтр'',
     <      2(''_''),''Cxвлн'',2(''_''),''Cxпс'',4(''_''),
