...
class Series:
    ...
    def _operator(self, sql):
        ...
        return Series(
            _axis_value,
            self.name, self.unique, 'loc.operator')
...
class Matrix:
    ...
    def __init__(self, data: MatrixGetDataType = None, axis_name: _MatrixAxisNameType = None, unique=True, sort=True,
            ...
            if type(item[0]) in [str, int, slice, Series]:
                ...
                if type(item) == str:
                    ...
                    if item.dtype == 'loc.operator':
                        ...
                        # 特别注意：如果multi_series里有返回dataframe的，必须要放在最后一个
                        # 如'bool'类型和'loc.unite'类型
                    ...
                    _axis_name.pop(self._dimension_num_dict[item])
                    i = 0
                    ...
                    return DataFrame(_data[0], _df_name, name=str(self._dimension_num_dict[item]) + '==' + str(
                        self.axis_name[self._dimension_num_dict[item]][0]))
                elif type(item) == int:
                ...
                if type(item[0][0]) == str:
                    ...
                    if type(item[0][1]) == slice:
                        ...
                        _axis_value = self.axis_name[_name]
                        axis_value_list = []
                        _res = np.where(np.array(_axis_value) == item[0][1])[0][0]
                        axis_value_list.append(_res)
                        select_list = axis_value_list
                    ...
                    return Matrix(_value, self.axis_name, unique=self.unique)
...
def matrix_fillna(arr: np.ndarray):
    # 目前适用于二维矩阵
    mask = np.isnan(arr)
