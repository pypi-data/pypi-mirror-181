...
class Plotter:
    ...
    def plot_dose(self):
        ...
        dosevalues = self.df.medium2_concentration[~self.df.medium2_concentration.isna()]
    ...
    def plot_generations(self):
        try:
            ax4 = self.ax.twinx()
            ax4.plot()
            df = self.df.log2_dilution_coefficient[~self.df.log2_dilution_coefficient.isna()]
            ax4.plot(df.index/3600, df.values, ".--", color="g", label="generation [log2(dilution coefficient)]")
