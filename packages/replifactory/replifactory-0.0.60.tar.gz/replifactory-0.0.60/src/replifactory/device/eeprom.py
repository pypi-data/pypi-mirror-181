...
class EEPROM:
    ...
    def __init__(self, device):
        self.device = device
        self.port = None
        self.loaded_device_keys = ['calibration_coefs_od', 'calibration_coefs_pumps', 'calibration_fan_speed_to_duty_cycle',
                    'calibration_milk_to_mv', 'calibration_milk_to_od', 'calibration_od_to_mv',
                    'calibration_pump_rotations_to_ml', 'drying_prevention_pump_period_hrs',
    ...
    def save_config_to_eeprom(self, from_file=False):
        ...
        if from_file:
            ...
            config_to_write = {k: self.device.__dict__[k] for k in self.device.__dict__ if k in self.loaded_device_keys}
        ...
        for page in range(512):
            ...
            if page % 20 == 0:
                print("Writing to EEPROM: %d%% done" % (page / 5.12), end="                              \r")
    ...
    def erase_memory(self):
        for page in range(512):
            ...
            if page % 10 == 0:
                print("Erasing EEPROM: %d%% done" % (page / 5.12), end="                 \r")
