...
class CalibrationViewerTab:
    def __init__(self, main_gui):
        ...
        self.widget = None
        self.refresh_button = widgets.Button(icon="fa-sync-alt", tooltip="Refresh", button_style="info",
    ...
    def update(self):
        self.widget = VBox([self.refresh_button, self.output], layout=Layout(height="1200px"))
...
class CalibrationTab:
    def __init__(self, main_gui):
        ...
        self.dev_config_dir = widgets.Dropdown(options=self.config_paths, description="", layout=layout,
                                               style={"description_width": "initial"}, index=None)
        self.load_from_device_button = widgets.Button(description="load from device", layout=layout, icon="fa-download")
        self.load_from_device_button.on_click(self.handle_load_from_device_button)
        self.save_to_device_button = widgets.Button(description="save to device", icon="fa-upload",layout=layout)
        self.save_to_device_button.on_click(self.handle_save_to_device_button)

        self.load_from_directory_button = widgets.Button(description="load from directory", icon="fa-file-download", layout=layout)
        self.load_from_directory_button.on_click(self.handle_load_from_directory_button)
        self.fit_functions = widgets.Button(description="fit calibration functions", icon="fa-cogs",layout=layout)
        self.fit_functions.on_click(self.handle_fit_calibration_button)
        self.refresh_button = widgets.Button(description="", icon="fa-sync-alt", tooltip="load calibration GUI",
        ...
        self.load_tab = VBox([HBox([self.load_from_device_button, self.save_to_device_button]),
                              HBox([self.load_from_directory_button, self.dev_config_dir]),
                              self.fit_functions,
                              self.output], layout=Layout(width="700px"))
        self.calibrate_tab = VBox([self.refresh_button, self.calibration_accordions_output], layout=Layout(width="700px"))

        self.view_tab = CalibrationViewerTab(self.main_gui).widget
        self.widget = widgets.Tab(children=[self.load_tab, self.calibrate_tab, self.view_tab], titles=["Load", "Calibrate", "View"])
        self.update()
    ...
    def handle_save_to_device_button(self, b):
        ...
        with self.output:
            ...
            print("Saved calibration data to device %s" % self.main_gui.device.ftdi_address)
    ...
    def handle_load_from_device_button(self, b):
        with self.output:
            ...
            print("Loaded calibration data from device %s" % self.main_gui.device.ftdi_address)
        ...
        with self.output:
            ...
            if os.path.exists(self.main_gui.experiment.device.directory):
                ...
                print("Saved calibration data to experiment directory: \n%s" % os.path.relpath(
                    self.main_gui.experiment.device.directory))
...
class SingleODAutoCalibrationTab:
    def __init__(self, main_od_autocalibration_gui, vial_number):
        ...
        self.run_button = widgets.Button(description="start v%d" % self.vial_number,
                                         tooltip="start serial dilution in vial %d" % vial_number)
        self.run_button.on_click(self.run)
        self.output = widgets.Output()
        self.widget = widgets.VBox([self.vial_volume, self.added_milk, self.run_button, self.output],
        ...
        try:
            with self.main_od_autocalibration_gui.output:
                ...
                for dilution_number in range(1, 15):
                    ...
                    print("Dilution %d: milk concentration: %.5f; Measured signal:" % (
                    dilution_number, current_milk_fraction),
                          "[%.3fmV, %.3fmV, %.3fmV]." % tuple(replicate_measurements), "time:", time.ctime())
...
class MilkODCorrectorTab:
    def __init__(self, device):
        ...
        self.plot_milk_button = widgets.Button(description="plot milk:signal",
        ...
        self.milk_concentrations = [
            widgets.FloatText(value=np.nan, description="milk", description_tooltip="milk concentration") for i in
            range(7)]
        self.milk_concentrations_widget = widgets.VBox(self.milk_concentrations)
        self.ods = [widgets.FloatText(value=np.nan, description="OD",
                                      description_tooltip="Measured OD of given milk concentration") for i in range(7)]
    ...
    def handle_plot_milk_button(self, button):
        with self.output:
            ...
            try:
                ...
                plt.close()
                print(
                    "Could not plot milk:OD data associated with device. Please use the fields above to fill it, then convert it.")
...
class ODAutoCalibrationTab:
    def __init__(self, device):
        ...
        self.added_milk = widgets.FloatText(description="added milk", description_tooltip="volume of milk added [mL]",
                                            value=0.500, step=0.1, layout=Layout(width="150px"))
        self.fill_vials = widgets.Button(description="fill vials",
        ...
        self.vial_widgets_list = [SingleODAutoCalibrationTab(main_od_autocalibration_gui=self, vial_number=v) for v in
                                  range(1, 8)]
        ...
        self.widget = VBox([self.header, self.vial_widgets, self.plot_output, self.milk_to_od.widget],
...
class CalibrateOD:
    def __init__(self, device):
        ...
        self.measure_signals_button = widgets.Button(description="measure",
                                                     tooltip="measure and log signal in all OD sensors")
