...
from rga.base import Interface, SerialInterface, TcpipInterface, Component
...
class Scans(Component):
    ...
    def get_analog_scan(self, uga_reset=False):
        ...
        with self.comm.get_lock():
            ...
            for index in range(total_points):
                self.spectrum[index] = self.comm._read_long()
            ...
            if uga_reset:
                ...
                self.total_current = 0
                self.total_current = self.comm._read_long()
        ...
        if uga_reset:
            ...
            if length > 4:
                ...
                self.total_current = int.from_bytes(last_data, 'little', signed=True)
    ...
    def get_single_mass_scan(self, mass):
        ...
        intensity = 0
        with self.comm.get_lock():
            self.comm._send('MR{}'.format(mass))
            intensity = self.comm._read_long()
        return intensity
    ...
    def get_peak_intensity_from_analog_scan_spectrum(self, mass, fit=False):
        x = self.mass_axis
        y = self.spectrum
        distance = 0.5
