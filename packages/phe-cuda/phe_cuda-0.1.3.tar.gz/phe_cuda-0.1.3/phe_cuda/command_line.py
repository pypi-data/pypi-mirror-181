...
def generate_keypair(keysize, id, output):
    ...
    pub, priv = phe_cuda.generate_paillier_keypair(n_length=keysize)
    ...
    jwk_public = {
        ...
        'alg': "PAI-GN1",
        "key_ops": ["encrypt"],
        'n': phe_cuda.util.int_to_base64(pub.n),
    ...
    jwk_private = {
        'kty': "DAJ",
        'key_ops': ["decrypt"],
        'p': phe_cuda.util.int_to_base64(priv.p),
        'q': phe_cuda.util.int_to_base64(priv.q),
        'pub': jwk_public,
...
def decrypt(private, ciphertext, output):
    ...
    _p = phe_cuda.util.base64_to_int(privatekeydata['p'])
    _q = phe_cuda.util.base64_to_int(privatekeydata['q'])

    private_key = phe_cuda.PaillierPrivateKey(pub, _p, _q)
...
def load_public_key(public_key_data):
    ...
    n = phe_cuda.util.base64_to_int(public_key_data['n'])
    pub = phe_cuda.PaillierPublicKey(n)
    return pub
...
def load_encrypted_number(enc_number_file, pub):
    ...
    enc = phe_cuda.EncryptedNumber(pub,
