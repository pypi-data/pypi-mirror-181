    ...
    def moduleEigengenes(expr, colors, impute=True, nPC=1, align="along average", excludeGrey=False, grey="grey",
        ...
        for i in range(len(modlevels)):
            ...
            try:
                ...
                u = u[:, 0:min(n, p, nPC)]
                v = v[0:min(n, p, nPC), :]
                print(datModule.T.shape)
                print("AAAA")
                print(min(n, p, nVarExplained))
                print("BBBB")
                print(v[0:min(n, p, nVarExplained), :].shape)
                tmp = datModule.copy()
                tmp = tmp.append(dict(zip(tmp.columns, v[0:min(n, p, nVarExplained), :])), ignore_index=True)
                print("CCCC")
                print(tmp.shape)
                print(tmp)
