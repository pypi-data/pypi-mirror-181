...
class Comparison:
    ...
    def calculate_jaccard_similarity(self):
        ...
        names = []
        for network in self:
            num = num + len(network.datExpr.var.moduleColors.unique())
            tmp = [f"{network.name}:" + s for s in network.datExpr.var.moduleColors.unique().tolist()]
            names = names + tmp
        jaccard_similarity = pd.DataFrame(0, columns=names, index=names)

        for network1 in networks:
            for network2 in networks:
                if network1 != network2:
                    modules1 = network1.datExpr.var.moduleColors.unique().tolist()
                    modules2 = network2.datExpr.var.moduleColors.unique().tolist()
                    for module1 in modules1:
                        for module2 in modules2:
                            list1 = network1.datExpr.var.index[network1.datExpr.var.moduleColors == module1].tolist()
                            list2 = network2.datExpr.var.index[network2.datExpr.var.moduleColors == module2].tolist()
                            jaccard_similarity.loc[
                                f"{network1.name}:{module1}", f"{network2.name}:{module2}"] = jaccard(
                                list1, list2)
