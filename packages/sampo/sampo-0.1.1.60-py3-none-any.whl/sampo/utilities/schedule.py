...
def merge_split_stages(real_task_df: pd.DataFrame) -> pd.Series:
    """
    Merge split stages of the same real task into one
    :param real_task_df: pd.DataFrame: one real task's stages dataframe, sorted by start time
    :return: pd.Series with the full information about the task
    """
    task_df = real_task_df.copy()
    if len(task_df) == 1:
        ...
        df.loc[0, 'successors'] = fixed_connections_lst
