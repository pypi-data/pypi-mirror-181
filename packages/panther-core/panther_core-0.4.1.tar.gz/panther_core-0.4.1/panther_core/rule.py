...
class Detection(ABC):
    ...
    def __init__(self, config: Mapping):
        ...
        if config.get("filters", None) is None:
            self.filters: List[Filter] = []
        elif isinstance(config["filters"], Filter):
            self.filters = [config["filters"]]
        elif isinstance(config["filters"], list) and all(
            isinstance(f, Filter) for f in config["filters"]
        ):
            self.filters = config["filters"]
        else:
            self._setup_exception = TypeError(
                "expected 'filters' to be of type Union[List[Filter], Filter, None]"
            )
            self.logger.info(
                "error while parsing filters for detection %s",
                self.detection_id,
                exc_info=self._setup_exception,
            )
