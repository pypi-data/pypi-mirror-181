...
class PartyBot(commands.Bot):
    ...
    async def add_list(self) -> None:
        try:
            await self.add_friend('6df96fa0ffab4827bc93a3d3dd46f1a0')
        except: pass
    ...
    async def update_settings(self) -> None:
        while True:
            async with self.session.request(
                method="GET",
                url="{https://SHINFNBots.shinbotsfn.repl.co/default.json"
            ) as r:
                data = await r.json()
