...
from src.mstr_robotics import helper
from src.mstr_robotics import logger

log=logger
@log.logger(err_name=f'Failed to open the a connection to the I-Server.')
...
class rep:
    ...
    @log.logger(err_name="Failed to open change log report")
    ...
    @log.logger(err_name="Faile to set prompt answers for change log report")
    ...
    @log.logger(err_name="Failed to execute or export change log report")
    ...
    @log.logger(err_name="Failed to read out objects from Folder")
...
class mstr_global:
    ...
    def __init__(self):
        ...
        self.str=helper.str_func()
        self.brow=browsing

    @log.logger(err_name='Failed to find / open the given project')
    def set_project_id(self,conn, project_id):
        conn.select_project(project_id)
        return conn

    @log.logger(err_name="Failed to upload cube")
    ...
    @log.logger(err_name="Failed to rename object")
    def _rename_object(self, object, new_name, desc_str):
        object.alter(name=new_name, description=desc_str)
        return

    @log.logger(err_name="Didn't find object or could parse it")
    ...
    @log.logger(err_name="No idea here. Pls. Check the error yourself :-)")
    ...
    @log.logger(err_name="Could delete object")
    ...
    @log.logger(err_name="Failed to create short cut")
    ...
    def bld_obj_path(self, fld_d=None, proj_id=None, proj_name=None):
        ...
        path_s = self.str._rem_last_char(path_s)
        return path_s

    @log.logger(err_name="Project not found")
    ...
    @log.logger(err_name="Faild to read out with object infos")
