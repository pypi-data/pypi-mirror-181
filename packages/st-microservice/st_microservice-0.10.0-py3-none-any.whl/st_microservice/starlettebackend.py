...
from .database import get_async_engine, AsyncDBMiddleware
...
# App factory
def create_app(routes: list[BaseRoute], secret: str, root_domain: str, database_uri: str | None, debug=False, database_ssl=False) -> Starlette:
    ...
    db_connect_args = {'sslmode': 'require'} if database_ssl else None

    middleware = [
        Middleware(CORSMiddleware, allow_origin_regex=root_domain_re, allow_credentials=True, allow_methods=['*']),
        Middleware(AsyncDBMiddleware, engine=get_async_engine(database_uri, db_connect_args)),
