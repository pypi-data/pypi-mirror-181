...
from sqlalchemy import MetaData, Table, Row, select, Executable, cast, String
...
def get_db_conn(info) -> AsyncConnection:
    return info.context['request'].state.db


async def get_or_none(db: AsyncConnection, table: T, primary_keys: dict[str, Any]) -> Row | None:
    q = select(table)
    for pk, val in primary_keys.items():
        q = q.where(getattr(table.c, pk) == val)
...
async def get_or_error(db: AsyncConnection, table: T, primary_keys: dict[str, Any]) -> Row:
    try:
...
async def get_all_records(info, q: Executable):
    return (await get_db_conn(info).execute(q)).all()


async def get_one_record(info, q: Executable):
    return (await get_db_conn(info).execute(q)).one()


async def get_one_field(info, q: Executable):
    return (await get_db_conn(info).execute(q)).scalar_one()
...
def resolve_type_inspector(_, info: GraphQLResolveInfo, type_name: str):
    ...
    return {'field_details': field_details, 'primary_keys': []}  # Todo: Fix Primary keys
...
def resolve_type_loader_factory(table_metadata: MetaData):
    ...
    async def resolve_type_loader(_, info, type_name, filters: list[FieldFilter], limit: int, offset: int):
        ...
        rows = []
        for rowm in (await get_db_conn(info).execute(q)).mappings().all():
            row = dict(rowm)
