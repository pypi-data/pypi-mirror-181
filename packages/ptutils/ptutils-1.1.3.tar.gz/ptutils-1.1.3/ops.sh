...
function action(){
  ...
  while [ $# -gt 0 ] && [[ "$1" =~ ^-.* ]]; do
    ...
    case "$param" in
      ...
      # -c|--context-file)  local ctx="$1";         shift ;;
      # -l|--context-label) local ctxlbl="$1";      shift ;;
  ...
  # if [ ! -z "$ctx" ]; then
  #   echo "  🗋  ${ctxlbl:-'Context:'}" 1>&2
  #   if [ -f "$ctx" ]; then
  #     cat  "$ctx" | sed 's/^/    | /' 1>&2
  #   else
  #     echo "     <Context file is missing>" 1>&2
  #   fi
  # fi
  if [ "${RC}" -eq 0 ]; then
...
function run(){
  ...
  action "$@" || errexit "Failed to run command: $*"
  # local action="$1"
  # shift
  # echo "ACTION: ${action}"
  # echo "  RUN:  $*"
...
function do_action(){
  case "$1" in
    ...
    coverage)
      shift
      activate_venv
      action -t "Generate Coverage Data" pytest --cov-config=./.coveragerc --cov=ptutils --cov-report html -vv --basetemp="${FOLDER}/ptutils" "$@" || errexit "Failed to generate code coverage report."
    ;;
    ...
    quality)
      ...
      rm ./op.log
      touch ./op.log
      action -t "Compute Cyclomatic Complexity (CC)" radon cc -s -i '__pending*' ./ptutils || errexit "Failed to compute code complexity."
      action -t "Compute Maintainability Index (MI)" radon mi -s -i '__pending*' ./ptutils || errexit "Failed to compute code maintainability."
      action -t "Compute raw statistics (RAW)" radon raw -s -i '__pending*' ./ptutils || errexit "Failed to compute code statistics."
      action -t "Analyze Code Quality" xenon -b C -m A -a A -i '__pending*' ./ptutils || errexit "Failed to analyze code quality."
    ;;
    ...
    style)
      ...
      activate_venv
      export PYTHONUNBUFFERED=yes
      action -t  "Check Source code Style Compliance" flake8 --max-line-length=120 --ignore=E201,E202,E401,E221,E241,E251,E271,W504,E272 --exclude '__pending*' ./ptutils || errexit "Failed code style checks."
    ;;
    -h|--help|help)
      usage
    ;;
    ...
    release-major)
      ...
      action -t  "Push changes" git push || errexit "Failed to puch code."
