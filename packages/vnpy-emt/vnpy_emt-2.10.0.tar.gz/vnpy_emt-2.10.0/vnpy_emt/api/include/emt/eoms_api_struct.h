...
///分页查询OTC持仓请求
//////////////////////////////////////////////////////////////////////////
struct EMTQueryOtcPositionByPageReq
{
    ///需要查询的订单条数
    int64_t         req_count;
    ///上一次收到的查询订单结果中带回来的索引，如果是从头查询，请置0
    int64_t         reference;
    ///证券代码，可以为空，如果为空，则默认查询所有持仓
    char      ticker[EMT_TICKER_LEN];
    ///交易市场
    EMT_MARKET_TYPE     market;
};
...
typedef struct
{
    char                               stkCode[16];                   // 合约代码
    char                               stkName[64];                   // 合约名称
    EMT_MARKET_TYPE                    market;                        // 总资产
    int64_t                            totalQty;                      // 总持仓
    int64_t                            sellableQty;                   // 可卖持仓
    double                             unrealizedPnl;                 // 浮动盈亏
    int64_t                            yesterdayPosition;             // 昨日持仓
    EMT_POSITION_DIRECTION_TYPE        positionDirection;             // 持仓方向
    double                             costPrice;                     // 成本价
    double                             positionAmt;                   // 持仓成本
}EMTOtcPositionInfo;
...
typedef struct
{
    ...
    double       buyingPower;                   // 可用资金
    double       securityAsset;                 // 证券资产
    double       fundBuyAmount;                 // 累计买入成交证券占用资金
    double       fundBuyFee;                    // 累计买入成交交易费用
    double       fundSellAmount;                // 累计卖出成交证券所得资金
    double       fundSellFee;                   // 累计卖出成交交易费用
    double       withholdingAmount;             // 场外系统预扣的资金
}EMTOtcAssetInfo;

//////////////////////////////////////////////////////////////////////////
///分页查询OTC委托请求
//////////////////////////////////////////////////////////////////////////
struct EMTQueryOtcOrdersByPageReq
{
    ///需要查询的订单条数
    int64_t req_count;
    ///上一次收到的查询订单结果中带回来的索引，如果是从头查询，请置0
    int64_t reference;
    ///证券代码，可以为空，如果为空，则默认查询当日所有委托回报
    char ticker[EMT_TICKER_LEN];
    ///交易市场
    EMT_MARKET_TYPE market;
};
...
///分页查询OTC成交请求
//////////////////////////////////////////////////////////////////////////
struct EMTQueryOtcTradesByPageReq
{
    ///需要查询的订单条数
    int64_t req_count;
    ///上一次收到的查询订单结果中带回来的索引，如果是从头查询，请置0
    int64_t reference;
    ///证券代码，可以为空，如果为空，则默认查询当日所有成交回报
    char ticker[EMT_TICKER_LEN];
    ///交易市场
    EMT_MARKET_TYPE market;
};
