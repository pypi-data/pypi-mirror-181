...
from gnuhealth import __version__, SERVER_VERSION
import gnuhealth.common as common
from gnuhealth.config import CONFIG, GNUHEALTH_ICON, PIXMAPS_DIR, BANNER, \
     get_config_dir
...
class DBListEditor(object):
    ...
    def __init__(self, parent, profile_store, profiles, callback):
        ...
        self.add_button.set_image(common.IconFactory.get_image(
                'gnuhealth-add', Gtk.IconSize.BUTTON))
    ...
    def test_server_version(cls, host, port):
        ...
        if not version:
            return None
        return version.split('.')[:2] == SERVER_VERSION.split('.')[:2]
...
class DBLogin(object):
    def __init__(self):
        ...
        self.dialog = Gtk.Dialog(title="GNU Health - " + _('Login'), modal=True)
        ...
        tooltips.set_tip(button_cancel,
            _('Cancel connection to the  server'))
