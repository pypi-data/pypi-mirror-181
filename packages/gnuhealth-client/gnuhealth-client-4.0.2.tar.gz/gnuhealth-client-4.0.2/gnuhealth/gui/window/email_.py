...
from gnuhealth.common import IconFactory, Tooltips, RPCExecute, RPCException
from gnuhealth.common.richtext import (
...
from gnuhealth.config import GNUHEALTH_ICON, CONFIG
...
class EmailEntry(Gtk.Entry):
    ...
    def _update_completion(self, text):
        ...
        def callback(results):
            try:
                results = results()
            except (GNUHealthError, GNUHealthServerError):
                logger.warning(
...
class Email(NoModal):
    ...
    def _add_file_button(self):
        ...
        button.set_image(IconFactory.get_image(
                'gnuhealth-remove', Gtk.IconSize.BUTTON))
