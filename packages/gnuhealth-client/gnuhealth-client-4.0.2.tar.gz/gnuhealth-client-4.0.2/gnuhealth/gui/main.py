...
# SPDX-FileCopyrightText: 2017-2022 GNU Health Community <info@gnuhealth.org>
#
...
import gnuhealth.plugins
import gnuhealth.rpc as rpc
import gnuhealth.translate as translate
from gnuhealth.action import Action
from gnuhealth.common import RPCExecute, RPCException, RPCContextReload
from gnuhealth.common.cellrendererclickablepixbuf import \
        CellRendererClickablePixbuf
from gnuhealth.config import CONFIG, GNUHEALTH_ICON, get_config_dir
from gnuhealth.exceptions import GNUHealthError, GNUHealthServerUnavailable
from gnuhealth.gui.window import Window
from gnuhealth.jsonrpc import object_hook
from gnuhealth.pyson import PYSONDecoder

# GNUHealth block: client version and other related imports
import time
import platform
from gnuhealth.gui.window.activity import Activity
...
class Main(Gtk.Application):
    ...
    def do_activate(self):
        ...
        self.window = Gtk.ApplicationWindow(
            application=self, title="GNU Health Hospital Management")
        ...
        menu.set_image(
            common.IconFactory.get_image(
                'gnuhealth-menu', Gtk.IconSize.BUTTON))
        ...
        favorite.set_image(common.IconFactory.get_image(
                'gnuhealth-bookmarks', Gtk.IconSize.BUTTON))
        ...
        Gtk.AccelMap.add_entry(
            '<gnuhealth>/Form/New', Gdk.KEY_N, Gdk.ModifierType.CONTROL_MASK)
        Gtk.AccelMap.add_entry(
            '<gnuhealth>/Form/Save', Gdk.KEY_S, Gdk.ModifierType.CONTROL_MASK)
        Gtk.AccelMap.add_entry(
            '<gnuhealth>/Form/Duplicate', Gdk.KEY_D,
            Gdk.ModifierType.CONTROL_MASK | Gdk.ModifierType.SHIFT_MASK)
        Gtk.AccelMap.add_entry(
            '<gnuhealth>/Form/Delete',
            Gdk.KEY_D, Gdk.ModifierType.CONTROL_MASK)
        Gtk.AccelMap.add_entry(
            '<gnuhealth>/Form/Next', Gdk.KEY_Page_Down, 0)
        Gtk.AccelMap.add_entry(
            '<gnuhealth>/Form/Previous', Gdk.KEY_Page_Up, 0)
        Gtk.AccelMap.add_entry(
            '<gnuhealth>/Form/Switch View', Gdk.KEY_L,
            Gdk.ModifierType.CONTROL_MASK)
        Gtk.AccelMap.add_entry(
            '<gnuhealth>/Form/Close', Gdk.KEY_W, Gdk.ModifierType.CONTROL_MASK)
        Gtk.AccelMap.add_entry(
            '<gnuhealth>/Form/Reload',
            Gdk.KEY_R, Gdk.ModifierType.CONTROL_MASK)
        Gtk.AccelMap.add_entry(
            '<gnuhealth>/Form/Attachments', Gdk.KEY_T,
            Gdk.ModifierType.CONTROL_MASK | Gdk.ModifierType.SHIFT_MASK)
        Gtk.AccelMap.add_entry(
            '<gnuhealth>/Form/Notes', Gdk.KEY_O,
            Gdk.ModifierType.CONTROL_MASK | Gdk.ModifierType.SHIFT_MASK)
        Gtk.AccelMap.add_entry(
            '<gnuhealth>/Form/Relate', Gdk.KEY_R,
            Gdk.ModifierType.CONTROL_MASK | Gdk.ModifierType.SHIFT_MASK)
        Gtk.AccelMap.add_entry(
            '<gnuhealth>/Form/Actions',
            Gdk.KEY_E, Gdk.ModifierType.CONTROL_MASK)
        Gtk.AccelMap.add_entry(
            '<gnuhealth>/Form/Report',
            Gdk.KEY_P, Gdk.ModifierType.CONTROL_MASK)
        Gtk.AccelMap.add_entry(
            '<gnuhealth>/Form/Email', Gdk.KEY_E,
            Gdk.ModifierType.CONTROL_MASK | Gdk.ModifierType.SHIFT_MASK)
        Gtk.AccelMap.add_entry(
            '<gnuhealth>/Form/Search', Gdk.KEY_F,
            Gdk.ModifierType.CONTROL_MASK | Gdk.ModifierType.SHIFT_MASK)
        ...
        try:
            ...
            if (not isinstance(exception, GNUHealthError)
    ...
    def on_quit(self, *args):
        try:
            if not self.close_pages():
                return True
        except GNUHealthServerUnavailable:
            pass
        rpc.logout()
        self.quit()
    ...
    def edit_limit(self):
        from gnuhealth.gui.window.limit import Limit
        Limit().run()
    ...
    def preferences(self):
        from gnuhealth.gui.window.preference import Preference
        if not self.close_pages():
            return False
    ...
    def about(self):
        from gnuhealth.gui.window.about import About
        About()
    ...
    def sig_win_menu(self, prefs=None):
        ...
        if not prefs:
            try:
                prefs = RPCExecute(
                    'model', 'res.user', 'get_preferences', False)
            except RPCException:
                return False
        ...
        screen = Screen(action['res_model'], mode=['tree'], view_ids=view_ids,
                        ...
                        limit=None, row_activate=self.menu_row_activate)
        ...
        favorite_renderer.connect(
            'clicked', lambda *a: GLib.idle_add(toggle_favorite, *a), treeview)
    ...
    def win_add(self, page, hide_current=False):
        ...
        button.add(common.IconFactory.get_image(
                'gnuhealth-close', Gtk.IconSize.MENU))
    ...
    def _open_url(self, url):
        urlp = urlparse(url)
        if not urlp.scheme == 'gnuhealth':
            return
        urlp = urlparse('http' + url[6:])
        database, path = list(
            map(unquote, (urlp.path[1:].split('/', 1) + [''])[:2]))
        if not path:
            return
...
# GNUHEALTH block
    ...
    def init_gnuhealth_env(self):
        ...
        side_menu.set_image(
            common.IconFactory.get_image('gnuhealth-menu',
                                         Gtk.IconSize.BUTTON))
        ...
        self.ghheader_contents.attach_next_to(favorite, side_menu,
                                              Gtk.PositionType.RIGHT, 1, 1)
        self.ghheader_contents.attach_next_to(
            self.global_search_entry, favorite, Gtk.PositionType.RIGHT, 1, 1)
    ...
    def set_ghfooter(self):
        ...
        self.vbox.pack_start(self.ghfooter, expand=False,
    ...
    def activate_cli(self, widget, data):
        ...
        if (command == 'sysinfo'):
            info = f"Request : {command} \n"
            client_info = '\n********* CLIENT INFORMATION *********\n\n'
            server_info = '\n********* SERVER INFORMATION *********\n\n'
            client_info = f"{client_info} {self.get_host_info()}\n"
    ...
    def get_host_info(self):
        ...
        client_version = f"GNU Health GTK Client version: {__version__}"
        pversion = f"Python version : {platform.python_version()}"
        with open('/etc/os-release') as f:
            os_version = f.readlines()
