...
from gnuhealth import bus, device_cookie, fingerprints
from gnuhealth.jsonrpc import ServerProxy, ServerPool, Fault
from gnuhealth.config import get_config_dir
from gnuhealth.exceptions import GNUHealthServerError, GNUHealthServerUnavailable
...
def execute(*args):
    ...
    if CONNECTION is None:
        raise GNUHealthServerError('403')
    try:
        name = '.'.join(args[:3])
        args = args[3:]
        ...
        raise GNUHealthServerUnavailable(*exception.args)
    logging.getLogger(__name__).debug(repr(result))
    return result
