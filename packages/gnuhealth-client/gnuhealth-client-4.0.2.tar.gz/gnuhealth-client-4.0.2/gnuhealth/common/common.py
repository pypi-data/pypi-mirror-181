...
from gnuhealth.config import GNUHEALTH_ICON, PIXMAPS_DIR
import sys
import webbrowser
import traceback
...
from gnuhealth.exceptions import GNUHealthServerError, GNUHealthError
from gnuhealth.pyson import PYSONEncoder
...
class IconFactory:
    ...
    def register_icon(cls, iconname):
        ...
        try:
            ...
            return
        idx = cls._gnuhealth_icons.index(icon_ref)
        to_load = slice(max(0, idx - cls.batchnum // 2),
            idx + cls.batchnum // 2)
        ids = [e[0] for e in cls._gnuhealth_icons[to_load]]
        try:
        ...
        for icon in icons:
            ...
            cls._gnuhealth_icons.remove((icon['id'], icon['name']))
...
def file_write(filename, data):
    ...
    dtemp = tempfile.mkdtemp(prefix='gnuhealth_')
...
def check_version(box, version=__version__):
    ...
    filename = 'gnuhealth-%s.tar.gz' % version
    if hasattr(sys, 'frozen'):
        if sys.platform == 'win32':
            bits = platform.architecture()[0]
            filename = 'gnuhealth-%s-%s.exe' % (bits, version)
        elif sys.platform == 'darwin':
            filename = 'gnuhealth-%s.dmg' % version
...
def process_exception(exception, *args, **kwargs):
    ...
    if isinstance(exception, GNUHealthServerError):
        if exception.faultCode == 'UserWarning':
            ...
            if PLOCK.acquire(False):
                try:
                    Login()
                except GNUHealthError as exception:
...
class Login(object):
    def __init__(self, func=rpc.login):
        ...
        while True:
            try:
                func(parameters)
            except GNUHealthServerError as exception:
                ...
                if value is None:
                    raise GNUHealthError('QueryCanceled')
