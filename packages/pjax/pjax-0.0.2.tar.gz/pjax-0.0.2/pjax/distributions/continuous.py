...
class Normal(Distribution):
    ...
    def sample(
        ...
        z = jax.random.normal(key, full_shape)
...
class StudentT(Distribution):
    ...
    def sample(
        ...
        full_shape = tuple(shape) + tuple(self.batch_shape)
        z = jax.random.t(key, self.df, full_shape)
        return self.mu + self.sigma * z
...
class Cauchy(Distribution):
    ...
    def sample(
        ...
        full_shape = tuple(shape) + tuple(self.batch_shape)
        z = jax.random.cauchy(key, full_shape)
        return self.loc + self.scale * z
...
class SkewNormal(Distribution):
    ...
    def sample(
        ...
        full_shape = tuple(shape) + tuple(self.batch_shape)
        z_1, z_2 = jax.random.normal(key, (2,) + full_shape)
        z = (self.a * jnp.abs(z_1) + z_2) / jnp.sqrt(1 + self.a**2)
        sim = jnp.asarray(self.loc + self.scale * z)
        return sim
