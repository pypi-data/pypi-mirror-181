...
class DucklingParameters(AnnotatorParameters):
    mapping: Dict[str, List[Dimension]] = Field(
        None,
        description="""Map a label to a list of [Duckling](https://github.com/facebook/duckling) dimensions to consider, to be chosen among:<br/>
...
class DucklingAnnotator(AnnotatorBase):
    ...
    def annotate(self, documents: List[Document], parameters: AnnotatorParameters) \
        ...
        if params.mapping is not None:
            ...
            for k, dim_lst in params.mapping.items():
                for dim in dim_lst:
        ...
        for document in documents:
            ...
            for sent in document.sentences:
                ...
                for dim in dims:
                    ...
                    if dname == "time":
                        try:
                            dt = pendulum.parse(props['value'])
                            props['value'] = dt.to_iso8601_string()
                        except ParserError:
                            pass
