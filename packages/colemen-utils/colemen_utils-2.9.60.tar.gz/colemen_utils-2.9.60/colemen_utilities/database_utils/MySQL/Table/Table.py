...
class Table:
    ...
    def gen_hash_id(self):
        ...
        if self.hash_id_prefix is not None:
            return f"{self.hash_id_prefix}_{_rand.rand((self._hash_id_column.validation.max_length - len(self.hash_id_prefix)))}"
    ...
    def insert(self,data:dict,**kwargs)->Union[int,dict,bool]:
        ...
        result = self.database.insert_to_table(self.table_name,data)
        if result:
            if return_row:
                s = self.select_query()
    ...
    def select_query(self):
        ...
        return self.database.select_query(self.table_name)
