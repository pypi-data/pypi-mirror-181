...
# class LoginLayout(HTML5Page):
#     def __init__(self, view, bookmarks, title='Login & Registration', css_id=None):
#         super().__init__(view, title, css_id)

#         self.use_layout(PageLayout(document_layout=Container()))

#         contents_layout = ColumnLayout(ColumnOptions('login_main', size=ResponsiveSize(md=4))).with_slots()
#         self.layout.contents.use_layout(contents_layout)

#         navbar = Navbar(view, css_id='login_nav')
#         navbar.use_layout(ResponsiveLayout('md', collapse_brand_with_content=True))
#         navbar.layout.set_brand_text('Read Time Events')

#         menu = Nav(view).use_layout(TabLayout(content_justification='fill')).with_bookmarks(bookmarks)
...
class LoginForm(Form):
    def __init__(self, view, event_channel_name, account_management_interface):
        ...
        register_bookmark = self.user_interface.get_bookmark(relative_path='/register',
                                                            description=_('Not a member yet? Register here!'))
        self.add_child(A.from_bookmark(view, register_bookmark))
...
class AccountUI(UserInterface):
    """A user-facing UserInterface for logging in and managing your own login.
       ...
       **Slots:**
         - login_main: All UI elements are put into this Slot for any View in this UserInterface.
    ...
    def assemble_login(self):
        login_local = self.define_view('/login', title=_('Log in'), detour=True)
        login_local.set_slot('login_main', LoginWidget.factory(self.account_management_interface))
    ...
    def assemble_reset_password(self):
        ...
        reset_password.set_slot('login_main', ResetPasswordWidget.factory(self.account_management_interface))
        
        choose_password = self.define_view('/choosePassword', title=_('Reset a password: Step 2'))
        choose_password.set_slot('login_main', ChoosePasswordWidget.factory(self.account_management_interface))
        
        password_changed = self.define_view('/passwordChanged', title=_('Password change complete'))
        password_changed.set_slot('login_main', PasswordChangedWidget.factory())
    ...
    def assemble_register_help(self, verify_bookmark):
        ...
        register_help.set_slot('login_main', RegisterHelpWidget.factory(self.account_management_interface))
        
        help_duplicate = self.define_view('/registerHelp/duplicate', title=_('Address already registered'))
        help_duplicate.set_slot('login_main', RegistrationDuplicatedWidget.factory(self.account_management_interface))
        
        help_pending = self.define_view('/registerHelp/pending', title=_('Registration still pending'))
        help_pending.set_slot('login_main', RegistrationPendingWidget.factory(self.account_management_interface, verify_bookmark))
        
        help_reregister = self.define_view('/registerHelp/reregister', title=_('Registration not found'))
        help_reregister.set_slot('login_main', RegistrationNotFoundWidget.factory(self.account_management_interface))
        
        help_sent = self.define_view('/registerHelp/pending/sent', title=_('Registration email sent'))
        help_sent.set_slot('login_main', RegistrationEmailSentWidget.factory(self.account_management_interface))
    ...
    def assemble_register(self):
        ...
        verify.set_slot('login_main', VerifyEmailWidget.factory(self.account_management_interface))
        ...
        register.set_slot('login_main', RegisterWidget.factory(self.bookmarks, self.account_management_interface))        

        congrats = self.define_view('/congrats', title=_('Congratulations'))
        congrats.set_slot('login_main', CongratsWidget.factory(self.account_management_interface, register_help_bookmark, verify_bookmark))
                
        thanks = self.define_view('/thanks', title=_('Thanks for all your troubles'))
        thanks.set_slot('login_main', ThanksWidget.factory())
