...
class gitsh:
    ...
    def enviar(self):
        ...
        total_de_arquivos_modificados_em_todo_o_periudo = f"total de arquivos modificados: {len(hcommit.diff(f'HEAD~{int(total_de_save_feitos)-1}'))}"
        descrcao = master.commit.message
        data = datetime.fromtimestamp(master.commit.committed_date)
        enviar = []
