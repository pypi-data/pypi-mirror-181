...
from requests_async.exceptions import ReadTimeout
    ...
    async def get_available_time_slots(
        ...
        if self._test_mode:
            ...
            try:
                ...
                _logger.error(
                    "Timeout while getting available time slots for %s", self.name
                )
            except Exception as get_meeting_points_e:
                _logger.error(
            ...
            for meeting in meetings[applicationId]:
                try:
                    ...
                    _logger.error(
                        ...
                        extra={
                            "extra_info": {"type": "app", "response": response.json()}
                        },
                    )
...
def init_all_editors():
    ...
    ypok_editor = Editor("ypok", "Ypok", "https://yservice.ypok.com/api.php", False)
    solocal_editor = Editor(
        "solocal", "Solocal", "https://api.solocal.com/v1/ants", False
    )
