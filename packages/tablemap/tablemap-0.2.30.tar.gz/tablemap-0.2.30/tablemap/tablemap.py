...
rowval = str/int/float
    (slash(/) means `or`)
row = dict[str, rowval]
    lowercase, row represents a row in a table, str is a column name (field)
"""
...
class Rows:
    ...
    Rows object can be instantiated in two ways.
        ...
        2. Directly pass a list of dicts(list[dict]) or dicts yielding iterator
        (Iterable[dict])
    ...
    def __getitem__(self, key):
        ...
        list[rowval]/Rows
            ...
            slicing is similar to list slicing except that negative values
            cannot be used
        """
    ...
    def order(self, *fields_maybe_with_desc):
        ...
        Parameters
        ----------
        *fields_maybe_with_desc: list[str]

            str is a field name but it can be with `desc` keyword, for example,
            'col desc'
    ...
    def fold(self, **kwargs):
        ...
        **kwargs: dict[str, Rows -> rowval]
        ...
        # then self._history[-1]['cmd'] must be 'group', it's check above
