...
class PolyActor(QObject, vmat.Menu, vmat.Mouse):
    """三维物体，将vtkPolyData通过vtkPolyDataMapper/vtkActor显示"""

    def __init__(self, view: vmat.View = None, visible: bool = True, pickable: bool = False,
        ...
        vmat.Menu.__init__(self)

        vmat.Mouse.__init__(self)
    ...
    def setData(self, other) -> None:
        ...
        elif isinstance(other, vmat.TopoDS_Shape):
            other = vmat.ccPd_Sh(other)
            self.setData(other)
        elif isinstance(other, vmat.Polyhedron_3):
            other = vmat.cgPd_Ph(other)
            self.setData(other)
        else:
...
class ImageBox(QObject, vmat.Menu, vmat.Mouse):
...
class ValueBox(TextBox):
    ...
    def _ask_value(self, **kwargs):
        value = vmat.askFloat(self._ValueMin, self._Value, self._ValueMax, self._ValueDecimals, self._ValueStep)
...
class ImageSlice(QObject, vmat.Menu, vmat.Mouse):
    """图像断层，将vtkImageData通过vtkImageResliceMapper/vtkImageSlice显示"""

    def __init__(self, view: vmat.View = None, visible: bool = True, pickable: bool = True,
        ...
        vmat.Mouse.__init__(self, menu=self)
...
class ImageVolume(QObject, vmat.Menu, vmat.Mouse):
