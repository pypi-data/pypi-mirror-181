...
def imVTK_SITK(image: Union[sitk.Image, vtk.vtkImageData]) -> vtk.vtkImageData:
    ...
    with tempfile.TemporaryDirectory() as p:
        ...
        image = vmat.imOpenFile_NIFTI(str(p))
        image.SetOrigin(origin)
        return image
...
def imSaveFile_NIFTI(image: vtk.vtkImageData, file=None) -> None:
    ...
    if file is None:
        file = vmat.askSaveFile('nii', '*.nii', '导出 (Export) NIFTI')
        if file is None:
            return
...
def imOpenFile_NIFTI(file=None) -> Optional[vtk.vtkImageData]:
    ...
    if file is None:
        file = vmat.askOpenFile('*.nii', '导入 (Import) NIFTI')
        if file is None:
            return
    with tempfile.TemporaryDirectory() as p:
        p = pathlib.Path(p) / '.nii'
...
def imSaveFile_XML(image: vtk.vtkImageData, file=None) -> None:
    ...
    if file is None:
        file = vmat.askSaveFile('vti', '*.vti', '导出 (Export) XML')
        if file is None:
            return
...
def imOpenFile_XML(file=None) -> Optional[vtk.vtkImageData]:
    ...
    if file is None:
        file = vmat.askOpenFile('*.vti', '导入 (Import) XML')
        if file is None:
            return
    with tempfile.TemporaryDirectory() as p:
        p = pathlib.Path(p) / '.vti'
...
def imIsosurface(image: vtk.vtkImageData, isovalue: Union[int, float]) -> vtk.vtkPolyData:
    ...
    return vmat.pdNormals(pd)


def imReslice(image: vtk.vtkImageData,
              cs: Optional[vmat.CS4x4] = None,
    ...
    if size is None:
        size = [vmat.imSize_Vt(image, cs.axis(0)),
                vmat.imSize_Vt(image, cs.axis(1)),
                vmat.imSize_Vt(image, cs.axis(2))]

    spc = [vmat.imSpacing_Vt(image, cs.axis(0)),
           vmat.imSpacing_Vt(image, cs.axis(1)),
           vmat.imSpacing_Vt(image, cs.axis(2))]
...
def imReslice_Blend(image: vtk.vtkImageData,
    ...
    ary = vmat.imArray_VTK(image)
    ary = blend(ary, scalar_min, scalar_max)

    image = vmat.imVTK_Array(ary, vmat.imOrigin(image), vmat.imSpacing(image))
