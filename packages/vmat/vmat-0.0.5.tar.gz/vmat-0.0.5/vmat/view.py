...
orientation_v = {
    "RAS": {
        "vu": [np.array([0, 1, 0]), np.array([0, 0, 1]), np.array([0, 0, 1])],
        "vr": [np.array([1, 0, 0]), np.array([0, 1, 0]), np.array([1, 0, 0])]
    },
    "LPS": {
...
class View(QGLWidget, Menu, Mouse):
    ...
    def __init__(self, parent=None, view_name="main_view"):
        vmat.appViews.append(self)
    ...
    def __setstate__(self, state):
        ...
        for kw in state['vtk']:
            vmat.vtkInstance_sets(getattr(self, kw), state['vtk'][kw])
    ...
    def __getstate__(self):
        ...
        for kw in state:
            if isinstance(state[kw], vtk.vtkObject):
                if not hasattr(vmat, 'pickle_' + state[kw].__class__.__name__):
                    state_vtk[kw] = vmat.vtkInstance_gets(state[kw])
    ...
    def cameraCS_FPlane(self) -> vmat.CS4x4:
        """返回观察坐标系
    ...
    def setCamera_FPlane(self, cs: vmat.CS4x4, height: float) -> None:
        """
        设置视图的观察坐标系
    ...
    def setCamera_FitAll(self) -> None:
        """设置观察范围适应全部场景可见物体"""
        view_props_bounds = []
        for prop in self._Renderer.GetViewProps():
            bounds = prop.GetBounds()
            if bounds:
                print(bounds)
                if bounds != (-1., 0., -1., 0., -1., 0.):
                    view_props_bounds.append(bounds)
        if len(view_props_bounds) == 0:
            b = np.array(self._Renderer.ComputeVisiblePropBounds())
        else:
            view_props_bounds = np.array(view_props_bounds)
            b = [min(view_props_bounds[:, 0]), max(view_props_bounds[:, 1]),
                 min(view_props_bounds[:, 2]), max(view_props_bounds[:, 3]),
                 min(view_props_bounds[:, 4]), max(view_props_bounds[:, 5]), ]
        dxyz = np.array([b[1] - b[0], b[3] - b[2], b[5] - b[4]])
    ...
    def setCamera_Axial(self) -> None:
        """设置观察者正视横断面"""
        vu = orientation_v["RAS"]["vu"][0]
        vr = orientation_v["RAS"]["vr"][0]
    ...
    def setCamera_Sagittal(self) -> None:
        """设置观察者正视矢状面"""
        vu = orientation_v["RAS"]["vu"][1]
        vr = orientation_v["RAS"]["vr"][1]
    ...
    def setCamera_Coronal(self) -> None:
        """设置观察者正视冠状面"""
        vu = orientation_v["RAS"]["vu"][2]
        vr = orientation_v["RAS"]["vr"][2]
    ...
    def snapshot_PNG(self, file=None) -> None:
        ...
        """
        if file is None:
            file = vmat.askSaveFile(self.tr('快照 (Snapshot)'), '*.png')
        if file is not None:
