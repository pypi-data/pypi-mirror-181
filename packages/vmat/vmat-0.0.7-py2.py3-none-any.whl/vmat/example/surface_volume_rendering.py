...
def view_surface(view, image, iso_value):
    pd = vmat.imIsosurface(image, iso_value)  # 提取CT值为iso_value的等值面，面绘制三维重建
    actor = vmat.PolyActor(view)  # 面绘制显示
...
def view_volume(view, image, iso_value):
    volume = vmat.ImageVolume(view)  # 体绘制显示
...
# 输入一个CT值用于等值面提取
iso_value = vmat.askInt(-1000, 200, 3000, None, 'HU')
if iso_value is None:
    vmat.appexit()

surface_view = vmat.View()  # 面绘制视图
view_surface(surface_view, image, iso_value)

volume_view = vmat.View()  # 体绘制视图
