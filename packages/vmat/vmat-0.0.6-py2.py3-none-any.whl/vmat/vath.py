...
class CS4x4:
    ...
    def copy(self) -> vmat.CS4x4:
        """
        复制实例

        :return: vmat.CS4x4 - 具有相同矩阵的实例
        """
        cs = CS4x4()
    ...
    def mpt(self, pt: Union[np.ndarray, List[float]], origin: Union[np.ndarray, List[float]] = None,
            other: Optional[vmat.CS4x4] = None) -> np.ndarray:
        """
    ...
    def mvt(self, vt: Union[np.ndarray, List[float]], other: Optional[vmat.CS4x4] = None) -> np.ndarray:
        """
    ...
    def translate(self, vt: Union[np.ndarray, List[float]]) -> vmat.CS4x4:
        """
        串接平移变换
        :param vt: 3x1平移变换
        :return: 新坐标系
    ...
    def rotate(self, angle: float, axis: Union[np.ndarray, List[float]],
               origin: Union[np.ndarray, List[float]] = None) -> vmat.CS4x4:
        """
        串接旋转变换
    ...
    def reflect(self, normal: np.ndarray, origin: Union[np.ndarray, List[float]] = None) -> vmat.CS4x4:
        """
        串接反射变换
    ...
    def scale(self, factor: float, origin: Union[np.ndarray, List[float]] = None) -> vmat.CS4x4:
        """
        串接缩放变换
...
def mpt_CS4x4(pt: Union[np.ndarray, List[float]], cs: vmat.CS4x4):
    """
    坐标变换：点pt由世界坐标系变换到新坐标系

    :param pt: 3x1点坐标
    :param cs: 坐标系
    :return: 变换后点坐标
    """
    p_cs = np.append(np.array(pt[:3]), 1)
    p_cs = p_cs @ cs.ary4x4inv()
    p_cs = p_cs[:3]
...
def CS4x4_InterLandmarks(pts0: List[np.ndarray], pts1: List[np.ndarray]) -> vmat.CS4x4:
    """
...
def coMidPt(p0: Union[np.ndarray, List[float]], p1: Union[np.ndarray, List[float]]):
    """
    计算两个点中点
    ...
    :return: 中点坐标
    """
    pt = np.array([0.5 * (p0[0] + p1[0]), 0.5 * (p0[1] + p1[1]), 0.5 * (p0[2] + p1[2])])
    return pt


def coDisToPt(p0: Union[np.ndarray, List[float]], p1: Union[np.ndarray, List[float]]):
    """
    计算两个点距离
    ...
    :return: 距离
    """
    dis = math.sqrt(math.pow(p0[0] - p1[0], 2) +
