...
def pdOpenFile_STL(file=None) -> Optional[vtk.vtkPolyData]:
    ...
    if file is None:
        file = vmat.askOpenFile('*.stl', '导入 (Import) STL')
        if file is None:
            return
...
def pdSaveFile_STL(pd: vtk.vtkPolyData, file=None) -> None:
    """保存vtkPolyData到STL文件"""
    if file is None:
        file = vmat.askSaveFile('stl', '*.stl', '导出 (Export) STL')
        if file is None:
            return
    with tempfile.TemporaryDirectory() as p:
        p = pathlib.Path(p) / '.stl'
...
def pdSaveFile_XML(pd: vtk.vtkPolyData, file=None) -> None:
    """保存vtkPolyData到vtp文件"""
    if file is None:
        file = vmat.askSaveFile('vtp', '*.vtp', '导出 (Export) XML')
        if file is None:
            return
...
def pdOpenFile_XML(file=None) -> Optional[vtk.vtkPolyData]:
    ...
    if file is None:
        file = vmat.askOpenFile('*.vtp', '导入 (Import) XML')
        if file is None:
            return
    with tempfile.TemporaryDirectory() as p:
        p = pathlib.Path(p) / '.vtp'
...
def pdDashLine(pt1: Union[np.ndarray, List[float]], pt2: Union[np.ndarray, List[float]]):
    """
    构造一条虚线
    ...
    distance = 2
    normalize = pt2 - pt1
    if np.linalg.norm(normalize):
        normalize = normalize / np.linalg.norm(normalize)
        num = int(np.floor(np.linalg.norm(pt2 - pt1) / distance))
        points = vtk.vtkPoints()
        points.Initialize()
        cells = vtk.vtkCellArray()
        cells.Initialize()
        for i in range(num + 1):
            pt = pt1 + i * distance * normalize
            points.InsertNextPoint(pt)
            if (i + 1) < (num + 1) and i % 2 == 0:
...
def pdCube(corner: List[Union[np.ndarray, List[float]]]):
    """
    构造立方体

    :param corner: 立方体的三个顶点
    :return: vtkPolyData，构造失败则返回None
    """
    x, y, z = abs(corner[0][0] - corner[1][0]), abs(corner[0][1] - corner[1][1]), abs(corner[0][2] - corner[1][2])
    c = vmat.coMidPt(corner[0], corner[1])
...
def pdApproximate_Pd(pd: vtk.vtkPolyData, match_vt: Union[np.ndarray, List[float]],
    ...
    center = vmat.pdCenter(pd)

    for i in range(pd.GetNumberOfPoints()):
        pt = np.array(pd.GetPoint(i))
        vt_center = vmat.vtOnPlane(center - pt, match_vt)

        cs = vmat.CS4x4(axis0=vt_center, axis2=match_vt, origin=pt)
        cs.orthogonalize([1])
        cs.normalize()
