...
def askOpenFile(nameFilter: str = '*',
    ...
    d = QFileDialog(None, title, str(s.value('Last' + title, defaultValue=vmat.desktopPath)))
...
def askSeries(series: List[vmat.SeriesData],
              title: str = tr('请选择目标系列 (Please select the target series)')) -> Optional[vmat.SeriesData]:
    """
    选择DICOM系列

    :param series: 候选的DICOM系列列表
    :param title: 对话框标题
    :return: vmat.SeriesData - 有效选择的DICOM系列，None - 没有有效选择
    """
