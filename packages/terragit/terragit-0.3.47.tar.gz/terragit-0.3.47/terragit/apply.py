...
class apply_changes:
    ...
    def apply(self,path,logsFolder,printLogs):
        if  path in pathList:
            ...
            popen = subprocess.Popen(state_list, stdout = subprocess.PIPE, stderr=subprocess.PIPE,shell = True, encoding = 'utf8')
            lines = popen.stdout.readlines()
            print("stdouuuuuuut" ,lines)
            errors = popen.stderr.readlines()
            print("stdeeeeeeeerrrrr" ,errors)
            popen.stdout.close()
            popen.stderr.close()
            print(bcolors.OKBLUE+path+":" +bcolors.ENDC)
            if errors != []:
                print("fllll eeerrors")
                for e in errors:
                    print(bcolors.FAIL + e)
            else:
                print("flll stdouuuuuuuur")
                for line in lines:
                    if(printLogs):
                        print(line)
