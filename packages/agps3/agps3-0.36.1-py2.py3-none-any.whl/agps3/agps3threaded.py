...
try:  # This kludge to get around imports with files and directories the same name.
    import agps3.agps3 as agps3  # Python 3
except ImportError:
    from .agps3 import agps3  # Python 2
...
class AGPS3mechanism(object):
    ...
    def unpack_data(self, usnap=0.2):  # 2/10th second sleep between empty requests
    ...
    def run_thread(self, usnap=0.2, daemon=True):
        """run thread with data"""
        # self.stream_data() # Unless other changes are made this would limit to localhost only.
        try:
            gps3_data_thread = Thread(
                target=self.unpack_data, args={usnap: usnap}, daemon=daemon
            )
        except TypeError:
    ...
    def stop(self):
        ...
        print(
            "Good bye."
        )  # You haven't gone anywhere, re-start it all with 'self.stream_data()'


if __name__ == "__main__":
    from agps3.misc import add_args

    args = add_args()

    agps3_thread = AGPS3mechanism()  # The thread triumvirate
    agps3_thread.stream_data(
        host=args.host, port=args.port, gpsd_protocol=args.gpsd_protocol
    )
    agps3_thread.run_thread(
        usnap=0.2
    )  # Throttle sleep between empty lookups in seconds defaults = 0.2 of a second.

    seconds_nap = int(
        args.seconds_nap
    )  # Threaded Demo loop 'seconds_nap' is not the same as 'usnap'
    while True:
        for nod in range(0, seconds_nap):
            print(
                "{:.0%} wait period of {} seconds".format(
                    nod / seconds_nap, seconds_nap
                ),
                end="\r",
            )
            sleep(1)
        ...
        print(
            "\nGPS3 Thread still functioning at {}".format(
                agps3_thread.data_stream.time
            )
        )
        print(
            "Lat:{}  Lon:{}  Speed:{}  Course:{}\n".format(
                agps3_thread.data_stream.lat,
