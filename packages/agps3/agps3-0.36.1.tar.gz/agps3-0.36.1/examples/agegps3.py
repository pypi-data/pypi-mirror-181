...
from agps3.agps3 import (
    agps3,
)  # Moe, remember to CHANGE to straight 'import agps3' if not installed,
...
link_data = (
    ...
    gps3data_file
)  # inserts 'the gps3data file' into a refresh mode default 4 second
f = open(link_file, "w")
f.write(link_data)
f.close()
    ...
    for new_data in gpsd_socket:
        if new_data:
            ...
            if data_stream.lat != "n/a":
                ...
                if data_stream.track == "n/a":
                    heading = (
                        data_stream.track
                    )  # 'track' frequently is missing and returns as 'n/a'
                else:
                    heading = round(
                        data_stream.track
                    )  # and heading precision in hundreths is just clutter.

                gps3data_history.append(longitude)
                gps3data_history.append(latitude)
                gps3data_history.append(altitude)
                hist_string = str(gps3data_history).replace(
                    " ", ""
                )  # GE > 7.1.xxxx spits up on spaces in <coordinates>
        ...
        time.sleep(
            0.7
        )  # default GE refresh rate is 4 seconds, therefore no refresh older than ~1 second from itself.
