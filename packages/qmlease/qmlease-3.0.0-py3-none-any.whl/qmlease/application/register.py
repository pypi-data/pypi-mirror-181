...
class Register:
    ...
    def register(
        ...
        if isinstance(qobj, QObject):
            name = name or self._pascal_2_snake_case(qobj.__class__.__name__)
            ...
            if namespace == '':
                ...
                if namespace not in self._namespace:
                    self._namespace.insert(namespace, Namespace(None))
    ...
    def _pascal_2_snake_case(self, name: str) -> str:
        ...
        """
        name = self._pattern_1.sub(r'\1_\2', name)
        return self._pattern_2.sub(r'\1_\2', name).lower()
