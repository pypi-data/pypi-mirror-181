...
def load_json_data(path, headers=None):
    """
    Return a tuple that contain : json load to str (index 0) and name of geojson (index 1)

    :param path: json path
    :param headers: for http request we can use optionally headers
...
def geojson_to_geolayer(
    ...
    data_geojson_in_dict, name = load_json_data(path=path, headers=headers)
