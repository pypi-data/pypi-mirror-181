    ...
    def optimize(
        ...
        for regressor in self.regressorCollection.selectedRegressors:
            for sampleIteration in range(downsampleCount):
                ...
                lowestClassCount = np.min(np.bincount(trainingLabels))
                balancedLabelMask = np.hstack(
                    [
                        sampler.choice(
                            np.where(trainingLabels == label)[0],
                            lowestClassCount,
                            replace=False,
                        )
                ...
                balancedData = trainingData[balancedLabelMask]
                balancedLabels = trainingLabels[balancedLabelMask]
                loadedObjective = partial(
                    objectiveToMinimize,
                    **{
                        "model": model,
                        "trainingData": balancedData,
                        "trainingLabels": balancedLabels,
                    },
                )
