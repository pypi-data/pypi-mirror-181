...
class TCPHandler(BaseRequestHandler):
    ...
    def _subscribe(self, data: SocketReader) -> bytes:
        ...
        key = data.read(key_len)
        if len(key) != key_len: raise ProtocolError("length")
        self.server.subscribe(key, self._cache_update)
    ...
    def _unsubscribe(self, data: SocketReader) -> bytes:
        ...
        key = data.read(key_len)
        if len(key) != key_len: raise ProtocolError("length")
        self.server.unsubscribe(key, self._cache_update)
...
class TCPServer(ThreadingTCPServer):
    ...
    def __init__(self, bind: Tuple[str, int], auto_kill: bool) -> None:
        ...
        self._subs = {} # bytes -> SubscriptionList
...
class Client:
    ...
    def subscribe(self, key: bytes, callback: Callable[[bytes], None]) -> SubscriptionClient:
        """
        ...
        """
        return SubscriptionClient(*self._socket.getpeername()[0:2], key, callback)
