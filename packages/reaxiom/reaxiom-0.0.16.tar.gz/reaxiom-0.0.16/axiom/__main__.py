...
def main(sys_argv):
    ...
    if sys_argv[1] == 'huggingface': # settup the huggingface token and username
        ...
        axiom_directory_server = input('axiom directory server:')
...
type: data.int'''
    ...
    if sys_argv[1] == 'ls.cache':
        if len(sys_argv) == 3 and sys_argv[2] == 'table':
            axiom.common.ls_cache(table=True)
        else:
            axiom.common.ls_cache(table=False)
    ...
    if sys_argv[1] == 'cp': # cp huggingface test local simple given_name
        ...
        axiom.common.cp(from_server, from_stage, to_server, to_stage, given_name)
