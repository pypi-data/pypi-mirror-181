...
def put_axiom_directory(info_yaml):
    stage = get_stage()
...
def put_node_huggingface(compressed_obj_file, info_yaml, sha256, given_name): # node means obj + info
    ...
    put_axiom_directory(info_yaml)

def get_axiom_directory(given_name):
    stage = get_stage()
    axiom_directory_server = open(os.path.join(config['home'], '.huggingface', 'axiom_directory_server')).read()
    key = open(os.path.join(config['home'], '.huggingface', 'key')).read()
...
def cp(from_server, from_stage, to_server, to_stage, sha256):
    pass