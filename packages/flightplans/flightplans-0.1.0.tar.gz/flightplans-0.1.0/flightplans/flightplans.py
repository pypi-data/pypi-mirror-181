...
# github      : https://github.com/erdogant/flightplans
...
# formatter = logging.Formatter('[%(asctime)s] [flightplans]> %(levelname)s> %(message)s', datefmt='%H:%M:%S')
formatter = logging.Formatter('[flightplans] >%(levelname)s> %(message)s')
...
class flightplans():
    ...
    def __init__(self, method='xgboost', verbose=20):
        """Initialize flightplans with user-defined parameters."""
        self.method=method
        # Set the logger
...
if __name__ == "__main__":
    ...
    df = flightplans.import_example()
    out = flightplans.fit(df)
    fig,ax = flightplans.plot(out)
