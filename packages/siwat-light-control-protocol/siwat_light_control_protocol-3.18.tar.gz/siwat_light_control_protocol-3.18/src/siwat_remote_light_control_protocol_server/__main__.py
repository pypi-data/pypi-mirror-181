...
MAXIMUM_BRIGHTNESS: int = config['MAXIMUM_BRIGHTNESS']
...
def report_state():
    ...
    mqttclient.publish(MQTT_BASE_TOPIC+"/report/color",
                       str(int(r))+","+str(int(g))+","+str(int(b)))
    mqttclient.publish(MQTT_BASE_TOPIC+"/report/effect",
                       led_effects.effects[effect]['name'])
    mqttclient.publish(MQTT_BASE_TOPIC+"/report/effectlist",
                       json.dumps(effects_list))
        ...
        if topic == MQTT_BASE_TOPIC+"/control/state":
                ...
                if state != 1:
                    ...
                    if effect == 0:
                        led.fill_led_with_color(
                            r*brightness/255.0*MAXIMUM_BRIGHTNESS/255.0, g*brightness/255.0*MAXIMUM_BRIGHTNESS/255.0, b*brightness/255.0*MAXIMUM_BRIGHTNESS/255.0)
                    elif effect >= 2:
            ...
            if effect != 1:
                try:
                    ...
                    return
            mqttclient.publish(
                MQTT_BASE_TOPIC+"/report/brightness", int(brightness))
        elif topic == MQTT_BASE_TOPIC+"/control/color":
            if effect != 1:
                try:
                    [rtmp, gtmp, btmp] = payload.split(',')
                    [r, g, b] = validate_rgb(
                        int(float(rtmp)), int(float(gtmp)), int(float(btmp)))
                    if effect >= 2:
                        effector.r = r
                        effector.g = g
                        effector.b = b
            ...
            if effect == 1:
                ...
                for data in datas:
                    led.set_led_at(data[0], data[1][0]*MAXIMUM_BRIGHTNESS/255.0, data[1]
                                   [1]*MAXIMUM_BRIGHTNESS/255.0, data[1][2]*MAXIMUM_BRIGHTNESS/255.0)
                led.show()
