...
class EDJVar (Var):
  ...
  # {{{
    ...
    #axes = list(var.axes)
    #axes[daxis] = axes[daxis].slice[:4]
    self.iview = View(iaxes)
    self.oview = View(oaxes)
  ...
  # {{{
    ...
    left = iview.integer_indices[0]

    ## All the points we need to request (unique occurrences only)
    #allpoints = reduce(np.union1d, [left+i for i in range(n)])
    ...
    allvalues = iview.get(self.var, pbar=pbar)
