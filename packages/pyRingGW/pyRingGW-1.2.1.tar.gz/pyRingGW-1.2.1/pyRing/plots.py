...
def plot_strain(get_waveform, dets, fixed_params, tgps, params = None, whiten_flag = False, whiten_method = 'TD', strain_only = 0, **kwargs):
    ...
    for i,d in enumerate(dets.keys()):
        ...
        if whiten_flag:
            ...
            if (np.max(np.abs(timeseries)) < 5.): ax.set_ylim([-5,5])
...
def global_corner(x, names, output):
...
def f0_t0_gr(samples_x, Mf_s, af_s, Num_mode, probs_flag=0, scatter_samples_flag=0, delta_QNM_plots=0, predictions_contour_flag=0, **kwargs):
    ...
    for n in n_values:
        ...
        for l in l_values:
            for m in range(-l,l+1,1):
                ...
                if(predictions_contour_flag): plot_contour(samples, level=[0.9], linest = 'solid', color=col, line_w=1.2, plot_legend=0)

                # Fine tune text positioning.
                shift_f   = 10
                shift_tau = 0.3
                if(l==2 and m==-2         ): shift_tau += -0.1
                if(l==3 and m==-2         ): shift_tau +=  0.1
                if(l==3 and m==-3         ): shift_tau +=  0.2
                if(l==2 and m==-2 and n==1): shift_tau += -0.1
                if(l==2 and m==-2 and n==1): shift_f   +=  5.0
                if(l==2 and m==-1 and n==1): shift_f   +=  3.0
                if(l==2 and m==-1 and n==1): shift_tau += -0.1
                if(m<0 and n==0           ): shift_f   +=  4.0

                # Invert label positioning between l=2 and l=3.
                if(l==2): shift_tau = -shift_tau
                font_text = {'color':  'black', 'weight': 'normal', 'size': 5}
                plt.plot(np.median(f0),         np.median(t0), color=col, marker='*', markersize=7,                    zorder=2)
                plt.text(np.median(f0)-shift_f, np.median(t0)-shift_tau, r'$(%d,%d,%d)$ '%(l,m,n), fontdict=font_text, zorder=3)
    ...
    if(probs_flag):
        ...
        patches = [mpatches.Patch(color=c, label = '$(\ell,m,n) = $'+labels[i]) for i,c in enumerate(colors_manual)]
