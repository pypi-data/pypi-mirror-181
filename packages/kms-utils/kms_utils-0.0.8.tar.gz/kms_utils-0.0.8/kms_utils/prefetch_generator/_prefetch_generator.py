...
import time
from typing import Any, Callable, Dict, Iterable, Optional, Generator
...
class PrefetchGenerator(Thread):
    ...
    """
    def __init__(self, iterable: Iterable, num_prefetch: int = 1, num_workers: int = 1, ordered: bool = True, shuffle: bool = False, processing_func: Optional[Callable[[Any], Any]] = None, func_kwargs: Optional[Dict[str, Any]] = {}) -> None:
    ...
    def make_units(self) -> Dict[int, PrefetchGeneratorUnit]:
        ...
        def assign(iterable, units):
            if isinstance(iterable, Generator):
                ...
                for gid, unit in units.items():
                    for item in iterable[gid::self.num_workers]:
                        unit.put(item)
        ...
        assign_thread = Thread(target=assign, args=(self.iterable, units))
