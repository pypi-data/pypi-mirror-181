...
class print_dbg():
    ...
    def __new__(cls, text: str, module: str, dbg_mode: bool = None):
        cls.print_dbg(cls, text, module, dbg_mode)
        # super(print_dbg, cls).__new__(cls, *args, **kwargs)

    def __init__(self):
        super(print_dbg, self).__init__()

    def __repr__(self):
        super(print_dbg, self).__repr__()

    def __str__(self):
        super(print_dbg, self).__str__()

    def print_dbg(cls, text: str, module: str, dbg_mode: bool = None):
        dbg_mode = dbg_mode if dbg_mode is not None else cls.dbg_mode
        if dbg_mode:
            print('dbg => ' + str(text)) if dbg.get_module(module) else None
        else:
            print(text) if dbg.get_module(module) else None
