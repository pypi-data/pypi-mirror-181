...
class HiveRepo(BaseRepo):
    ...
    def try_write_append(self, df, options: Optional[Option] = []):
        return self.write_append(df, options)

    def write_append(self, df, options: Optional[Option] = []):
        """
    ...
    @monad.monadic_try(error_cls=error.RepoWriteError)
    def try_upsert(self, df, options: Optional[Option] = []):
        """
    ...
    def upsert(self, df, options: Optional[Option] = []):
        """
    ...
    def _perform_upsert(self, df, _batch_id=None, options: Optional[Option] = []):
    ...
    #
    @monad.monadic_try(error_cls=error.RepoWriteError)
    def try_write_stream(self, stream, trigger: dict = None, options: Optional[Option] = []):
        return self.write_stream_append(stream, trigger, options)

    def write_stream_append(self, stream, trigger: dict = None, options: Optional[Option] = []):
        """
    ...
    def stream_write_via_delta_upsert(self,
        ...
        if self.is_delta_table_from_path().is_left():
            ...
            self.stream_query = self._stream_write_upsert(stream, self.table_name, trigger_condition, options)
    ...
    def _stream_write_upsert(self,
        ...
        return (stream.writeStream
                ...
                .options(**opts)
                .trigger(**trigger_condition)
                .foreachBatch(partial(self._perform_upsert, options=options))
                .outputMode('append')
                .start())
