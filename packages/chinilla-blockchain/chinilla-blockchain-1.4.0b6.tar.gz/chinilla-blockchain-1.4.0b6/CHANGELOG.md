...
## 1.6.1 Chinilla blockchain 2022-11-03
...
## 1.6.0 Chinilla blockchain 2022-9-20
...
## 1.5.1 Chinilla blockchain 2022-8-23
...
## 1.5.0 Chinilla blockchain 2022-7-26
...
## 1.4.0 Chinilla blockchain 2022-6-29
...
## 1.3.5 Chinilla blockchain 2022-5-11
...
## 1.3.4 Chinilla blockchain 2022-4-19
...
## 1.3.3 Chinilla blockchain 2022-4-02
...
## 1.3.2 Chinilla blockchain 2022-4-01
...
- Added new RPC, get_version, to the daemon to return the version of Chinilla (Thanks @dkackman).
- Added new config.yaml setting, reserved_cores, to specify how many cores Chinilla will not use when launching process pools. Using 0 will allow Chinilla to use all cores for process pools. Set the default to 0 to allow Chinilla to use all cores. This can result in faster syncing and better performance overall especially on lower-end CPUs like the Raspberry Pi4.
- Added new RPC, get_logged_in_fingerprint, to the wallet to return the currently logged in fingerprint.
- Added new CLI option, chinilla keys derive, to allow deriving any number of keys in various ways. This is particularly useful to do an exhaustive search for a given address using chinilla keys derive search.
...
- VDF, chinillapos, and blspy workflows updated to support python 3.10 wheels.
...
- Thanks to @cross for an optimization to chinillapos to use rename instead of copy if the tmp2 and final files are on the same filesystem.
- Updated to use chinillapos 1.0.9.
...
- Fixed issues with displaying IPv6 address in CLI with chinilla show -c.
- Thanks to @chuwt for fix to looping logic during node synching.
- Fixed the chinilla-blockchain RPM to set the permission of chrome-sandbox properly.
...
- Thanks @jcteng for fixing a bug on the Chinilla DID wallet that showed 'vojo:'' instead of 'vojo'.
...
- Omitted from the 1.0.2 changelog, we fixed one crash in harvester with the release of chinillapos 1.0.0 as well.

## 1.0.2 Chinilla Blockchain 2021-03-30

### Added

- We have released version 1.0.0 of [chinillapos](https://github.com/Chinilla/chinillapos). This includes a 20% speed increase for bitfield plotting compared to the previous version on the same machine. In many cases this will mean that bitfield plotting is as fast or faster than non bitfield plotting.
...
- This is a hard fork/breaking change from RC6/7. THCX Coins will **not** be moved forward but your plots and keys and parts of your configuration do. When you install this version before 10AM PDST on 3/16/2021 it will load up, start finding peers, and otherwise wait for the flag drop at that time to start farming. This is likely to be the last dress rehearsal for vanillanet launch. Our [3/15/2021 blog post](https://www.chinilla.com/2021/03/15/vanillanet-update.html) has more details on the current vanillanet launch plan.
...
- RC3 is a new chain to support the last major chinillalisp changes. THCX from the RC1/2 chain do not come forward to this chain but plots and keys continue to work as usual.
...
- 'chinilla wallet send' now takes in THCX or HCX as units instead of vojos.
...
- We updated chinillapos to hopefully address some harvester crashes when moving plot files.
...
- There was a potential bug in Python 3.9.0 that required us to update blspy, chinillapos, chinillavdf, and chinillabip158.
...
- The user interface now refers to chinilla coins with their correct currency code of HCX.
...
- Building the sub reposities (chinillapos, chinillavdf, blslibrary) now requires CMake 3.14+.

### Fixed

- There was a regression in Chinilla Proof of Space ([chinillapos](https://github.com/Chinilla/chinillapos)) that came from our efforts to speed up plotting on Windows native. Now k>=32 plots work correctly. We made additional bug fixes and corrected limiting small k size generation.
...
- The libraries chinilla-pos, chinilla-fast-vdf, and chinilla-bip-158 have been moved to their own repositories: [chinillapos](https://github.com/Chinilla/chinillapos), [chinillavdf](https://github.com/Chinilla/chinillavdf), and [chaibip158](https://github.com/Chinilla/chinillabip158). They are brought in by chinilla-blockchain at install time. Our BLS signature library remains at [bls-signatures](https://github.com/Chinilla/bls-signatures).
- The install process now brings in chinillapos, chinillavdf, etc from Pypi where they are auto published via GitHub Actions ci using cibuildwheel. Check out `.github/workflows/build.yml` for build methods in each of the sub repositories.
...
- `install-timelord.sh` has been pulled out from `install.sh`. This script downloads the source python package for chinillavdf and compiles it locally for timelords. vdf_client can be included or excluded to make building normal user wheels easier.
...
- New AJAX based full node UI. To access go to [http://127.0.0.1:43555/index.html](http://127.0.0.1:43555/index.html) with any modern web browser on the same machine as the full node.
- If you want to benchmark your CPU as a VDF you can use vdf_bench square_asm 500000 for the assembly optimized test or just vdf_bench square 500000 for the plain C++ code path. This tool is found in lib/chinillavdf/fast_vdf/.
...
- Implemented an RPC interface with JSON serialization for streamables - currently on port 43555.
