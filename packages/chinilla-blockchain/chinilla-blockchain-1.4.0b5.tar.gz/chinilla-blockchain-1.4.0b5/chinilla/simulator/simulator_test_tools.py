...
from chinilla.util.errors import KeychainFingerprintExists
...
async def get_full_chinilla_simulator(
    ...
    """
    A chinilla root Path is required.
    The chinilla root Path can be a temporary directory (tempfile.TemporaryDirectory)
    ...
    with Lockfile.create(daemon_launch_lock_path(chinilla_root)):
        ...
        async for simulator in start_simulator(chinilla_root, automated_testing):
            yield simulator, chinilla_root, config, mnemonic, fingerprint, keychain
