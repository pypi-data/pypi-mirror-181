...
class _BaseClient(object):
    ...
    def _jsonrpc_retry_call(self, *args, **kwargs):
		...
		# Gent 遇到NullPointerExceptionError错误也重启uiautomator，而不是只显示错误信息
		# 这是因为Android 12中有时候会出现空指针异常，然后无法继续重新连接
...
class _AppMixIn:
    ...
    def app_current(self):
        ...
        package = None
        output, _ = self.shell(['dumpsys', '-t', '60', 'activity', 'activities'])
        ...
        output, _ = self.shell(['dumpsys', '-t', '60', 'activity', 'top'])
