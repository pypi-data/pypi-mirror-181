...
class FilingBase():
    def __init__(self, cik):
        ...
        self.latest_cover_page = None
        self.latest_portfolio_value = None
        self.latest_count_holdings = None
    ...
    def _parse_13f_url(self, url):
        ...
        self.latest_count_holdings = count_holdings
    ...
    def get_latest_filing(self, simplified=True):
        ...
        if (len(self.latest_holdings_table)>0) & (len(self.latest_simplified_holdings_table)>0):
            print("forgoing re-run")
            if simplified==True:
                return self.latest_simplified_holdings_table
            else:
                return self.latest_holdings_table

        self.latest_cover_page, self.latest_holdings_table, self.latest_simplified_holdings_table = self._parse_13f_url(latest_url_date[0])
        if simplified==True: 
            return self.latest_simplified_holdings_table
        else: 
            return self.latest_holdings_table
    
    def get_latest_filing_cover_page(self):
        """Returns the latest 13F-HR filing cover page."""
        if self.latest_cover_page != None:
            print("foregoing re-run")
            return self.latest_cover_page
        else:
            self.get_latest_filing()
