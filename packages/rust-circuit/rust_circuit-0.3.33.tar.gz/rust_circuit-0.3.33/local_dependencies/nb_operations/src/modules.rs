...
use circuit_base::{
    ...
    module::{are_args_used, is_intersecting_free_syms},
    CircuitNode, CircuitRc, Module, ModuleArgSpec, ModuleSpec, Rearrange, Symbol,
};
use circuit_rewrites::{
    deep_rewrite::SimpFnSubset,
    module_rewrite::{elim_no_input_module, extract_rewrite_raw, module_remove_unused_inputs},
};
...
#[pyfunction(bind_name = "\"upd_bind\".to_owned()", short_if_not_leaf = "true")]
pub fn default_update_bindings_nested_namer(
    ...
    NestedModuleNamer::Dyn(NestedModuleNamerDynStruct(Arc::new(
        move |base_circuit, running_circuit, modules, pushed_overall_mod_count| {
            if pushed_overall_mod_count.is_none() && short_if_not_leaf {
                Ok(base_circuit
                    .name()
...
#[pyfunction(
    namer = "default_update_bindings_nested_namer(\"upd_bind\".to_owned(), true)",
