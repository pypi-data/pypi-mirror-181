...
pub fn intersect_free_syms(circuit: &CircuitRc, syms: &[&Symbol]) -> bool {
    if let Some(other) = circuit.as_symbol() {
        syms.contains(&other)
    } else {
        syms.iter()
...
pub fn recur_check_no_free_bound_by_nested(
    ...
    if seen.insert(circ.info().hash) {
        if let Some(m) = circ.as_module() {
            ...
            if !intersect_free_syms(&circ, syms) {
                return Ok(());
            }
            ...
            if intersect_free_syms(&m.spec.circuit, &syms) {
