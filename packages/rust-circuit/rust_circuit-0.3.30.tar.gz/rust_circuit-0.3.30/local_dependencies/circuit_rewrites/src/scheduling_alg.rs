...
use crate::{
    scheduled_execution::{PySchedulingOOMManyError, SchedulingOOMError},
...
impl Dag {
    ...
    pub fn schedule_isolated(&mut self, settings: &DagSimpSettings) -> Result<bool> {
        ...
        for iso in &isolated {
            ...
            {
                let resy = timed!(
                    direct_only_sub.schedule_exhaustive(required_first, settings),
                    10,
                    settings.verbose >= 2
                );
    ...
    pub fn schedule_exhaustive(
        ...
        let mut best: (Vec<(u32, usize)>, usize) = (vec![], settings.acceptable_subdag_max + 1);
        ...
        macro_rules! choose {
            ($i:expr) => {{
                ...
                let endel1 = options_stack.len() - 1;
                options_stack[endel1].sort_by_cached_key(|&x| {
                    self.parents[x as usize]
                        .iter()
                        .map(|&parent| children_left[parent as usize])
                        .max()
                        .unwrap_or(0)
                })
            }};
        ...
        for numiters in 0usize.. {
            ...
            if order.len() == self.node_costs.len() {
                ...
                if start_instant.elapsed().as_nanos() > settings.exhaustive_settle_ns as u128 {
                    break;
                }
                backtrack!();
            } else {
            ...
            if numiters.trailing_zeros() > 8
                && start_instant.elapsed().as_nanos() > settings.exhaustive_give_up_ns as u128
            {
                bail!(SchedulingOOMError::Simp {
                    cyto: self.repr_cytoscape_link(),
                    origin: "exhaustive timeout".to_owned()
                })
            }
        }
        Ok(best.0)
