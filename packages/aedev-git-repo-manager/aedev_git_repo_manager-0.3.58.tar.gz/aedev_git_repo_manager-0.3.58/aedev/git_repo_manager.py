...
def _check_resources_i18n_po(file_name: str, content: str):
    ...
    mo_nam = os.path.splitext(file_name)[0] + ".mo"
    _chk_if(69, os.path.isfile(mo_nam), f"missing compiled message file {mo_nam}")
    _chk_if(69, native or os.path.getmtime(file_name) < os.path.getmtime(mo_nam), f"{file_name} not compiled into .mo")
...
def _check_types_linting_tests(pdv: PdvType):
    ...
    project_path = pdv_str(pdv, 'project_path')
    project_packages = pdv_val(pdv, 'project_packages') or ['main']
    project_type = pdv_str(pdv, 'project_type')
    excludes = ["migrations"] if project_type == DJANGO_PRJ else []
    root_packages = [package for package in project_packages if '.' not in package]

    cae.vpo(f"    - found project packages: {_pp(root_packages)}")
    ...
    cae.vpo(f"    - compiled command line options and arguments: {_pp(args)}")
    ...
    with _in_prj_dir_venv(project_path):
        ...
        _cl(63, "python -m pytest --cov-report html"
                f"{''.join([' --cov=' + package for package in root_packages])}"
