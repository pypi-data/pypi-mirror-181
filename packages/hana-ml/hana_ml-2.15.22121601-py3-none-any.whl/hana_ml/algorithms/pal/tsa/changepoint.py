...
class BCPD(PALBase):
    ...
    def fit_predict(self, data, key=None, endog=None, features=None):
        ...
        if not self._disable_hana_execution:
            decompose = conn.table(decompose_tbl).columns
            # ID is timestamp
            if not self.is_index_int:
                if conn.table(tcp_tbl).shape[0] > 0:
                    tcp = conn.table(tcp_tbl).columns
                    tcp_sql =  """
                                SELECT {1},
                    ...
                    tcp_timestamp = conn.table(tcp_tbl)

                if conn.table(scp_tbl).shape[0] > 0:
                    scp = conn.table(scp_tbl).columns
                    scp_sql =  """
                                SELECT {1},
                                ADD_SECONDS('{0}', ({3}-1) * {2}) AS {3}
                                FROM {4}
                    ...
                    scp_timestamp = conn.table(scp_tbl)
