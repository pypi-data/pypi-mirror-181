...
class _PathInitializer(object):
    ...
    def get_paths_from_os(self):
        ...
        pysdk_py_runtime_dir = os.path.join(
            matlabroot, "toolbox", "compiler_sdk", "pysdk_py"
        )
        ...
        if not os.path.isdir(pysdk_py_runtime_dir):
            raise RuntimeError(
                "Could not find the directory {0}".format(pysdk_py_runtime_dir)
            )
        if not os.path.isdir(matlab_mod_dist_dir):
            raise RuntimeError(
                "Could not find the directory {0}".format(matlab_mod_dist_dir)
            )
