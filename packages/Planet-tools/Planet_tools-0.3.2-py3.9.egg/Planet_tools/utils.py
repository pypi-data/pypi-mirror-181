...
def pipe_data(name, fileid):
    ...
    for j in range(len(Us_n)):
        ...
        Us.append(usn)
    conta, smear = np.asarray(dta["U1"])[msk], np.asarray(dta["U2"])[msk]
    # Making an arrays of zero for other elements
    status, event, dark, conta_err, smerr, locx, locy =\
        np.zeros_like(fl), np.zeros_like(fl), np.zeros_like(fl), np.zeros_like(fl), np.zeros_like(fl),\
        np.zeros_like(fl)+np.median(xc), np.zeros_like(fl)+np.median(yc)
