...
def convert_value_to_invariant(cell : str):
    return cell.replace(',', '.')

def convert_to_invariant(data):
    return map(lambda row : map(convert_value_to_invariant,row), data)
    
def get_as_float(cell : str):
    return float(convert_value_to_invariant(cell))
...
class SimulationProfile:
    ...
    def __init__(self, entry):
        ...
        self.solver_steps = get_as_float(entry[1][1])
        self.total_time = get_as_float(entry[2][1])
        self.matrix_size = get_as_float(entry[3][1])
        self.plotted_points = get_as_float(entry[4][1])
        self.integration_method = entry[5][1]
        self.accuracy = get_as_float(entry[6][1])
        self.run_time = get_as_float(entry[7][1])
