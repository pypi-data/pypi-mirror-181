...
class QueryProvider():
    ...
    def createQuery(self,pagination):
        ...
        if self.order_attributes is not None and self.filter_attributes is not None and self.aggregation is None:
            ...
            query += " (SELECT * ,row_number() OVER (ORDER BY {colOrderBy}) AS rank FROM {tablename} where {filter}) x".format(colOrderBy = colValueOrderBy,tablename=self.delta_table_name,filter=self.filter_attributes)
            query += " WHERE x.rank > {skip}".format(skip=self.skip_count)
            query += " LIMIT {numrows}".format(numrows=self.numrows)
        elif self.order_attributes is not None and self.aggregation is None:
            query += " FROM "
            colValueOrderBy = ','.join([att + " " + dir for att, dir in self.order_attributes.items()])
            query += " (SELECT * ,row_number() OVER (ORDER BY {colOrderBy}) AS rank FROM {tablename}) x".format(colOrderBy = colValueOrderBy,tablename=self.delta_table_name)
            query += " WHERE x.rank > {skip}".format(skip=self.skip_count)
            query += " LIMIT {numrows}".format(numrows=self.numrows)        
        else: 
            if self.filter_attributes is not None:
                query += " FROM "
                if pagination == True:
                    query += " (SELECT * ,row_number() OVER (ORDER BY skypoint_delta_index) AS rank FROM {tablename} where {filter}) x".format(tablename=self.delta_table_name,filter=self.filter_attributes)
                else:
                    query += " (SELECT * ,row_number() OVER (ORDER BY (SELECT NULL)) AS rank FROM {tablename} where {filter}) x".format(tablename=self.delta_table_name,filter=self.filter_attributes)
