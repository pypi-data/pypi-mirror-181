...
class Image(Base):
    ...
    def is_valid(self, detector: str):
        valid_cropped_image = [cropped_image.face_detected for cropped_image in self.croppedImages if
                               cropped_image.detectors.name == detector]
        assert len(valid_cropped_image) == 1
        return valid_cropped_image[0]
    ...
    def make_cropped_image(self, detector):
        cropped_image = [cropped_image for cropped_image in self.croppedImages if
                         cropped_image.detectors.name == detector]
