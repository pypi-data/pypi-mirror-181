...
public:
  ...
  using ArrowTensorT =
      typename detail::ArrowTensorType<detail::arrow_string_view>::type;
...
class TensorBaseBuilder<std::string>: public ObjectBuilder {
  public:
    ...
    std::shared_ptr<Object> _Seal(Client &client, std::shared_ptr<Tensor<std::string>> &__value) {
        size_t __value_nbytes = 0;

        __value->meta_.SetTypeName(type_name<Tensor<std::string>>());
        if (std::is_base_of<GlobalObject, Tensor<std::string>>::value) {
            __value->meta_.SetGlobal(true);
        }
