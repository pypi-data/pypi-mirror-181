...
class DataSource(ABC):
    ...
    def get_by_idx_set(
        self, index_set: Collection[Union[int, np.integer]]
    ) -> np.ndarray:
...
class TensorDataSource(DataSource):
    ...
    def split(
        self, splitter: TrainValTestSpliter
    ) -> Tuple[TensorDataSource, TensorDataSource, Optional[TensorDataSource]]:
    ...
    def __getitem__(self, item: Union[int, slice, Collection[int]]) -> np.ndarray:
        if isinstance(
            item, (slice, int, np.integer, Collection[Union[np.integer, int]])
        ):
            return self.tensors[item]
        raise ValueError(
            f"{self.__class__}.__getitem__() Unsupported type: {type(item)}"
        )
    ...
    @validate_arguments(config=ArbitraryTypes)
    def select_features(
        self, features: Collection[int], name: Optional[str] = None
    ) -> TensorDataSource:
...
class TimeseriesDataSource(TensorDataSource):
    ...
    def __init__(
        ...
        assert (
            instances > 0
        ), f"Not enough data for window size {self.length} and sampling rate {sampling_rate}"
    ...
    def split(
        ...
        "TimeseriesDataSource", "TimeseriesDataSource", Optional["TimeseriesDataSource"]
    ]:
    ...
    def get_targets(self, target_name: str = "target") -> TensorDataSource:
        ...
        if self._target_params is None:
            raise ValueError(
                f"This instance {self.name} of {self.__class__.__name__} was confifured without Targets"
            )
        ...
        strided_na = non_strided_na[
            :: self.stride, 0, ::pred_stride, self._target_params.target_idx
        ]
        if self._target_params.reverse:
            strided_na = strided_na[:, ::-1]
        assert strided_na.shape == (self.size, pred_len)
        new_data_source = TensorDataSource(
            name=target_name, tensors=strided_na, encoders=self.encoders
        )
    ...
    def _get_window_by_instance_idx(self, instance_idx: int) -> Tuple[int, int]:
        ...
        w_sz = self.w_sz
        assert (
            start_idx + w_sz <= self.tensors.shape[0]
        ), f"Corrupted data in generator at index {instance_idx}"
...
class TargetTimeseriesDataSource(TimeseriesDataSource):
    @classmethod
    @validate_arguments(config=ArbitraryTypes)
    def from_timeseries_datasource(
        cls, ds: TimeseriesDataSource, name: Optional[str] = None
    ) -> "TargetTimeseriesDataSource":
        assert (
            ds._target_params is not None
        ), "Can't create target datasource without target parameters"
    ...
    def _get_target_window_by_instance_idx(self, instance_idx: int) -> Tuple[int, int]:
        ...
        assert (
            start_idx + pred_w_sz <= self.tensors.shape[0]
        ), f"Corrupted data in generator at index {instance_idx}"
    ...
    def select_features(
        ...
        raise NotImplementedError(
            "Can't implement select features on a target DataSource"
        )
...
class CompoundDataSource(DataSource):
    def __init__(self, name: str, data_sources: Sequence[DataSource]):
        ...
        for ds in data_sources:
            assert self.sz == len(
                ds
            ), f"All data sources must have the same length. {self.sz} != {len(ds)}"

    def split(
        self, splitter: TrainValTestSpliter
    ) -> Tuple["CompoundDataSource", "CompoundDataSource", "CompoundDataSource"]:
    ...
    def fit_encode(self, encoders: List[DataEncoder]) -> "CompoundDataSource":
        ...
        """
        cloned_encoders = [
            [enc.clone() for enc in encoders] for _ in range(len(self._data_sources))
        ]
        encoded_dss = [
            ds.fit_encode(cloned_encoders[i]) for i, ds in enumerate(self._data_sources)
        ]
    ...
    def unchain_tds(self) -> TensorDataSource:
        ...
        raise ValueError(
            f"Can't split a {self.__class__} with {first_ds.__class__} inside"
            f" as the encoders also will be split"
        )
...
class DataSet:
    ...
    def split_encode(
        ...
        train_inputs, val_inputs, test_inputs = self._split_encode_sources_map(
            self.input_sources, splitter, encoders
        )
        if self.target_sources:
            train_targets, val_targets, test_targets = self._split_encode_sources_map(
                self.target_sources, splitter, encoders
            )
        else:
    ...
    def get_encoders(self) -> Dict[str, List[DataEncoder]]:
        ...
        all_encoders = {
            name: source.get_encoders()
            for sources in both_sources
...
class XBatchGenerator(data_utils.Sequence):
    @validate_arguments(config=ArbitraryTypes)
    def __init__(
        self, inputs_map: Dict[str, DataSource], batch_size: PositiveInt = 128
    ):
...
class XYBatchGenerator(XBatchGenerator):
    ...
    def __add__(self, other: "XYBatchGenerator") -> "XYBatchGenerator":
        ...
        return self.__class__(
            inputs, targets, self.shuffle, self.batch_size, self.sampling_rate
        )
