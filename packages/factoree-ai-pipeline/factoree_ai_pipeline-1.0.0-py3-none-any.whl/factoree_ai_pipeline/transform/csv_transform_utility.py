    ...
    def standardize_file_data(self, file_data: Csv) -> (Csv, Csv):
        ...
        for i in range(self.timestamp_column + 1, len(file_data[0])):
            ...
            if sensor_type is None:
                ...
                grid = standardized_data

            for row in range(len(file_data)):
                if len(grid) <= row:
                    grid.append([self.standardize_value(sensor_type, file_data[row][self.timestamp_column])])

                grid[row].append(self.standardize_value(sensor_type, file_data[row][i]))
