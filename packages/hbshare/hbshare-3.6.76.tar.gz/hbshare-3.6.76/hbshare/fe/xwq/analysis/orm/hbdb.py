...
class HBDB:
    ...
    def read_fund_maxdrawdown_period_given_code_and_date(self, code, date):
        sql = "select jjdm, jzrq, zblb, zbnp, zdhchfts, zdhccxts from st_fund.t_st_gm_qjzdhc_zx where m_opt_type <> '03' and jjdm='{0}' and jzrq >= '{1}'".format(code, date)
        df = self.get_df(sql, db='funduser')
        return df

    def read_fund_maxdrawdown_period_given_codes_and_date(self, codes, date):
        codes = '"' + '","'.join(codes) + '"'
        sql = "select jjdm, jzrq, zblb, zbnp, zdhchfts, zdhccxts from st_fund.t_st_gm_qjzdhc_zx where m_opt_type <> '03' and jjdm in ({0}) and jzrq = '{1}'".format(codes, date)
        df = self.get_df(sql, db='funduser')
        return df
