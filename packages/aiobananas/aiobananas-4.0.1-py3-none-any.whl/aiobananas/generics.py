...
class BaseApiResponse(GenericModel, Generic[TModelOutputs]):
    ...
    def as_response(self, model: type[TOtherOutputs]) -> Response[TOtherOutputs]:
        if self.modelOutputs is None:
            raise ValueError("modelOutputs must not be None")
...
class StartApiResponse(BaseApiResponse[TModelOutputs], Generic[TModelOutputs]):
    ...
    def model_outputs_must_match_finished(cls, v, values):
        if v and (
            not values.__contains__("modelOutputs") or values["modelOutputs"] is None
        ):
...
class CheckApiResponse(BaseApiResponse[TModelOutputs], Generic[TModelOutputs]):
    ...
    def model_outputs_must_match_finished(cls, v, values):
        if values["message"] == "success" and v is None:
            raise ValueError('modelOutputs must not be None if message is "success"')
        elif not (values["message"] == "success") and v is not None:
            raise ValueError('modelOutputs must be None message is not "success"')
        return v
