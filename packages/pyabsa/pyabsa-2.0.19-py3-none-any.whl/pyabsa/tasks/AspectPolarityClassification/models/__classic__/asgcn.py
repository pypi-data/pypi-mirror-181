    ...
    def position_weight(self, x, aspect_double_idx, text_len, aspect_len):
        ...
        for i in range(batch_size):
            ...
            for j in range(text_len[i], seq_len):
                weight[i].append(0)
        weight = torch.tensor(weight, dtype=torch.float).unsqueeze(2).to(self.config.device)
        return weight * x
...
class ASGCN(nn.Module):
    ...
    def __init__(self, bert, config):
        ...
        self.dense = nn.Linear(self.config.hidden_dim * 6, self.config.output_dim) \
            if self.config.lsa else nn.Linear(self.config.hidden_dim * 2, self.config.output_dim)
