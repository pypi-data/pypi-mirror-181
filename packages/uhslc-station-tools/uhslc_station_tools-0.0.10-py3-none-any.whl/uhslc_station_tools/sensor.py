...
class Station:
                ...
                if 'ALL' not in key:
                    ...
                    if key in combined_sealevel_data:
                        ...
                        outliers_tuple = (np.sort(np.concatenate((sensor_outliers[key],
                                     utils.find_outliers(time, flat_data, rate, offsets[key])[0]), axis=0)))
    ...
    def save_fast_delivery(self, din_path: str, path: str, is_test_mode=False, callback: Callable = None):
        ...
        for month in self.month_collection:
            ...
            primary_sensor = utils.get_channel_priority(din_path, station_num)  # returns primary channel
            ...
            data_obj[primary_sensor.lower()] = {
                'time': utils.datenum2(_data[primary_sensor].get_time_vector()),
            ...
            if month_end + 1 > 12:
                month_end = 1
                year_end = year + 1
            data_hr = filt.hr_process(data_obj, datetime(year, month.month, 1, 0, 0, 0),
