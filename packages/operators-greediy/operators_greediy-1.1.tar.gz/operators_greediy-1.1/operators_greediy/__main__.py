...
def main():
    ...
    parser.add_argument('cover', type=str, nargs='?', help='A dictionary whose keys are the names of mobile operators, '
                                                 'and the values are the sets of regions they cover', default=[])
    parser.add_argument('regions', type=str, nargs='?', help='The set of all regions that need to be covered', default=[])
    ...
    if test:
        print('TEST')
        if pytest:
            pytest.main(['-q', 'operators_greediy/test.py'])
        else:
            print('No pytest found, only doctest will run')
    else:
        phone_operator_regions = dict()
        phone_operators = re.findall(r'[a-zA-ZА-Яа-я!]+', phone_operator_regions_str)
        regions_covered_mas = re.findall(r'\[[^a-zA-ZА-Яа-я]+]', phone_operator_regions_str)
        regions_covered = []
