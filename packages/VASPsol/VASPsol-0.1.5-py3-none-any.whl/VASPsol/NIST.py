...
import json
import os
from GammaLoaders import GammaLoaders as gl
from difflib import get_close_matches, SequenceMatcher
import matplotlib.pyplot as plt
from sigma_functions import sigma_functions as sf
...
import sigma_functions as sf
def get_tr_sigma_profile(name):
    tr_path = r'C:\Users\efons\Dropbox (UFL)\NASA\VASPsol\Solvation Database'
    tr_csv = pd.read_excel(tr_path+'/'+'MNSol_alldata.xls')
    tr_cosmo_fnames = [tr_path+'/cosmo_files/'+i for i in os.listdir(tr_path+'/cosmo_files/') if '.cosmo' in i]
...
def get_custom_sigma(name):
    name = r'C:\Users\efons\Dropbox (UFL)\NASA\VASPsol\Solvation Database\custom_profiles'+'/'+name+'.cosmo'
    sigma_profile = sf.sigma_function(name)[1]
    size = sf.get_size(name)
    return sigma_profile, size

def tr_sig(name):
    try:
        job = get_custom_sigma(name)
        return job[0], job[1]
    except:
        try:
            job = get_tr_sigma_profile(name)
            size = get_tr_shape(name)
            return job, size
        except:
            try:
                print(name, ', not found in tr set')
                #try:
                df = gl.get_vt_csv()
                choices = df['Compound Name'].to_numpy().astype(np.str_)
                #print(choices)

                match = get_close_matches(name.upper(), choices, n=1)
                if len(match) > 0:
                    match = match[0]
                    ratio = similiar(match, name.upper())
                    print(match, ratio)
                    if ratio > 0.7:
                        job = gl.get_sigma_profile(match)
                        size = gl.get_DFT_outputs(match)[-5:-3]
                        print('FOUND : ',match)
                        return job, size
                    else:
