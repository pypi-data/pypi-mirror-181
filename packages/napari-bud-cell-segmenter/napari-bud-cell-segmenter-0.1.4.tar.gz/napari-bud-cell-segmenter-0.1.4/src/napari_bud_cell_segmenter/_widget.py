...
class Preprocessing(QWidget):
    ...
    def _run_masks(self, button):
        ...
        self.viewer.add_image(ch1_cropped, name="ch1_cropped", colormap="red", visible=False)
        self.viewer.add_image(ch2_cropped, name="ch2_cropped", colormap="green", visible=False)
        self.viewer.add_image(ch3_cropped, name="ch3_cropped", colormap="blue", visible=False)
        self.viewer.add_image(ch4_cropped, name="ch4_cropped", colormap="gray", visible=True)
        self.viewer.add_image(bud_distancemap, name="bud_distancemap", visible=False)
...
def segmentation(viewer: 'napari.viewer.Viewer', layer: Layer, spot_sigma: float = 15, outline_sigma: float = 3) -> "napari.types.LayerDataTuple":
    ...
    print("np.min(np.unique(spots): ", np.min(np.unique(spots)))
    #if np.min(np.unique(spots)) > 0:
...
def detect_blobs(viewer: 'napari.viewer.Viewer', layer: Layer, threshold: float = 300) -> "napari.types.LayerDataTuple":
#def detect_blobs(viewer: 'napari.viewer.Viewer', layer: Layer, max_sigma: float = 2, threshold: float = 300) -> "napari.types.LayerDataTuple":
    ...
    points_layer = (data_layer, {'name': LAYER_NAME, 'size': POINT_SIZE}, 'points')
...
def export_data(viewer: 'napari.viewer.Viewer', dir_path=pathlib.Path()):
    ...
    for layer in layers: 
        print("layer: ", layer)
        save_name = str(dir_path) + "/" + layer.name 
        layer.save(save_name)
    
    """
    #print("viewer.layers[0]: ", viewer.layers[0])
    #print("type(viewer.layers[0]): ", type(viewer.layers[0]))
    #print("viewer.layers.NAMES: ", viewer.layers.NAMES)
    #LayerType = globals().get(f'ImageData')
    #print("type = : ", isinstance(viewer.layers[0], LayerType))
    #print("\nlayers: ", layers)
    #print("\nnb_layers: ", nb_layers)
    ...
    for layer in layers:
        ...
        print("Layer : ", layer_name, "  data_type: ", data_type)
