...
from strategy_api.tm_api.event.engine import EventEngine, EVENT_TIMER, Event
from strategy_api.tm_api.object import TickData, BarData, Interval, OrderRequest, OrderData, Direction, OrderType, \
    Offset, Status, CancelRequest, HistoryRequest
...
class BinanceFutureUsdtGateway(BaseGateway):
    ...
    def on_tick(self, tick: TickData):
        self.on_tick_handler(tick)

    # K线数据回调
    def on_bar(self, bar: BarData):
        self.on_bar_handler(bar)

    # 订单信息回调
    def on_order(self, order: OrderData):
        self.on_order_handler(order)
...
class BinanceUsdtRestApi(RestClient):
    ...
    def query_history(self, req: HistoryRequest) -> List[BarData]:
        ...
        while True:
            ...
            try:
                if self.proxy:
                    resp: Response = requests.get(url=F_REST_HOST + path, params=params,
                                                  proxies={"http": self.proxy[7:], "https": self.proxy[7:]}, timeout=10)
                else:
                    resp: Response = requests.get(url=F_REST_HOST + path, params=params, timeout=10)
            except Exception as e:
                print("查询历史K线数据失败:{}".format(repr(e)))
                break
...
class BinanceUsdtDataWebsocketApi(WebsocketClient):
    ...
    def on_packet(self, packet: dict):
        ...
        bar: BarData = self.bars.get(symbol)
