...
class Listing:
    ...
    def fetch(self):
        sync(get_loop(), self.client.fetch, self)

    @staticmethod
    async def _insert_dir(data_storage, parent_id, name, time, path):
        await data_storage.insert(
            [
                {
                    "parent_id": parent_id,
                    "path": path,
        ...
        new_dir_id = await data_storage.get_dir(parent_id, name)
        return new_dir_id

    async def insert_dir(self, parent_id, name, time, path):
    ...
    async def insert_file(self, parent_id, name, time):
        node = Node(None, parent_id, name, time)
        await self.insert_file_from_node(node)

    async def insert_file_from_node(self, node):
        await self.data_storage.insert([node._asdict()], is_dir=node.is_dir)

    async def insert_root(self) -> int:
        return await self.data_storage.insert_root(self.storage)
