...
class AbstractDataStorage(ABC):
    ...
    @abstractmethod
    async def insert(self, values: List[Dict[str, Any]], **kwargs) -> None:
        """Inserts file or directory node into the database"""

    @abstractmethod
    async def get_dir(self, parent_id: str, name: str) -> Optional[str]:
    ...
    @abstractmethod
    async def insert_root(self, storage: Storage) -> int:
        """
