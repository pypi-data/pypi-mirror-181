...
class DefaultDataStorage(AbstractDataStorage):
    ...
    async def insert(self, values: List[Dict[str, Any]], **kwargs):
        if kwargs.get("is_dir", False):
            self._insert_directories(values)
        else:
            self._insert_files(values)

    async def get_dir(self, parent_id: str, name: str):
        res = self.db.execute(
            f"""
            SELECT dir_id
            FROM {self.table_name}
    ...
    async def insert_root(self, storage: Storage):
        await self.insert([{"dir_id": 0}], is_dir=True, is_root=True)
