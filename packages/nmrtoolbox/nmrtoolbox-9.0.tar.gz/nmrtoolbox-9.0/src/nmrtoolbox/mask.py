...
import itertools
from nmrtoolbox.util import PeakOutsideOfMaskError, AxisData
...
class Mask:
    ...
    def _get_axis_properties(self):
        ...
        # TODO: these properties should all be put in AxisData object
        # get axis labels to ensure that queries against the mask use ppm positions in the same axis order
        self.axis_labels = self.get_xml('connjur:decoupledNucleus', dtype=str)
    ...
    def ppm_in_mask(self, peak_ppm, peak_axis_property, box_radius=2):
        ...
        """
        # TODO
        #  If Mask adopts AxisData, then update this to call self.axis_labels() and get rid of .all()
        if not isinstance(peak_axis_property, AxisData):
            raise TypeError('must provide info about axes as AxisData object')
        if not(peak_axis_property.axis_keys() == self.axis_labels).all():
        ...
        # TODO: first_pt_ppm and last_pt_ppm should be converted into Range objects
