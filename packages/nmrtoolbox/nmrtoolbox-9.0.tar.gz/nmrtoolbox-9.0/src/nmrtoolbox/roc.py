...
class roc:
    ...
    def __init__(
        ...
        if roi_list is not None:
            ...
            if roi_object.axis_keys() != self.recovered_peaks.axis_keys():
    ...
    def _validate_input(self):
        ...
        if '1H' in self.recovered_peaks.axis_keys() and 'HN' in self.injected_peaks.axis_keys():
            self.recovered_peaks.axis_property['HN'] = self.recovered_peaks.axis_property.pop('1H')

        if self.injected_peaks.axis_keys() != self.recovered_peaks.axis_keys():
            raise ValueError('The injected and recovered peak tables have different axis labels.')

        if hasattr(self, 'roi'):
            if self.injected_peaks.axis_keys() != self.roi.axis_keys():
                raise ValueError(
    ...
    def plot_outliers(self, file_out='outliers.pdf', show_figure=False):
        ...
        for dim in [0, 1, 2]:
            ...
            ax.set_title(self.recovered_peaks.axis_keys()[dim])
    ...
    def plot_peaks(self, dir_out=os.getcwd(), file_basename='peaks', show_figure=False):
        ...
        for dims in slice_axes:
            ...
            plt.xlabel(f"{self.recovered_peaks.axis_keys()[xdim]} Domain [ppm]")
            plt.ylabel(f"{self.recovered_peaks.axis_keys()[ydim]} Domain [ppm]")
