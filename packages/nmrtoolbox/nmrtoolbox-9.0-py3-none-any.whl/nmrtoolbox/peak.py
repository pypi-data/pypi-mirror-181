...
class Peak:
    ...
    def axis_dimensions(self):
        ...
        return self.axis_property.get_field(name='dimension')

    def axis2dimension(self, axis):
        """Convert an axis (e.g. 'HN') to a dimension (e.g. 'X')"""
        try:
            return self.axis_property.get_field(name='dimension', axis=axis)[0]
    ...
    def get_par(self, par, axis_dimensions='all'):
        ...
        if '*' not in par:
            ...
            if axis_dimensions == 'all':
                axis_dimensions = self.axis_dimensions()
            ...
            for axis in axis_dimensions:
                if axis not in self.axis_dimensions():
    ...
    def in_mask(self, mask: Mask, box_radius=2):
        ...
        #  maybe upgrade Mask class to use AxisData and then call axis_keys() method here
        if not (mask.axis_labels == self.axis_keys()).all():
            raise ValueError('mask and peak have different axes')

        try:
            return mask.ppm_in_mask(
                peak_ppm=self.get_par('*_PPM'),
                peak_axis_property=self.axis_property,
...
class PeakTable:
    ...
    def _set_carrier_frequency(
        ...
        if carrier_frequency is None:
            ...
            if carrier_frequency.axis_keys() != self.axis_keys():
