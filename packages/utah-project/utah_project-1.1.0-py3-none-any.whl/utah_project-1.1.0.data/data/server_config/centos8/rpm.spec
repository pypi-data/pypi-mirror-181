Name: %{_org}_%{_app}
%define is_se_linux_enabled %( if which getenforce > /dev/null 2>&1; then if [[ $(getenforce) != "Disabled" ]]; then echo "1"; else echo "0"; fi; else echo "0"; fi )
%define is_systemctl_enabled %( if systemctl > /dev/null 2>&1; then echo "1"; else echo "0"; fi )
Requires: httpd
