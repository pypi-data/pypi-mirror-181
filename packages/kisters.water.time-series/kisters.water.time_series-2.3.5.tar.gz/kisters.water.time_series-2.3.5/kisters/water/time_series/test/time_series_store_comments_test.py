...
class TimeSeriesStoreCommentsTest:
    ...
    async def test_02_write_comments(self):
        async with self.STORE.client as client:
            ...
            comments = [comment async for comment in client.read_comments(path=self.TS_PATH)]
            assert len(comments) == 2

    async def test_03_read_comments(self):
        async with self.STORE.client as client:
            first_comment = [
                comment
                async for comment in client.read_comments(
    ...
    async def test_04_delete_comments(self):
        async with self.STORE.client as client:
