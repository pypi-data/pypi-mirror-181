...
const PREDICT_DEF_REGEX = /\s*def\s+predict\s*\((\s*\w+\s*(:\s*\w+\s*)?(=[\w\W]+)?,?){0,3}\)\s*(->\s*\S+\s*)?:\s*/i;
...
const CVAT_INVOKE_DEF_REGEX = /\s*def\s+cvat_invoke\s*\((\s*\w+\s*(:\s*\w+\s*)?(=[\w\W]+)?,?){0,3}\)\s*(->\s*\S+\s*)?:\s*/i;
...
//# sourceMappingURL=lib_index_js.267a6efd39b9773d3e2e.js.map