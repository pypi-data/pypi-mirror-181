...
class WaterSlice:
    ...
    def add_another_slice(self, other_slice):  # this should be -> other_slice: WaterSlice -> but got an error
        ...
        """
        self.temperature = (self.mass * self.temperature + other_slice.mass * other_slice.temperature) / (self.mass + other_slice.mass)
    ...
    def heat_losses_horizontal(self, u_value_tank: Any, seconds_per_timestep: float = 1, ambient_temperature: float = 20)-> Any:
        ...
        # W/m^2*K * m^2 * K * s = Ws
        energy_losses = u_value_tank * loss_area * (self.temperature - ambient_temperature) * seconds_per_timestep
    ...
    def heat_losses_vertical_top_or_bottom(self, u_value_tank: float, seconds_per_timestep: float = 1, ambient_temperature: float = 20) -> float:
        ...
        """
        energy_losses = u_value_tank * self.area * (self.temperature - ambient_temperature) * seconds_per_timestep
...
class StratifiedWarmWaterStorage(Component):
    ...
    def __init__(self, component_name: str,my_simulation_parameters: SimulationParameters,  config: WarmWaterStorageConfig) -> None:
        ...
        # Input
        self.chp_charging_side_input_mass: ComponentInput = self.add_input(self.component_name, StratifiedWarmWaterStorage.CHP_ChargingSideInput_mass, lt.LoadTypes.WARM_WATER, lt.Units.KG_PER_SEC, True)
        self.chp_charging_side_input_temperature: ComponentInput = self.add_input(self.component_name, StratifiedWarmWaterStorage.CHP_ChargingSideInput_temperature, lt.LoadTypes.WARM_WATER, lt.Units.CELSIUS, True)
        self.heating_discharging_side_input_mass: ComponentInput = self.add_input(self.component_name, StratifiedWarmWaterStorage.Heating_DischargingSideInput_mass, lt.LoadTypes.WARM_WATER, lt.Units.KG_PER_SEC, True)
        self.heating_discharging_side_input_temperature: ComponentInput = self.add_input(self.component_name, StratifiedWarmWaterStorage.Heating_DischargingSideInput_temperature, lt.LoadTypes.WARM_WATER, lt.Units.CELSIUS, True)
        # gas --> ?not mandatory?
        self.gas_charging_side_input_mass: ComponentInput = self.add_input(self.component_name, StratifiedWarmWaterStorage.Gas_ChargingSideInput_mass, lt.LoadTypes.WARM_WATER, lt.Units.KG_PER_SEC, True)
        self.gas_charging_side_input_temperature: ComponentInput = self.add_input(self.component_name, StratifiedWarmWaterStorage.Gas_ChargingSideInput_temperature, lt.LoadTypes.WARM_WATER, lt.Units.CELSIUS, True)
        # warm water
        self.ww_discharging_side_input_mass: ComponentInput = self.add_input(self.component_name, StratifiedWarmWaterStorage.WW_DischargingSideInput_mass, lt.LoadTypes.WARM_WATER, lt.Units.KG_PER_SEC, True)
        self.ww_discharging_side_input_temperature: ComponentInput = self.add_input(self.component_name, StratifiedWarmWaterStorage.WW_DischargingSideInput_temperature, lt.LoadTypes.WARM_WATER, lt.Units.CELSIUS, True)

        # Output
        self.chp_charging_side_output_mass: ComponentOutput = self.add_output(self.component_name, StratifiedWarmWaterStorage.CHP_ChargingSideOutput_mass, lt.LoadTypes.WARM_WATER, lt.Units.KG_PER_SEC)
        self.chp_charging_side_output_temperature: ComponentOutput = self.add_output(self.component_name, StratifiedWarmWaterStorage.CHP_ChargingSideOutput_temperature, lt.LoadTypes.WARM_WATER, lt.Units.CELSIUS)
        self.heating_discharging_side_output_mass: ComponentOutput = self.add_output(self.component_name, StratifiedWarmWaterStorage.Heating_DischargingSideOutput_mass, lt.LoadTypes.WARM_WATER, lt.Units.KG_PER_SEC)
        self.heating_discharging_side_output_temperature: ComponentOutput = self.add_output(self.component_name, StratifiedWarmWaterStorage.Heating_DischargingSideOutput_temperature, lt.LoadTypes.WARM_WATER, lt.Units.CELSIUS)
        # gas
        self.gas_charging_side_output_mass: ComponentOutput = self.add_output(self.component_name, StratifiedWarmWaterStorage.Gas_ChargingSideOutput_mass, lt.LoadTypes.WARM_WATER, lt.Units.KG_PER_SEC)
        self.gas_charging_side_output_temperature: ComponentOutput = self.add_output(self.component_name, StratifiedWarmWaterStorage.Gas_ChargingSideOutput_temperature, lt.LoadTypes.WARM_WATER, lt.Units.CELSIUS)
        # warm water
        self.ww_discharging_side_output_mass: ComponentOutput = self.add_output(self.component_name, StratifiedWarmWaterStorage.WW_DischargingSideOutput_mass, lt.LoadTypes.WARM_WATER, lt.Units.KG_PER_SEC)
        self.ww_discharging_side_output_temperature: ComponentOutput = self.add_output(self.component_name, StratifiedWarmWaterStorage.WW_DischargingSideOutput_temperature, lt.LoadTypes.WARM_WATER, lt.Units.CELSIUS)

        # temperatures
        self.temperature_0_percent: ComponentOutput = self.add_output(self.component_name, StratifiedWarmWaterStorage.Temperature0Percent, lt.LoadTypes.WARM_WATER, lt.Units.CELSIUS)
        self.temperature_20_percent: ComponentOutput = self.add_output(self.component_name, StratifiedWarmWaterStorage.Temperature20Percent, lt.LoadTypes.WARM_WATER, lt.Units.CELSIUS)
        self.temperature_40_percent: ComponentOutput = self.add_output(self.component_name, StratifiedWarmWaterStorage.Temperature40Percent, lt.LoadTypes.WARM_WATER, lt.Units.CELSIUS)
        self.temperature_60_percent: ComponentOutput = self.add_output(self.component_name, StratifiedWarmWaterStorage.Temperature60Percent, lt.LoadTypes.WARM_WATER, lt.Units.CELSIUS)
        self.temperature_80_percent: ComponentOutput = self.add_output(self.component_name, StratifiedWarmWaterStorage.Temperature80Percent, lt.LoadTypes.WARM_WATER, lt.Units.CELSIUS)
        self.temperature_100_percent: ComponentOutput = self.add_output(self.component_name, StratifiedWarmWaterStorage.Temperature100Percent, lt.LoadTypes.WARM_WATER, lt.Units.CELSIUS)

        # Outputs for information
        self.tank_enthalpy: ComponentOutput = self.add_output(self.component_name, StratifiedWarmWaterStorage.TankEnthalpy, lt.LoadTypes.WARM_WATER, lt.Units.KWH)
        self.tank_mean_temperature: ComponentOutput = self.add_output(self.component_name, StratifiedWarmWaterStorage.TankMeanTemperature, lt.LoadTypes.WARM_WATER, lt.Units.CELSIUS)
        self.amount_of_slices: ComponentOutput = self.add_output(self.component_name, StratifiedWarmWaterStorage.AmountOfSlices, lt.LoadTypes.WARM_WATER, lt.Units.ANY)
        self.heat_losses: ComponentOutput = self.add_output(self.component_name, StratifiedWarmWaterStorage.HeatLosses, lt.LoadTypes.WARM_WATER, lt.Units.WATT)
        self.tank_mass: ComponentOutput = self.add_output(self.component_name, StratifiedWarmWaterStorage.TankMass, lt.LoadTypes.WARM_WATER, lt.Units.KG)
    ...
    def i_simulate(self, timestep: int, stsv: SingleTimeStepValues, force_convergence: bool) -> None:
        ...
        # charging side
        # from CHP
        chp_charging_side_input_mass_sec = stsv.get_input_value(self.chp_charging_side_input_mass)
        chp_charging_side_input_mass = chp_charging_side_input_mass_sec * self.seconds_per_timestep
        chp_charging_side_input_temperature = stsv.get_input_value(self.chp_charging_side_input_temperature)
        # from gas
        gas_charging_side_input_mass_sec = stsv.get_input_value(self.gas_charging_side_input_mass)
        gas_charging_side_input_mass = gas_charging_side_input_mass_sec * self.seconds_per_timestep
        gas_charging_side_input_temperature = stsv.get_input_value(self.gas_charging_side_input_temperature)
        # total charging input
        total_charging_side_input_mass = chp_charging_side_input_mass + gas_charging_side_input_mass
        is_from_top_upper = True

        if total_charging_side_input_mass > 0:
            total_charging_side_input_temperature = (chp_charging_side_input_mass * chp_charging_side_input_temperature + gas_charging_side_input_mass * gas_charging_side_input_temperature) / total_charging_side_input_mass
        else:
        ...
        if total_discharging_side_input_mass > 0:
            total_discharging_side_input_temperature = (heating_discharging_side_input_mass * heating_discharging_side_input_temperature + ww_discharging_side_input_mass * ww_discharging_side_input_temperature) / total_discharging_side_input_mass
        else:
        ...
        stsv.set_output_value(self.chp_charging_side_output_mass, chp_charging_side_input_mass_sec)
        stsv.set_output_value(self.gas_charging_side_output_mass, gas_charging_side_input_mass_sec)
        ...
        # output temperatures
        stsv.set_output_value(self.chp_charging_side_output_temperature, output_bottom.temperature)
        stsv.set_output_value(self.gas_charging_side_output_temperature, output_bottom.temperature)
