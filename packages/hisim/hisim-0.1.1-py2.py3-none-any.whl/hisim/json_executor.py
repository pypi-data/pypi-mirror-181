...
class JsonExecutor:
    ...
    def process_one_component_entry(self, component_entry: ComponentEntry, class_dict: Any, my_simulation_parameters: SimulationParameters) -> Any:
        ...
        for parameter_name in signature_component.parameters:
            if parameter_name == "my_simulation_parameters":
                ...
                log.warning("Found unclear parameter: " + parameter_name + " in component " + component_entry.component_full_classname)
        if not found_simulation_parameters:
            raise ValueError("The class " + component_entry.component_full_classname + " has no simulation parameters parameter")
        if not found_config:
