...
class MdcWorker:
    ...
    def stop(self, wait_for_flush=False):
        ...
        self._threads = []
        self.logger.info("data collector worker stopped")

    def _thread_run(self, index):
        self.logger.info("worker thread %d: start", index)
        while(not self._stopped):
            ...
            self.logger.debug("worker thread %d: sending payload %s@%s, queue length - %d", index, payload.designation(), payload.id(), queue_len)
            try:
                succeed, msg = self._sender.send(payload)
                if not succeed:
                    self.logger.error("worker thread %d: send payload %s@%s failed - %s", index, payload.designation(), payload.id(), msg)
                else:
                    self.logger.info("worker thread %d: send payload %s@%s succeeded - %s", index, payload.designation(), payload.id(), msg)
            except Exception as ex:
                traceback.print_exc()
                self.logger.error("worker thread %d: send payload %s@%s raise exception: %s", index, payload.designation(), payload.id(), "{0}".format(ex))
        
        self.logger.info("worker thread %d: exit", index)
