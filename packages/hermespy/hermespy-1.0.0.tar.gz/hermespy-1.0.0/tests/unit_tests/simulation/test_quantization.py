...
from hermespy.simulation.analog_digital_converter import AnalogDigitalConverter, Gain, AutomaticGainControl, \
...
class TestQuantization(unittest.TestCase):
    ...
    def test_quantization_no_gain_control(self):
        ...
        quantization_levels = - max_amplitude + quantization_step * (quantization_idx + .5)

        # add random noise within quantization interval
        random_noise = self.rng.uniform(-quantization_step / 2, quantization_step/2,
                                        size=self.num_samples)
        ...
        quantization_levels = np.append(quantization_levels,
