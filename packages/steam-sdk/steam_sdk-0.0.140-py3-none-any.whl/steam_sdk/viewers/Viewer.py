...
class Viewer:
    ...
    def plot_signal(self, event_label: str, meas_label: str, sim_label: str, fig_options: dict, flag_display: bool = True, full_path_figure_to_save: str = None):
        ...
        if flag_display:
            ...
            fig.clf()
            plt.clf()  # clears the figure with all the axis in it (leaves window still open)
            plt.close()  # closes the window
        # return fig, ax   # NOTE: return value not needed... if needed figure has to be cleared at one time
