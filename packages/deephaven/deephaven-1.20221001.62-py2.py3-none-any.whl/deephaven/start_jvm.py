...
def start_jvm(devroot=None,
    ...
    if not any(elem.startswith('--add-opens') for elem in jvm_options):
        if java_version == '11':
            ...
            jvm_options.add('--add-opens=java.base/jdk.internal.misc=ALL-UNNAMED')
            jvm_options.add('--add-opens=java.base/jdk.internal.access=ALL-UNNAMED')
            # This is to workaround a jvm bug, and can be removed when https://github.com/openjdk/jdk/pull/9060 is merged.
            # These options must be in order, and the set defined or passed in won't guarantee that, so we'll change it to a list.
            jvm_options=list(jvm_options)
            jvm_options.append('-XX:+UnlockDiagnosticVMOptions')
            jvm_options.append('-XX:DisableIntrinsic=_currentThread')
