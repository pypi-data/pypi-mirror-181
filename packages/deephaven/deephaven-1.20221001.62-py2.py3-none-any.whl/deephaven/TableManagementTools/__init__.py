...
def getAllDbDirs(tableName, rootDir, levelsDepth):
    ...
    :return: (java.util.List<java.io.File>) all sub-table directories for tableName
    """
    return _custom_getAllDbDirs(tableName, rootDir, levelsDepth)


@_passThrough
def readGroupingTable(*args):
    """
    **Incompatible overloads text - text from the first overload:**
    
    Read a grouping table written by writeGroupingTable(ColumnDefinition, File, Map).
    
    *Overload 1*  
      :param tablePath: (java.io.File) - the path to the source table
      :param keyColDef: (com.illumon.dataobjects.ColumnDefinition<?>...) - the list of ColumnDefinitions of the grouping columns
      :return: (com.illumon.iris.db.tables.Table) a Table containing the groupings or null if it does not exist
      
    *Overload 2*  
      :param tablePath: (java.io.File) - the path to the source table
      :param columnNames: (java.lang.String...) - the column to locate grouping for
      :return: (com.illumon.iris.db.tables.Table) a Table containing the groupings or null if it does not exist
    """
    return _java_type_.readGroupingTable(*args)
...
@_passThrough
def writeDataIndexTable(destinationDir, indexTable, indexColumnName, *keyColumnNames):
    """
    ...
    :param destinationDir: (java.io.File) - the destination for the source table
    :param indexTable: (com.illumon.iris.db.tables.Table) - the table containing the index
    :param indexColumnName: (java.lang.String) - the name of the Index column
    :param keyColumnNames: (java.lang.String...) - the ordered names of key columns
    """
    return _java_type_.writeDataIndexTable(destinationDir, indexTable, indexColumnName, *keyColumnNames)
...
def writeDeephavenTables(sources, tableDefinition, destinations):
    ...
    Write out tables to disk in the Deephaven format.
     All tables are assumed to share the given TableDefinition, which will be written to all locations as passed in.
...
@_passThrough
def writeGroupingTable(groupingColDef, destinationDir, grouping):
    """
    Write out the Grouping table for the specified column.  This will place the grouping information in a table
     adjacent to the data table in a directory titled "Index-<Column name>".
    
    *Overload 1*  
      :param groupingColDef: (com.illumon.dataobjects.ColumnDefinition) - the grouping column definition
      :param destinationDir: (java.io.File) - the destination for the source table
      :param grouping: (java.util.Map<?,com.illumon.iris.db.v2.utils.ReadOnlyIndex>) - the grouping of values
      
    *Overload 2*  
      :param groupingColDef: (com.illumon.dataobjects.ColumnDefinition<?>) - the grouping column definition
      :param destinationDir: (java.io.File) - the destination for the source table
      :param grouping: (com.illumon.iris.db.tables.Table) - the grouping of values
    """
    return _java_type_.writeGroupingTable(groupingColDef, destinationDir, grouping)
...
def writeParquetTables(sources, tableDefinition, codecName, destinations, groupingColumns):
    ...
    :param destinations: (java.io.File[]) - The destination paths.  If the parquet extension is missing, the default parquet file
                            name is used (table.parquet)
...
def writeTable(*args):
    ...
    *Overload 3*  
      :param sourceTable: (com.illumon.iris.db.tables.Table) - source table
      :param definition: (com.illumon.iris.db.tables.TableDefinition) - table definition.  Will be written to disk as given.
