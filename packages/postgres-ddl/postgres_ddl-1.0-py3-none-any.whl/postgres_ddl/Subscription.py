...
class Subscription(DDL):
    ...
    def DDL_Create(self):
        ...
        r += "WITH ("
        r += self.NewLine(1)
        r += f"{self.Indent()}enabled = {str(self.IsEnabled).upper()},"
        r += self.NewLine(1)
        r += f'{self.Indent()}slot_name = "{self.Slot}",'
        r += self.NewLine(1)
        r += f"{self.Indent()}synchronous_commit = '{self.SyncCommit}'"
        r += self.NewLine(1)
        r += ");"
        r += self.NewLine(2)
