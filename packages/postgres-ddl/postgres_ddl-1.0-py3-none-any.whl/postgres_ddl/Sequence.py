...
class Sequence(DDL):
    ...
    def DDL_Create(self):
        ...
        r += self.NewLine(1)
        r += f"{self.Indent()}INCREMENT {self.Increment}"
        r += self.NewLine(1)
        r += f"{self.Indent()}MINVALUE {self.MinValue}"
        r += self.NewLine(1)
        r += f"{self.Indent()}MAXVALUE {self.MaxValue}"
        r += self.NewLine(1)
        r += f"{self.Indent()}START 1"
        r += self.NewLine(1)
        r += f"{self.Indent()}CACHE {self.Cache}"
        r += self.NewLine(1)
        r += f"{self.Indent()}{self.IsCycle}"
        r = r.strip() + ";"
        r += self.NewLine(2)
