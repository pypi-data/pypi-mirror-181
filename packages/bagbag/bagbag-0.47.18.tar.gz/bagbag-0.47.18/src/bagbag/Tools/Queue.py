...
class Queue():  
    ...
    def New(self, queueName:str, size:int=None) -> namedQueue:
    ...
    def NewConfirm(self, queueName:str, size:int=None, timeout:int=900) -> namedConfirmQueue:
        """
...
class namedConfirmQueue():
    ...
    def Get(self, wait=True) -> typing.Tuple[int, typing.Any]:
        self.lock.Acquire()
        while True:
            nowtime = int(Time.Now())
            if nowtime - self.lastchecktimeout > 1:
                r = self.db.Table(self.name).Where("stime", "<", int(Time.Now()) - self.timeout).OrderBy("id").First()
                self.lastchecktimeout = nowtime
            else:
                r = None 
