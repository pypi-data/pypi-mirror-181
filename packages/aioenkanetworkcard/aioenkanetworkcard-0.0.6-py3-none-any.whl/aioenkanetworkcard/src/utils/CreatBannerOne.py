...
import math,re,os,re,os,queue,asyncio
...
async def weaponAdd(weaponRes,characters,lvlName):
    ...
    position,font = centrText(name, witshRam = 315, razmer = 24, start = 159, Yram = 30, y = 13)
    d.text(position, str(name), font= font, fill=coloring) 
    d.text((435 ,53), f"R{lvlUp}", font= t24, fill=(248,199,135,255)) 
    position,font = centrText(f"{lvlName}: {lvl}/90", witshRam = 152, razmer = 17, start = 235, Yram = 28, y = 90)
    d.text(position, f"{lvlName}: {lvl}/90", font= font, fill=coloring) 

    position,font = centrText(baseAtt, witshRam = 90, razmer = 24, start = 180, Yram = 30, y = 50)    
    d.text(position, str(baseAtt), font= font, fill=coloring)
    if proc:
        position,font = centrText(f'{dopStat}%', witshRam = 90, razmer = 24, start = 320, Yram = 30, y = 50)
        d.text(position, f'{dopStat}%', font= font, fill=coloring)
    else:
        position,font = centrText(str(dopStat), witshRam = 90, razmer = 24, start = 320, Yram = 30, y = 50)
...
def nameBanner(nameRes,characters,person,lvlName):
    ...
    centrName,fonts = centrText(characters.name, witshRam = 220, razmer = 33,start = 2)
    ...
    centrName,fonts = centrText(f"{lvlName}: {characters.level}/90", witshRam = 148, razmer = 17, start = 5)
...
async def stats(statRes,characters,assets):
    ...
    for key in reversed(list(g)):
        ...
        pX,fnt = centrText(value, witshRam = 119, razmer = 20, start = 325)
        ...
        if key[0] in dopStatAtribute:
            ...
            if dopStatValArtifact != 0:
                pX,fnt = centrText(f"{dopStatVal} + {dopStatValArtifact}", witshRam = 113, razmer = 13, start = 0)
...
async def talants(talatsRes,characters):
    ...
    for key in characters.skills:
        ...
        talantsBg.paste(imagesIconTalants, (8,7),imagesIconTalants)
        px,fnt = centrText(key.level, witshRam = 25, razmer = 17, start = 3, Yram = 16, y = -1) 
...
def naborArtifact(info,ArtifactNameBg,rezArtSet):
    ...
    for key in info:
        if info[key] > 1:
            ...
            centrName,fonts = centrText(key, witshRam = 240, razmer = 15, start = 4, Yram = 24, y = 1) 
...
async def creatDopStat(infpart,rezArtDop):
    res = []
    for key in infpart:
        ...
        px,fnt = centrText(v, witshRam = 142, razmer = 24, start = 33) 
...
async def creatArtifact(artifacResSave,infpart,imageStats):
    ...
    await creatDopStat(infpart.detail.substats,rezArtDop)
    ...
    centrName,fonts = centrText(val, witshRam = 52, razmer = 17, start = 65)
...
async def generationOne(characters,assets,img,adapt,signatureRes,lvl,splash):
    ...
    try:
        ...
        await asyncio.gather(pic,wep,state,const,tal,artif)
        
        Thread(target=nameBanner,args=(nameRes,characters,person,lvl)).start()
        result =  asyncio.create_task(appedFrame(frame.get(),weaponRes.get(),nameRes.get(),statRes.get(),constantRes.get(),talatsRes.get(),artifacRes.get(),artifactSet.get(),signatureRes))
        result = await result
        return result
