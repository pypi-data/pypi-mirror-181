...
class QottoAuthService:
    ...
    def create_user_request(
            self,
            identity: Union[Identity, str], user: Union[User, str],
            comment: str = None,
    ) -> UserRequest:
        user_id = user.id if isinstance(user, User) else user
        identity_id = identity.id if isinstance(identity, Identity) else identity
        payload = self.api.mutation(
            ...
            input_value=dict(userId=user_id, identityId=identity_id, comment=comment),
    ...
    def reject_user_request(
    ...
    ) -> tuple[Optional[User], Optional[Identity]]:
        user_request_id = user_request.id if isinstance(user_request, UserRequest) else user_request
        payload = self.api.mutation(
        ...
        user = User.from_dict(payload['user']) if payload['user'] else None
