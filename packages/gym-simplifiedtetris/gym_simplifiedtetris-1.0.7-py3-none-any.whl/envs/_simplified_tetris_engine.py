...
class SimplifiedTetrisEngine(object):
    ...
    def __init__(
        ...
        self._grid = np.zeros((self._width, self._height), dtype="bool")
        self._colour_grid = np.zeros((self._width, self._height), dtype="int")
    ...
    def _render(self, mode: str = "human") -> np.ndarray:
        ...
        if mode == "human" and self._show_agent_playing:
            ...
            ) == 3:  # Right arrow has been pressed.
    ...
    def _get_grid(self) -> np.ndarray:
        """Return the array of the current grid containing the colour tuples.
    ...
    def _update_anchor(self) -> None:
        ...
        self._anchor = [int(self._width / 2 - 1), self._piece_size - 1]
    ...
    def _is_illegal(self) -> bool:
        ...
        for x_coord, y_coord in self._piece._coords:
            ...
            if (
                (block_off_left := (x_pos < 0))
                or (block_off_right := (x_pos >= self._width))
                or (block_below_bot := (y_pos >= self._height))
    ...
    def get_best_action(
        ...
        """Find the best action out of the available actions by looking ahead to future possible states and averaging over their V-values.
    ...
    def get_col_heights(self) -> np.array:
        """Get the column heights of the current grid.
    ...
    def get_feat_values(self, obs_space: str) -> np.array:
        """Get the feature values according to the obs space.
