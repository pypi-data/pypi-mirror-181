...
class Polymino(object):
    ...
    def __post_init__(self):
        """Initialise the Polymino object."""
        ...
        self._max_y_coord = self._generate_max_min("max_y_coord")
        self._min_y_coord = self._generate_max_min("min_y_coord")
        self._max_x_coord = self._generate_max_min("max_x_coord")
        self._min_x_coord = self._generate_max_min("min_x_coord")

    def _generate_max_min(self, coord_stat: str) -> Dict[int, int]:
        """Return the max or min x or y coordinates.

        :param coord_stat: coordinate statistic
        :return: max or min x or y coordinates.
        """
        settings = MAX_MIN_SETTINGS[coord_stat]

        func = settings["func"]
        idx = settings["index"]

        max_min_coords = {}
        for rotation, coords in self._all_coords.items():
            max_min_coords[rotation] = func([coord[idx] for coord in coords])
