...
def _log(
  ...
  logging.info(
    f'Replicated dense weights: {[v.name.split(":")[0] for v in dense_vars]}')
  logging.info(
    f'Replicated sparse weights: {[v.name.split(":")[0] for v in sparse_vars]}')
  logging.info(
    f'Sharded weights: {[v.name.split(":")[0] for v in sharded_vars]}')
