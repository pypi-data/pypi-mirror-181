...
from hestia_earth.models.utils.site import region_factor
from . import MODEL
...
def should_download(site: dict, by_region=True) -> bool:
    try:
        from hestia_earth.earth_engine import should_run
    except ImportError:
...
def download(term: str, site: dict, data: dict, by_region=True) -> dict:
    ...
    try:
        ...
        debugValues(site, collection=collection, time=f"{current_time_ms() - now}ms", properties=properties)
        return properties
    except Exception as e:
...
def get_region_factor(term_id: str, site: dict, termType: TermTermType = TermTermType.MEASUREMENT):
    ...
    value = region_factor(MODEL, region_id, term_id, termType) or region_factor(MODEL, country_id, term_id, termType)
    debugValues(site, model=MODEL, term=term_id,
                lookup_value=value)
    return value
