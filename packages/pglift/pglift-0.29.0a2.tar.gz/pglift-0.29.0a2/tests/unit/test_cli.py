...
from pglift.cli import CLIContext, InvalidSettingsError, Obj, cli
...
def test_site_configure(
    runner: CliRunner, settings: Settings, obj: Obj, tmp_path: Path
) -> None:
    ...
    do_install.assert_called_once_with(settings, env=f"SETTINGS=@{tmp_path}")

    with patch.object(_install, "undo") as undo_install:
        result = runner.invoke(cli, ["site-configure", "uninstall"], obj=obj)
    assert result.exit_code == 0, result
    undo_install.assert_called_once_with(settings)
...
def test_argspec(
    ...
    returns_fpath = datadir / f"ansible-returns-{objtype}.json"
    if write_changes:
        returns_fpath.write_text(json.dumps(returns, indent=2, sort_keys=True) + "\n")
    returns_expected = json.loads(returns_fpath.read_text())
    assert returns == returns_expected
...
def test_instance_create(
    ...
    result = runner.invoke(cli, ["instance", "create", "in/va/lid"], obj=obj)
