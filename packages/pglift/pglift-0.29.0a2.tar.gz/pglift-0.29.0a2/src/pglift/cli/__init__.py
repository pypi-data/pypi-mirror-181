...
from .. import _install, exceptions, version
...
from .util import Group, pass_ctx
...
@pass_ctx
def site_settings(ctx: Context, schema: bool) -> None:
    """Show site settings."""
    if schema:
        rich.print_json(Settings.schema_json())
    else:
...
def site_configure(
    ...
    if action == "install":
        ...
        _install.do(ctx.settings, env=env)
    elif action == "uninstall":
        _install.undo(ctx.settings)
