...
from libraryArsein.PostData import http,httpfiles,get_Errors
import asyncio
...
class Robot_Rubika:
    ...
    def __init__(self, Sh_account: str):
        self.Auth = "".join(findall(r"\w",Sh_account))
        self.prinet = copyright_Arsein.arsein
        self.enc = encoderjson(Sh_account)
        if self.Auth.__len__() < 32 or self.Auth.__len__() > 32:
            print("شناسه اکانت شما اشتباه است  لطفا مجدد بررسی نمائید")
        else:
            if get_Errors == True:
                print("شناسه اکانت شما اشتباه است  لطفا مجدد بررسی نمائید")
