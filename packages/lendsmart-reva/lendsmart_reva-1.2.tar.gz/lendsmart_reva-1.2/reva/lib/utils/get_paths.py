...
class PathGetter:
        ...
        for namespace in namespaces:
            files_to_update.extend(
                self.get_files_with_prefix(
                    self.get_ui_config_path(namespace), ["WFI_"]
                    )
            )
    ...
    def get_site_settings_json_path(self, namespaces: list):
        ...
        for namespace in namespaces:
            files_to_update.extend(
                self.get_files_with_prefix(
                    self.get_ui_config_path(namespace), ["STS_"]
                    )
            )
        return files_to_update

    def get_loan_products_json_path(self, namespaces: list):
        """
        Returns the file paths of loan products
        """
        files_to_update = []
        for namespace in namespaces:
            files_to_update.extend(
                self.get_files_with_prefix(
                    self.get_ui_config_path(namespace), ["LPD_"]
                    )
            )
        return files_to_update

    def get_document_access_control_json_path(self, namespaces: list):
        """
        Returns the file paths of document access control
        """
        files_to_update = []
        for namespace in namespaces:
            files_to_update.extend(
                self.get_files_with_prefix(
                    self.get_ui_config_path(namespace), ["DAC_"]
                    )
            )
        return files_to_update
