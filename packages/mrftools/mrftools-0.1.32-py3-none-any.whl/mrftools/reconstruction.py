...
import ISMRMRDQuaternionHelper
import time
import torchkbnufft as tkbn
from tqdm import tqdm
from mrftools import DictionaryEntry
import CoilCombine
import kornia as K
...
# Images should have shape [svdComponents, coils, z, x, y]
def PerformWalshCoilCombination(images, verbose=False, kernelSize=5, niter=5, device=None):
    if(device==None):
        if torch.cuda.is_available():
            device = torch.device("cuda")
        else:
    ...
    coil_map, rho = CoilCombine.calculate_csm_walsh(images[0,:,:,:,:], smoothing=kernelSize, niter=niter) 
...
def PerformInatiCoilCombination(images, verbose=False, kernelSize=5, niter=5, device=None):
    ...
    shape = np.shape(images)
    combinedImageData = torch.zeros((shape[0], shape[2], shape[3], shape[4]), dtype=torch.complex64)
    coil_map, combinedImageData[0, :, :, :] = CoilCombine.calculate_csm_inati_iter_local(images[0,:,:,:,:], verbose=verbose, smoothing=kernelSize, niter=niter) 
