...
def ERP_customersave(api_sdk, option, dData, app2, rc, app3):
    ...
    for i in dData:
        ...
        #     continue
        if rc.getStatus(app3, i['FNumber'], 'RDS_OA_ODS_bd_CustomerDetail') and judgeDate(i['FNumber'],
                                                                                          api_sdk) == False:
            model = {
                "Model": {
                    "FCUSTID": 0,
...
def SaveAfterAllocation(api_sdk, i, rc, app2):
    FOrgNumber = rc.getOrganizationFNumber(app2, i['FApplyOrgName'])

    model = {
        "Model": {
            "FCUSTID": queryDocuments(app2, i['FNumber'], FOrgNumber['FORGID']),
            "FCreateOrgId": {
                "FNumber": "100"
            },
            "FUseOrgId": {
    ...
    print(f"修改编码为{i['FNumber']}的信息:" + res)
...
def AlloctOperation(forbid, number, api_sdk, i, rc, app2):
    ...
    SaveAfterAllocation(api_sdk, i, rc, app2)
