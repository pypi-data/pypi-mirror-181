...
async def present(
    ...
    result["comment"].append(
        "No-op: There is no create/update function for gcp.compute.images"
    )
...
async def absent(
    ...
    if not ctx.get("rerun_data"):
        delete_ret = await hub.exec.gcp_api.client.compute.images.delete(
        ...
        handle_operation_ret = await hub.tool.gcp.operation_utils.handle_operation(
            ctx, ctx.get("rerun_data"), "compute.images"
        )
...
async def describe(hub, ctx) -> Dict[str, Dict[str, Any]]:
    ...
    Examples:
        .. code-block:: bash
            ...
            $ idem describe gcp.compute.images
    """
    result = {}

    describe_ret = await hub.exec.gcp.compute.images.list(
