...
async def list_(
    ...
    Examples:
        .. code-block: sls
            ...
            random-name:
              exec.run:
              - path: gcp.compute.accelerator_types.list
              - kwargs:
    ...
    if zone and not include_all_scopes:
        ret = await hub.exec.gcp_api.client.compute.accelerator_types.list(
            ctx,
            project=project,
            zone=zone,
            filter=filter,
        ...
        ret = await hub.exec.gcp_api.client.compute.accelerator_types.aggregatedList(
            ctx,
            project=project,
            filter=filter,
            orderBy=order_by,
...
async def get(
    ...
    Examples:
        .. code-block: sls
            ...
            random-name:
              exec.run:
              - path: gcp.compute.accelerator_types.get
              - kwargs:
    ...
    if resource_id:
        ret = await hub.exec.gcp_api.client.compute.accelerator_types.get(
            ctx,
        ...
        result["result"] = False
        result["comment"] = [
            f"gcp.compute.accelerator_types {name} either resource_id or project, zone and name"
