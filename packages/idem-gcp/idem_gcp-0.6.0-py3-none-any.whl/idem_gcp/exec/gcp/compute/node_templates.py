...
async def list_(
    ...
    Examples:
        .. code-block: sls
            ...
            random-name:
              exec.run:
              - path: gcp.compute.node_templates.list
              - kwargs:
    ...
    if region:
        ret = await hub.exec.gcp_api.client.compute.node_templates.list(
            ctx,
            project=project,
            region=region,
            filter=filter,
        ...
        ret = await hub.exec.gcp_api.client.compute.node_templates.aggregatedList(
            ctx,
            project=project,
            filter=filter,
            maxResults=max_results,
...
async def get(
    hub, ctx, project: str, region: str, node_template: str
) -> Dict[str, Any]:
    ...
    Examples:
        .. code-block: sls
            ...
            random-name:
              exec.run:
              - path: gcp.compute.node_templates.get
              - kwargs:
                  project: project-name
                  region: region
    ...
    ret = await hub.exec.gcp_api.client.compute.node_templates.get(
