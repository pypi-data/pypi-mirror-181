...
async def list_(
    ...
    Examples:
        .. code-block: sls
            ...
            random-name:
              exec.run:
              - path: gcp.compute.resource_policies.list
              - kwargs:
    ...
    if region:
        ret = await hub.exec.gcp_api.client.compute.resource_policies.list(
            ctx,
            project=project,
            region=region,
            filter=filter,
        ...
        ret = await hub.exec.gcp_api.client.compute.resource_policies.aggregatedList(
            ctx,
            project=project,
            filter=filter,
            orderBy=order_by,
...
async def get(
    ...
    Examples:
        .. code-block: sls
            ...
            random-name:
              exec.run:
              - path: gcp.compute.resource_policies.get
              - kwargs:
                  project: project-name
                  region: region-name
    ...
    if resource_id:
        ret = await hub.exec.gcp_api.client.compute.resource_policies.get(
            ctx,
        ...
        result["result"] = False
        result["comment"] = [
            f"gcp.compute.resource_policies {name} either resource_id or project, region and name"
