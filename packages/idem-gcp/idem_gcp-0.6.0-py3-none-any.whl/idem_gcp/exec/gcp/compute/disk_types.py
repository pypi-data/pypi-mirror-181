...
async def list_(
    ...
    Examples:
        .. code-block: sls
            ...
            random-name:
              exec.run:
              - path: gcp.compute.disk_types.list
              - kwargs:
    ...
    if zone and not include_all_scopes:
        ret = await hub.exec.gcp_api.client.compute.disk_types.list(
            ctx,
            project=project,
            zone=zone,
            filter=filter,
        ...
        ret = await hub.exec.gcp_api.client.compute.disk_types.aggregatedList(
            ctx,
            project=project,
            filter=filter,
            orderBy=order_by,
...
async def get(
    ...
    Examples:
        .. code-block: sls
            ...
            random-name:
              exec.run:
              - path: gcp.compute.disk_types.get
              - kwargs:
                  name: disk-type-name
                  project: project-name
                  zone: zone-name
    ...
    if resource_id:
        ret = await hub.exec.gcp_api.client.compute.disk_types.get(
            ctx,
        ...
        result["result"] = False
        result["comment"] = [
            f"gcp.compute.disk_types {name} either resource_id or project, zone and name"
