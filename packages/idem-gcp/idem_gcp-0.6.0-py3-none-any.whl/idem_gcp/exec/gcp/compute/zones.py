...
async def list_(
    ...
    ret = await hub.exec.gcp_api.client.compute.zones.list(
        ctx,
        project=project,
        filter=filter,
        orderBy=order_by,
...
async def get(
    ...
    if resource_id:
        ret = await hub.exec.gcp_api.client.compute.zones.get(
            ctx,
        ...
        result["result"] = False
        result["comment"] = [
            f"gcp.compute.zones {name} either resource_id or project and name"
