...
def construct_resource_id(hub, resource_type: str, input_props: Dict[str, Any]) -> str:
    ...
    resource_path = hub.tool.gcp.resource_prop_utils.get_resource_path(resource_type)
    try:
        return resource_path.format(**input_props)
    except:
...
def get_resource_replace_method_request_param_names(
    hub, resource_type: str
    ...
    replace_method_request_param_names = None
    try:
        replace_method_request_param_names = hub_ref["REPLACE_METHOD_REQUEST_PARAMS"]
    finally:
        return replace_method_request_param_names


def get_updated_request_param_names(
    hub, resource_type: str, method_name: str, params: Dict[str, Any]
) -> Dict[str, Any]:
    replace_method_request_param_names = hub.tool.gcp.resource_prop_utils.get_resource_replace_method_request_param_names(
        resource_type
    )
    params_copy = copy.deepcopy(params)
    if (
        replace_method_request_param_names is not None
        and method_name in replace_method_request_param_names.keys()
    ):
        method_param_names_to_replace = replace_method_request_param_names[method_name]

        for (
            current_param_name,
            param_name_replacement,
        ) in method_param_names_to_replace.items():
            if current_param_name in params_copy.keys():
                params_copy[param_name_replacement] = params_copy.pop(
                    current_param_name
                )
