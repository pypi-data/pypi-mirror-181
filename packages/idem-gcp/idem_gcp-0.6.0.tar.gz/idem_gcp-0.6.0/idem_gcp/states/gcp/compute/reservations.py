...
async def present(
    ...
    result["comment"].append(
        "No-op: There is no create/update function for gcp.compute.reservations"
    )
...
async def absent(
    ...
    Examples:
        .. code-block:: sls
            ...
            my-reservation:
              gcp.compute.reservations.absent:
    ...
    if not ctx.get("rerun_data"):
        ...
        delete_ret = await hub.exec.gcp_api.client.compute.reservations.delete(
        ...
        handle_operation_ret = await hub.tool.gcp.operation_utils.handle_operation(
            ctx, ctx.get("rerun_data"), "compute.reservations"
        )
    ...
    result["comment"].append(
        hub.tool.gcp.comment_utils.delete_comment("gcp.compute.reservations", name)
    )
    return result
...
async def describe(hub, ctx) -> Dict[str, Dict[str, Any]]:
    ...
    Examples:
        .. code-block:: bash
            ...
            $ idem describe gcp.compute.reservations
    """
    result = {}

    # TODO: Pagination
    describe_ret = await hub.exec.gcp_api.client.compute.reservations.aggregatedList(
        ctx, project=ctx.acct.project_id
    )

    if not describe_ret["result"]:
        hub.log.debug(
            f"Could not describe gcp.compute.reservations {describe_ret['comment']}"
        )
        return {}
    ...
    for resource in describe_ret["ret"]["items"]:
        ...
        result[resource_id] = {
            "gcp.compute.reservations.present": [
