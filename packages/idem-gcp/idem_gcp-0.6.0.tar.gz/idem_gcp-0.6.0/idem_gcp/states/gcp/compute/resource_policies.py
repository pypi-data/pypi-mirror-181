...
async def present(
    ...
    Examples:
        .. code-block:: sls
            ...
            example_resource_name:
              gcp.compute.resource_policies.present:
                - name: value
                - project: value
                - region: value
    """
    ...
    result["comment"].append(
        "No-op: There is no create/update function for gcp.compute.resource_policies"
    )
...
async def absent(
    ...
    Examples:
        .. code-block:: sls
            ...
            resource_is_absent:
              gcp.compute.resource_policies.absent:
    ...
    if not ctx.get("rerun_data"):
        delete_ret = await hub.exec.gcp_api.client.compute.resource_policies.delete(
        ...
        handle_operation_ret = await hub.tool.gcp.operation_utils.handle_operation(
            ctx, ctx.get("rerun_data"), "compute.resource_policies"
        )
...
async def describe(hub, ctx) -> Dict[str, Dict[str, Any]]:
    ...
    Examples:
        .. code-block:: bash
            ...
            $ idem describe gcp.compute.resource_policies
    """
    result = {}

    describe_ret = await hub.exec.gcp.compute.resource_policies.list(
