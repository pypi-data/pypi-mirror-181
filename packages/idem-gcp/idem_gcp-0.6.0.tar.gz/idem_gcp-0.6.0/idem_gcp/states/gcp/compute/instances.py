...
async def present(
    ...
    if not resource_id:
        resource_id = (ctx.get("old_state") or {}).get(
            ...
            "compute.instances", {**locals(), "instance": name}
        )
    ...
    if resource_id:
        old_get_ret = await hub.exec.gcp.compute.instances.get(
    ...
    if result["old_state"]:
        ...
        # Handling cases where if the user does not provide a value for these properties,
        # their values must be taken from the latest get responses, i.e. old
        ...
        # network_interface is not updatable via this method
        # Perform update
        ...
        if not update_ret["result"]:
            ...
            result["comment"].append(
                hub.tool.gcp.comment_utils.update_comment("gcp.compute.instances", name)
            )
        ...
        if not create_ret["result"]:
            ...
            result["comment"].append(
                hub.tool.gcp.comment_utils.create_comment("gcp.compute.instances", name)
            )
    ...
    get_ret = await hub.exec.gcp.compute.instances.get(ctx, resource_id=resource_id)
...
async def absent(
    ...
    result["comment"].append(
        hub.tool.gcp.comment_utils.delete_comment("gcp.compute.instances", name)
    )
    return result
...
async def describe(hub, ctx) -> Dict[str, Dict[str, Any]]:
    ...
    Examples:
        .. code-block:: bash
            ...
            $ idem describe gcp.compute.instances

    """
    result = {}

    describe_ret = await hub.exec.gcp.compute.instances.list(
