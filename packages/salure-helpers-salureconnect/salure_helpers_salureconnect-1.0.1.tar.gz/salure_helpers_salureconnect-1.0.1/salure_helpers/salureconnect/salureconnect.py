...
class SalureConnect:
    def __init__(self, customer: str = None, api_token: str = None, staging: bool = False):
        if os.getenv("SALURECONNECT_CUSTOMER_NAME") is not None:
            self.customer = os.getenv("SALURECONNECT_CUSTOMER_NAME")
            self.api_token = os.getenv("SALURECONNECT_API_TOKEN")
        elif customer is not None and api_token is not None:
            self.customer = customer
            self.api_token = api_token
        else:
            raise ValueError('No customer name or api token specified')
