...
def choice_filter(typ,choices):
  t_ = typ+'_'
  return [c.replace(t_,'',1) for c in choices if c.startswith(t_)]
...
def get_names(args):
  ...
  for typ in vytools.config.ITEMTYPES:
    ...
    if subtypelist is not None:
      lst += [typ+':'+name for name in subtypelist]
  return lst
...
def upload_parser(subparsers, choices):
  upload_sub_parser = subparsers.add_parser('upload',
    help='Upload bundles and/or nodules')
  ...
  subtype(['bundle','nodule'], choices, upload_sub_parser)
...
def main():
  ...
  if not notConfiguring:
    ...
    if args.action == 'build':
      ...
      results = vytools.run(lst, anchors=anchors, clean=args.clean, save=args.save, object_mods=object_mods, cmd=cmd, persist=persist, compose=rootcompose)
