...
class ScenarioWrapper(AbstractScenarioWrapper):
    ...
    def get_rou_file(self) -> str:
        ...
        # find net-file
        rou_files = tree.findall('route-files')  # e.g., vehicles, and pedestrians
        ...
        for vehicle in tree_routes.findall("vehicle"):
            edges = list(map(int, vehicle.find("route").get("edges").split(' ')))
            edges_corr = []
            ...
            if route_did_change:  # overwite file
                edges_str = ' '.join(map(str, edges_corr))
                vehicle.find("route").set("edges", edges_str)

        if file_did_change:
            warnings.warn("Route had to be corrected. Save as new file and return its file path.")
            name = rou_file.attrib['value'].split('.')
            name.insert(1,"corr")
            new_name = '.'.join(name)
            new_path = os.path.join(file_directory, new_name)
            tree_routes_orig.write(new_path)
            rou_path = new_path
