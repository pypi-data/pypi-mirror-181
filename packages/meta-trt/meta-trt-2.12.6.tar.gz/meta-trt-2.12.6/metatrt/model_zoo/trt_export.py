...
class EngineBuilder:
    ...
    def create_engine(self, trt_file, precision, calib_input=None, calib_cache=None, calib_num_images=5000,
                      calib_batch_size=8):
        """
        Build the TensorRT engine and serialize it to disk.
        :param trt_file: The path where to serialize the engine to.
        ...
        print("Building {} Engine in {}".format(precision, trt_file))
        inputs = [self.network.get_input(i) for i in range(self.network.num_inputs)]
        ...
        with self.builder.build_engine(self.network, self.config) as engine, open(trt_file, "wb") as f:
            print("Serializing engine to file: {:}".format(trt_file))
