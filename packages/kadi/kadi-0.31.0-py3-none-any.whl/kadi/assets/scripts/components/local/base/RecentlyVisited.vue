...
<template>
  <div class="card" v-show="items.length > 0" v-if="kadi.globals.user_active">
    <div class="card-header py-1">
      <collapse-item :id="id" :is-collapsed="collapsed" @collapse="collapsed = $event">
        <strong>{{ $t('Recently visited') }}</strong>
      </collapse-item>
    </div>
    <div class="card-body collapse items" :id="id" ref="body">
      <div class="list-group list-group-flush">
        <div class="list-group-item list-group-item-action text-body" v-for="item in items" :key="item.id">
          <a class="stretched-link" :href="item.endpoint">
...
export default {
  ...
  mounted() {
    if (!kadi.globals.user_active) {
      ...
      if (!this.collapsed) {
        // Append this class manually once, as it is handled by the collapse item component afterwards.
