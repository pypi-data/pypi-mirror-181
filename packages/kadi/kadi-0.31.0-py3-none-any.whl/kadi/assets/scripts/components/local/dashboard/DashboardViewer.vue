...
<template>
  <div>
    ...
    <div v-if="!inEditMode" class="row">
      <div class="col-md-8">
        <div v-if="isEditable">
          <button class="btn btn-sm btn-primary my-1" @click="newDashboard">
            <i class="fa-solid fa-plus"></i>
            {{ $t('New') }}
          </button>
          <button class="btn btn-sm btn-primary my-1" v-if="selectedDashboardFile" @click="enterEditMode">
            <i class="fa-solid fa-pencil"></i>
            {{ $t('Edit') }}
          </button>
          <button class="btn btn-sm btn-primary my-1" v-if="selectedDashboardFile" @click="deleteDashboard">
...
export default {
  ...
  methods: {
    ...
    saveDashboard() {
      ...
      const filename = `${this.activeDashboard.name}.json`;

      const file = new File([JSON.stringify(this.activeDashboard, null, 2)], filename);
      const upload = this.directUpload.create({
        file,
        size: file.size,
      });

      upload.name = filename;
      upload.forceReplace = false;
