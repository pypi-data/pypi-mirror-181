...
def get_crl_by_uri(base_url: str, uri: str) -> bytes:
    """Retrieve CRL by URI."""
    response = requests.get(url=f"{base_url}/sgx/certification/v3/crl",
                            json={"uri": uri},
                            timeout=30)
...
def get_root_ca_crl(base_url: str) -> CertificateRevocationList:
    ...
    response = requests.get(url=f"{base_url}/sgx/certification/v3/rootcacrl",
                            timeout=30)

    return load_der_x509_crl(bytes.fromhex(response.text))


def get_pck_certificate(base_url: str, encrypted_ppid: Optional[str],
                        cpusvn: str, pcesvn: str, pceid: str, qeid: str) -> str:
    """Retrieve PCK Certificate from parameters."""
    response = requests.get(url=f"{base_url}/sgx/certification/v3/pckcert",
                            params={
...
def get_pck_cert_crl(
        base_url: str, ca: Literal["processor",
                                   "platform"]) -> CertificateRevocationList:
    """Retrieve PCK CRL."""
    response = requests.get(url=f"{base_url}/sgx/certification/v3/pckcrl",
                            params={
                                "ca": ca,
                                "encoding": "der"
                            },
...
def get_tcbinfo(
        base_url: str,
        fmscp: str) -> Tuple[Tuple[Certificate, Certificate], Dict[str, Any]]:
    """Retrieve TCB info from `fmscp`."""
    response = requests.get(url=f"{base_url}/sgx/certification/v3/tcb",
                            params={"fmspc": fmscp},
                            timeout=30)
...
def get_qe_identity(
    ...
    response = requests.get(url=f"{base_url}/sgx/certification/v3/qe/identity",
                            timeout=30)
...
def get_qve_identity(
    ...
    response = requests.get(url=f"{base_url}/sgx/certification/v3/qve/identity",
                            timeout=30)
