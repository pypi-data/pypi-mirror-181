...
class ReportBody:  # 384 bytes
    ...
    def __bytes__(self) -> bytes:
        ...
        return REPORT_BODY.pack(self.cpu_svn, self.misc_select, self.reserved1,
                                self.isv_ext_prod_id, self.flags, self.xfrm,
                                self.mr_enclave, self.reserved2, self.mr_signer,
                                self.reserved3, self.config_id,
                                self.isv_prod_id, self.isv_svn, self.config_svn,
                                self.reserved4, self.isvn_family_id,
                                self.report_data)
...
class Quote:
    ...
    def from_bytes(cls, raw_quote: bytes) -> 'Quote':
        ...
        offset: int = 48 + 384
        (version, sign_type, epid_group_id, qe_svn, pce_svn, xeid, basename,
         raw_report_body) = QUOTE.unpack(view[:offset])
        report_body: ReportBody = ReportBody.from_bytes(raw_report_body)
        signature_len: int = int.from_bytes(view[offset:offset + 4],
                                            byteorder="little")
        offset += 4
    ...
    def __bytes__(self) -> bytes:
        """Serialize Quote."""
        return QUOTE.pack(self.version, self.sign_type, self.epid_group_id,
                          self.qe_svn, self.pce_svn, self.xeid, self.basename,
                          bytes(
                              self.report_body)) + self.signature_len.to_bytes(
                                  4, byteorder="little") + self.signature
