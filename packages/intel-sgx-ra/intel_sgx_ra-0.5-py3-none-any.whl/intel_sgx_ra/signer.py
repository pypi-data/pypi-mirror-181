...
def mr_signer_from_pk(public_key: Union[RSAPublicKey, Path, bytes]) -> bytes:
    ...
    if isinstance(public_key, bytes):
        ...
        pk = cast(RSAPublicKey,
                  load_pem_public_key(data=Path(public_key).read_bytes()))
    else:
        pk = public_key

    modulus: bytes = pk.public_numbers().n.to_bytes(pk.key_size // 8,
                                                    byteorder="little")

    return hashlib.sha256(modulus).digest()


def mr_signer_from_cert(
        certificate: Union[x509.Certificate, Path, bytes]) -> bytes:
    """Compute MRSIGNER from X.509 certificate."""
    cert: x509.Certificate

    if isinstance(certificate, bytes):
        cert = cast(x509.Certificate,
                    x509.load_pem_x509_certificate(data=certificate))
    ...
    if not isinstance(cert.public_key(), RSAPublicKey):
        raise Exception("Certificate public key must be RSA public key")
