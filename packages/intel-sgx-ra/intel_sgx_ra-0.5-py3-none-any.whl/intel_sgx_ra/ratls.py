...
def ratls_verification(
        ratls_cert: Union[str, bytes, Path, x509.Certificate]) -> Quote:
    ...
    success: bool = hashlib.sha256(
        pk).digest() == quote.report_body.report_data[:32]

    logging.info("[ %4s ] ra-tls verification of public key",
...
def ratls_verification_from_url(url: str) -> Quote:
    ...
    ca_data: bytes = ssl.get_server_certificate(
        (hostname, int(port))).encode("utf-8")
