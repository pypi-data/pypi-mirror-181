...
def _parseTail(userDebug=True,logUserId=None,level=None,whereClause=None,deleteLogs=True):
    deleteList = []
    def deleteRecords(delrecs):
        for rec in delrecs['records']:
            Sobjects.delete('ApexLog',rec)
    ...
    while (True):
        ...
        if len(logRecords) > 0:
            ...
            for record in logRecords:
                ...
                _timez = _time.split('.')[0] + "Z"
                if deleteLogs==True:
                    a['records'].append(record['Id'])
            
            if deleteLogs==True:
                print(f"Deleting records in ApexLog {a}")
                threading.Thread(target=deleteRecords,args=(a,), daemon=True).start()
...
def parseLog(logId=None,filepath=None,printLimits=True,userDebug=True,lastN=1,logUserId=None,level=None,whereClause=None,writeToFile=False,tail=False,deleteLogs=False):
    body = None

    if tail == True:
        _parseTail(userDebug=userDebug,logUserId=logUserId,level=level,whereClause=whereClause,deleteLogs=deleteLogs)
