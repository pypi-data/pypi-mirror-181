...
def option_d(args):
    help = f"""
        {utils.CYELLOW}-d objectName{utils.CEND} --> Describe an object
            {utils.CYELLOW}-d objectName:fieldName{utils.CEND} --> describe a field in the objec"""
    if '-h' in args:
        return help    
...
def option_logs(args):
    help = f"""
        {utils.CYELLOW}-logs Id{utils.CEND} --> parse the log with the provided Id
        {utils.CYELLOW}-logs -last X {utils.CEND}--> parses the last X logs. X as number.
        {utils.CYELLOW}-logs{utils.CEND} --> lis log records in the org. 
            ...
            {utils.CYELLOW}-where X {utils.CEND}--> parses the last X logs. 
            ...
            {utils.CYELLOW}-inputfile {utils.CEND}--> parses the file provided in the inputfile. {utils.CGREEN}InCli -logs -inputfile "path to file/xxxx.log"{utils.CEND}"""
    ...
    if logId == None and last== None and inputfile==None:
        connectionInit(args)
        ...
        debugLogs.printLogs(logUserId=id,limit=lim,whereClause=whereClause)
        return

    if inputfile == None:
        connectionInit(args)
    debugLogs.parseLog(logId,lastN=last,level=level,whereClause=whereClause,writeToFile=toFile,filepath=inputfile)
