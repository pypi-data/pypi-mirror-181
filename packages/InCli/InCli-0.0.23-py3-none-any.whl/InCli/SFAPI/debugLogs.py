...
def parseLog(logId=None,filepath=None,printLimits=True,userDebug=True,lastN=1,logUserId=None,level=None,whereClause=None):
    body = None
    ...
    return parse(body,printLimits=printLimits,userDebug=userDebug,level=level)
...
def parse(logData,printLimits=True,userDebug=True,level=None):
 ...
 #           print()
    ...
    printDebugList(repDebugList)

    print()

    return debugList
def printDebugList(debugList):
    for d in debugList:
...
def printParsedLog(d):
  ...
  #  val = d['string'] if d['type'] == 'USER_INFO' else val
   ...
   # _slevel = f"{level}"
#    if level == 0 and _context['prevTimes'][_slevel][1]==0:
#        _elapsedPrevD = d['timeStamp'][0]
   ...
   # prev = _context['prevTimes'][level][1] if level in _context['prevTimes'] else _context['prevTimes'][f"{level-1}"][0]
   ...
   # _context['prevTimes'][level] = [d['timeStamp'][0],_timeStamp1]
    ...
    if 1==2:
        ...
        print(f"{i:15} {ms(d['elapsedTime']):10}|{_elapsedPrevD:8}|{d['timeStamp'][0]:12}|{_timeStamp1:12}|{ms(delta(d,'timeStamp')):10}|{_totalQueriesTrace:15}|{_totalQueriesD:4}|{_totalQueries1:4}|{_plimit:1}|{_cpuPrevD:6}|{_cpuTime0:6}|{_cpuTime1:6}|{_sql2:3}|{_sqlcmt2:3}|{_typeColor}{_type:12}{CEND}|{_exp:1}|{_apexline:>4}|{val[:150]:50}"+CEND)
    else:
        if _context['firstLineOut'] == True:
            print(f"{'time(ms)':10}|{'elapsed':10}|{'time1(ns)':12}|{'time2(ns)':12}|{'t2-t1':10}|{'Qd':4}|{'Qt':4}|{'L':1}|{'cpuD':6}|{'CPUin':6}|{'Q':3}|{'Qcm':3}|{'type':12}|{'E':1}|{'al':4}{'':50}")
            _context['firstLineOut'] = False

        print(f"{ms(d['elapsedTime']):10}|{_elapsedPrevD:8}|{d['timeStamp'][0]:12}|{_timeStamp1:12}|{ms(delta(d,'timeStamp')):10}|{_totalQueriesD:4}|{_totalQueries1:4}|{_plimit:1}|{_cpuPrevD:6}|{_cpuTime0:6}|{_sql2:3}|{_sqlcmt2:3}|{_typeColor}{_type:12}{CEND}|{_exp:1}|{_apexline:>4}|{val[:150]:50}"+CEND)
