...
openItemsList = []
def append_and_increaseIdent(obj,increase=True):
    openItemsList.append(obj)
    if increase == True:
        increaseIdent()
    _context['debugList'].append(obj)

def decreaseIdent_pop_setFields(type,value,key='key',endsWith=None,decrease=True):
    if decrease == True:
        decreaseIdent()
    obj = popFromList(type=type,key=key,value=value,endsWith=endsWith)
    setTimes(_context['line'],obj)
    return obj

def parseUserInfo():
    if '|USER_INFO|' in _context['line']:
        obj = setTimes(_context['line'],field='output',value=_context['chunks'][4],type='USER_INFO')
        openItemsList.append(obj)

def parseSOQL():
    line = _context['line']
    chunks = _context['chunks']
    ...
    if '|SOQL_EXECUTE_BEGIN|' in line:
        ...
        obj['apexline'] = chunks[2][1:-1]
        append_and_increaseIdent(obj,increase=False)

    if '|SOQL_EXECUTE_END|' in line:
        _context['totalQueries'] = _context['totalQueries'] + 1
        obj = decreaseIdent_pop_setFields(type="SOQL",key='type',value='SOQL',decrease=False)
...
def parseDML():
    ...
    if '|DML_BEGIN|' in line:
        ...
        obj['apexline'] = chunks[2][1:-1]
        obj['output'] = f"{obj['OP']} {obj['Type']}" 
        append_and_increaseIdent(obj)

    if '|DML_END|' in line:
        decreaseIdent_pop_setFields('DML',key='Id',value=chunks[2])
...
def parseConstructor():
    ...
    if '|CONSTRUCTOR_EXIT|' in line:
        decreaseIdent_pop_setFields(type='CONSTRUCTOR',key='output',value=chunks[5])
...
def parseCodeUnit():
    ...
    if '|CODE_UNIT_FINISHED|' in line:
        decreaseIdent_pop_setFields('CODE_UNIT',key='output',value=chunks[2])
...
def parseFlow():
    ...
    if '|FLOW_START_INTERVIEWS_ENDxx|' in line:
        decreaseIdent_pop_setFields('FLOW_START_INTERVIEWS',key='output',value='FLOW_START_INTERVIEWS')
    ...
    if '|FLOW_START_INTERVIEW_END|' in line:
        decreaseIdent_pop_setFields('FLOW_START_INTERVIEW',value=chunks[2])
...
def popFromList(type,value,key='key',endsWith=False):
    try:
