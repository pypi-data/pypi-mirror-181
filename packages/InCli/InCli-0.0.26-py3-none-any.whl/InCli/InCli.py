...
def option_cc(args):
    ...
    if '-h' in args:
        help = f"""
        {utils.CGREEN}-cc{utils.CEND} 
            Checks the catalogs. Gets all catalogs, does a getOffers, getOfferDetails, basketwithoutconfig and basket with config
            {utils.CGREEN}-code{utils.CEND} - Catalogue Code. does it for a single catalog. 
            {utils.CGREEN}-list{utils.CEND} - List all the catalogues. 
            """ 
        return help
...
def option_o(args):
    if '-h' in args:
        ...
        {utils.CGREEN}-o{utils.CEND} --> List all Objects
            {utils.CGREEN}-name objectName{utils.CEND}  --> get one row from the object
            {utils.CGREEN}-name objectName:Id{utils.CEND} --> get the row especified by the Id
...
def option_logs(args):
    ...
    connectionInit(args)
    debugLogs.parseLog(logId,lastN=last,level=level,whereClause=whereClause,writeToFile=toFile)
