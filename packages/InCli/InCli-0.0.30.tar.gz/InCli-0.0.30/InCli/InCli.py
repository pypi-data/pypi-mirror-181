...
def option_logs(args):
    help = f"""
        ...
        {utils.CYELLOW}-logs -tail {utils.CEND}--> activelly parsers new debug logs as they are created.
            {utils.CGREEN}-logs -u OrgAlias -where "LogLength>3000"  -tail{utils.CEND} -> parses log files of size > 3000 as they are created.
    ...
    if inputfile == None:
        connectionInit(args)
    debugLogs.parseLog(logId=logId,lastN=last,level=level,whereClause=whereClause,writeToFile=toFile,filepath=inputfile,tail=tail)
