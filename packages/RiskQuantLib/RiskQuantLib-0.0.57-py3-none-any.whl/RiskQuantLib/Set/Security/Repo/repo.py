...
class setRepo(setBase):
    ...
    def setMaturityValue(self,maturityValueNum):
        from RiskQuantLib.Property.MarketValue.marketValue import marketValue
        if not hasattr(self,'__maturityValue'):
            self.__maturityValue = marketValue(maturityValueNum)
            self.maturityValue = self.__maturityValue.value
        else:
            self.__maturityValue.setValue(maturityValueNum)
            self.maturityValue = self.__maturityValue.value

    def setRepoRate(self,repoRateNum):
        from RiskQuantLib.Property.InterestRate.interestRate import interestRate
        if not hasattr(self,'__repoRate'):
            self.__repoRate = interestRate(repoRateNum)
            self.repoRate = self.__repoRate.value
        else:
            self.__repoRate.setValue(repoRateNum)
            self.repoRate = self.__repoRate.value

    def setMaturityDate(self,maturityDate):
        self.maturityDate = maturityDate

    def setIsReverseRepo(self,isReverseRepoBool = False):
        self.isReverseRepo = isReverseRepoBool
