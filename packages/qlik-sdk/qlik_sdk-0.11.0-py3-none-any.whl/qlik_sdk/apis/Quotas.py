    ...
    def __init__(self_, **kvargs):
        ...
        if "data" in kvargs and kvargs["data"] is not None:
            if (
                len(kvargs["data"]) > 0
                and f"list[{type(kvargs['data'][0]).__name__}]"
                == GetQuotasResult.__annotations__["data"]
            ):
                self_.data = kvargs["data"]
            else:
                self_.data = [Quota(**e) for e in kvargs["data"]]
...
class Quota:
    ...
    def __init__(self_, **kvargs):
        ...
        if "id" in kvargs and kvargs["id"] is not None:
            if type(kvargs["id"]).__name__ == Quota.__annotations__["id"]:
                self_.id = kvargs["id"]
            else:
                self_.id = kvargs["id"]
        if "type" in kvargs and kvargs["type"] is not None:
            if type(kvargs["type"]).__name__ == Quota.__annotations__["type"]:
...
class QuotaAttributes:
    ...
    def __init__(self_, **kvargs):
        ...
        if "quota" in kvargs and kvargs["quota"] is not None:
            if (
                type(kvargs["quota"]).__name__
                == QuotaAttributes.__annotations__["quota"]
            ):
                self_.quota = kvargs["quota"]
            else:
                self_.quota = kvargs["quota"]
        if "unit" in kvargs and kvargs["unit"] is not None:
            if type(kvargs["unit"]).__name__ == QuotaAttributes.__annotations__["unit"]:
                self_.unit = kvargs["unit"]
            else:
                self_.unit = kvargs["unit"]
        if "usage" in kvargs and kvargs["usage"] is not None:
            if (
                type(kvargs["usage"]).__name__
                == QuotaAttributes.__annotations__["usage"]
            ):
        ...
        if "warningThresholds" in kvargs and kvargs["warningThresholds"] is not None:
            if (
                type(kvargs["warningThresholds"]).__name__
                == QuotaAttributes.__annotations__["warningThresholds"]
            ):
