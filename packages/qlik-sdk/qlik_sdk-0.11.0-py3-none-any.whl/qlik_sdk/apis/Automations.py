# This is spectacularly generated code by spectacular v0.8.0 based on
# Qlik Cloud Services 0.529.8

from __future__ import annotations

from dataclasses import asdict, dataclass

from ..auth import Auth, Config
from ..listable import ListableResource


@dataclass
class AutomationDetailResponseObject:
    """

    Attributes
    ----------
    connectorIds: list[str]
      A list of connectors used inside the workspace.
    createdAt: str
    description: str
    endpointIds: list[str]
      A list of endpoints used inside the workspace.
    executionToken: str
    id: str
    lastRun: RunDetailResponseObject
    lastRunAt: str
    lastRunStatus: str
    name: str
    ownerId: str
    runMode: str
    schedules: list[ScheduleResponseObject]
    snippetIds: list[str]
      A list of snippets used inside the workspace.
    state: str
    updatedAt: str
    workspace: object
      The workspace generated by the Qlik Application Automations editor.
    """

    connectorIds: list[str] = None
    createdAt: str = None
    description: str = None
    endpointIds: list[str] = None
    executionToken: str = None
    id: str = None
    lastRun: RunDetailResponseObject = None
    lastRunAt: str = None
    lastRunStatus: str = None
    name: str = None
    ownerId: str = None
    runMode: str = None
    schedules: list[ScheduleResponseObject] = None
    snippetIds: list[str] = None
    state: str = "available"
    updatedAt: str = None
    workspace: object = None

    def __init__(self_, **kvargs):

        if "connectorIds" in kvargs and kvargs["connectorIds"] is not None:
            if (
                type(kvargs["connectorIds"]).__name__
                == AutomationDetailResponseObject.__annotations__["connectorIds"]
            ):
                self_.connectorIds = kvargs["connectorIds"]
            else:
                self_.connectorIds = kvargs["connectorIds"]
        if "createdAt" in kvargs and kvargs["createdAt"] is not None:
            if (
                type(kvargs["createdAt"]).__name__
                == AutomationDetailResponseObject.__annotations__["createdAt"]
            ):
                self_.createdAt = kvargs["createdAt"]
            else:
                self_.createdAt = kvargs["createdAt"]
        if "description" in kvargs and kvargs["description"] is not None:
            if (
                type(kvargs["description"]).__name__
                == AutomationDetailResponseObject.__annotations__["description"]
            ):
                self_.description = kvargs["description"]
            else:
                self_.description = kvargs["description"]
        if "endpointIds" in kvargs and kvargs["endpointIds"] is not None:
            if (
                type(kvargs["endpointIds"]).__name__
                == AutomationDetailResponseObject.__annotations__["endpointIds"]
            ):
                self_.endpointIds = kvargs["endpointIds"]
            else:
                self_.endpointIds = kvargs["endpointIds"]
        if "executionToken" in kvargs and kvargs["executionToken"] is not None:
            if (
                type(kvargs["executionToken"]).__name__
                == AutomationDetailResponseObject.__annotations__["executionToken"]
            ):
                self_.executionToken = kvargs["executionToken"]
            else:
                self_.executionToken = kvargs["executionToken"]
        if "id" in kvargs and kvargs["id"] is not None:
            if (
                type(kvargs["id"]).__name__
                == AutomationDetailResponseObject.__annotations__["id"]
            ):
                self_.id = kvargs["id"]
            else:
                self_.id = kvargs["id"]
        if "lastRun" in kvargs and kvargs["lastRun"] is not None:
            if (
                type(kvargs["lastRun"]).__name__
                == AutomationDetailResponseObject.__annotations__["lastRun"]
            ):
                self_.lastRun = kvargs["lastRun"]
            else:
                self_.lastRun = RunDetailResponseObject(**kvargs["lastRun"])
        if "lastRunAt" in kvargs and kvargs["lastRunAt"] is not None:
            if (
                type(kvargs["lastRunAt"]).__name__
                == AutomationDetailResponseObject.__annotations__["lastRunAt"]
            ):
                self_.lastRunAt = kvargs["lastRunAt"]
            else:
                self_.lastRunAt = kvargs["lastRunAt"]
        if "lastRunStatus" in kvargs and kvargs["lastRunStatus"] is not None:
            if (
                type(kvargs["lastRunStatus"]).__name__
                == AutomationDetailResponseObject.__annotations__["lastRunStatus"]
            ):
                self_.lastRunStatus = kvargs["lastRunStatus"]
            else:
                self_.lastRunStatus = kvargs["lastRunStatus"]
        if "name" in kvargs and kvargs["name"] is not None:
            if (
                type(kvargs["name"]).__name__
                == AutomationDetailResponseObject.__annotations__["name"]
            ):
                self_.name = kvargs["name"]
            else:
                self_.name = kvargs["name"]
        if "ownerId" in kvargs and kvargs["ownerId"] is not None:
            if (
                type(kvargs["ownerId"]).__name__
                == AutomationDetailResponseObject.__annotations__["ownerId"]
            ):
                self_.ownerId = kvargs["ownerId"]
            else:
                self_.ownerId = kvargs["ownerId"]
        if "runMode" in kvargs and kvargs["runMode"] is not None:
            if (
                type(kvargs["runMode"]).__name__
                == AutomationDetailResponseObject.__annotations__["runMode"]
            ):
                self_.runMode = kvargs["runMode"]
            else:
                self_.runMode = kvargs["runMode"]
        if "schedules" in kvargs and kvargs["schedules"] is not None:
            if (
                len(kvargs["schedules"]) > 0
                and f"list[{type(kvargs['schedules'][0]).__name__}]"
                == AutomationDetailResponseObject.__annotations__["schedules"]
            ):
                self_.schedules = kvargs["schedules"]
            else:
                self_.schedules = [
                    ScheduleResponseObject(**e) for e in kvargs["schedules"]
                ]
        if "snippetIds" in kvargs and kvargs["snippetIds"] is not None:
            if (
                type(kvargs["snippetIds"]).__name__
                == AutomationDetailResponseObject.__annotations__["snippetIds"]
            ):
                self_.snippetIds = kvargs["snippetIds"]
            else:
                self_.snippetIds = kvargs["snippetIds"]
        if "state" in kvargs and kvargs["state"] is not None:
            if (
                type(kvargs["state"]).__name__
                == AutomationDetailResponseObject.__annotations__["state"]
            ):
                self_.state = kvargs["state"]
            else:
                self_.state = kvargs["state"]
        if "updatedAt" in kvargs and kvargs["updatedAt"] is not None:
            if (
                type(kvargs["updatedAt"]).__name__
                == AutomationDetailResponseObject.__annotations__["updatedAt"]
            ):
                self_.updatedAt = kvargs["updatedAt"]
            else:
                self_.updatedAt = kvargs["updatedAt"]
        if "workspace" in kvargs and kvargs["workspace"] is not None:
            if (
                type(kvargs["workspace"]).__name__
                == AutomationDetailResponseObject.__annotations__["workspace"]
            ):
                self_.workspace = kvargs["workspace"]
            else:
                self_.workspace = kvargs["workspace"]
        for k0, v in kvargs.items():
            k = k0.replace("-", "_")
            if k not in getattr(self_, "__annotations__", {}):
                self_.__setattr__(k, v)

    def copy(self, name: str = None) -> object:
        """
        Copy an automation.

        Parameters
        ----------
        name: str = None
          Name of the new automation.
        """
        data = {"name": name}
        response = self.auth.rest(
            path="/automations/{id}/actions/copy".replace("{id}", self.id),
            method="POST",
            params={},
            data=data,
        )
        return response.json()

    def disable(self) -> None:
        """
        Disables an automation.

        Parameters
        ----------
        """
        self.auth.rest(
            path="/automations/{id}/actions/disable".replace("{id}", self.id),
            method="POST",
            params={},
            data=None,
        )

    def enable(self) -> None:
        """
        Enable an automation.

        Parameters
        ----------
        """
        self.auth.rest(
            path="/automations/{id}/actions/enable".replace("{id}", self.id),
            method="POST",
            params={},
            data=None,
        )

    def move(self, ownerId: str = None) -> None:
        """
        Move an automation to a different user.
        This step will remove the history and change logs of this automation. All linked connections used inside an automation will be detached.

        Parameters
        ----------
        ownerId: str = None

        """
        data = {"ownerId": ownerId}
        self.auth.rest(
            path="/automations/{id}/actions/move".replace("{id}", self.id),
            method="POST",
            params={},
            data=data,
        )

    def get_runs(
        self, filter: str = None, limit: int = 10, sort: str = "id", max_items: int = 10
    ) -> ListableResource[RunListObject]:
        """
        Retrieves a list of runs for a specific automation.

        Parameters
        ----------
        filter: str = None
          Allowed filters: status, context, startTime and title
        limit: int = 10
          The number of runs to retrieve.
        sort: str = "id"
          The field to sort by, with +- prefix indicating sort order. (?query=-startTime => sort on the startTime field using descending order)
        """
        query_params = {}
        if filter is not None:
            query_params["filter"] = filter
        if limit is not None:
            query_params["limit"] = limit
        if sort is not None:
            query_params["sort"] = sort
        response = self.auth.rest(
            path="/automations/{id}/runs".replace("{id}", self.id),
            method="GET",
            params=query_params,
            data=None,
        )
        return ListableResource(
            response=response.json(),
            cls=RunListObject,
            auth=self.auth,
            path="/automations/{id}/runs".replace("{id}", self.id),
            max_items=max_items,
            query_params=query_params,
        )

    def create_run(self, data: RunDetailRequestObject) -> RunDetailResponseObject:
        """
        Create a run

        Parameters
        ----------
        data: RunDetailRequestObject
          Run object to create
        """
        if data is not None:
            try:
                data = asdict(data)
            except:
                data = data
        response = self.auth.rest(
            path="/automations/{id}/runs".replace("{id}", self.id),
            method="POST",
            params={},
            data=data,
        )
        obj = RunDetailResponseObject(**response.json())
        obj.auth = self.auth
        return obj

    def delete(self) -> None:
        """
        Delete an automation

        Parameters
        ----------
        """
        self.auth.rest(
            path="/automations/{id}".replace("{id}", self.id),
            method="DELETE",
            params={},
            data=None,
        )

    def set(
        self, data: AutomationDetailRequestObject
    ) -> AutomationDetailResponseObject:
        """
        Update an automation

        Parameters
        ----------
        data: AutomationDetailRequestObject
          Automation object to update
        """
        if data is not None:
            try:
                data = asdict(data)
            except:
                data = data
        response = self.auth.rest(
            path="/automations/{id}".replace("{id}", self.id),
            method="PUT",
            params={},
            data=data,
        )
        self.__init__(**response.json())
        return self


@dataclass
class AutomationDetailRequestObject:
    """

    Attributes
    ----------
    description: str
    name: str
    schedules: list[ScheduleRequestObject]
    state: str
    workspace: object
      The workspace generated by the Qlik Application Automations editor.
    """

    description: str = None
    name: str = None
    schedules: list[ScheduleRequestObject] = None
    state: str = "available"
    workspace: object = None

    def __init__(self_, **kvargs):

        if "description" in kvargs and kvargs["description"] is not None:
            if (
                type(kvargs["description"]).__name__
                == AutomationDetailRequestObject.__annotations__["description"]
            ):
                self_.description = kvargs["description"]
            else:
                self_.description = kvargs["description"]
        if "name" in kvargs and kvargs["name"] is not None:
            if (
                type(kvargs["name"]).__name__
                == AutomationDetailRequestObject.__annotations__["name"]
            ):
                self_.name = kvargs["name"]
            else:
                self_.name = kvargs["name"]
        if "schedules" in kvargs and kvargs["schedules"] is not None:
            if (
                len(kvargs["schedules"]) > 0
                and f"list[{type(kvargs['schedules'][0]).__name__}]"
                == AutomationDetailRequestObject.__annotations__["schedules"]
            ):
                self_.schedules = kvargs["schedules"]
            else:
                self_.schedules = [
                    ScheduleRequestObject(**e) for e in kvargs["schedules"]
                ]
        if "state" in kvargs and kvargs["state"] is not None:
            if (
                type(kvargs["state"]).__name__
                == AutomationDetailRequestObject.__annotations__["state"]
            ):
                self_.state = kvargs["state"]
            else:
                self_.state = kvargs["state"]
        if "workspace" in kvargs and kvargs["workspace"] is not None:
            if (
                type(kvargs["workspace"]).__name__
                == AutomationDetailRequestObject.__annotations__["workspace"]
            ):
                self_.workspace = kvargs["workspace"]
            else:
                self_.workspace = kvargs["workspace"]
        for k0, v in kvargs.items():
            k = k0.replace("-", "_")
            if k not in getattr(self_, "__annotations__", {}):
                self_.__setattr__(k, v)


@dataclass
class AutomationList:
    """

    Attributes
    ----------
    data: list[AutomationListObject]
    links: Links
    """

    data: list[AutomationListObject] = None
    links: Links = None

    def __init__(self_, **kvargs):

        if "data" in kvargs and kvargs["data"] is not None:
            if (
                len(kvargs["data"]) > 0
                and f"list[{type(kvargs['data'][0]).__name__}]"
                == AutomationList.__annotations__["data"]
            ):
                self_.data = kvargs["data"]
            else:
                self_.data = [AutomationListObject(**e) for e in kvargs["data"]]
        if "links" in kvargs and kvargs["links"] is not None:
            if (
                type(kvargs["links"]).__name__
                == AutomationList.__annotations__["links"]
            ):
                self_.links = kvargs["links"]
            else:
                self_.links = Links(**kvargs["links"])
        for k0, v in kvargs.items():
            k = k0.replace("-", "_")
            if k not in getattr(self_, "__annotations__", {}):
                self_.__setattr__(k, v)


@dataclass
class AutomationListObject:
    """

    Attributes
    ----------
    createdAt: str
    description: str
    id: str
    lastRunAt: str
    lastRunStatus: str
    name: str
    ownerId: str
    runMode: str
    state: str
    updatedAt: str
    """

    createdAt: str = None
    description: str = None
    id: str = None
    lastRunAt: str = None
    lastRunStatus: str = None
    name: str = None
    ownerId: str = None
    runMode: str = None
    state: str = "available"
    updatedAt: str = None

    def __init__(self_, **kvargs):

        if "createdAt" in kvargs and kvargs["createdAt"] is not None:
            if (
                type(kvargs["createdAt"]).__name__
                == AutomationListObject.__annotations__["createdAt"]
            ):
                self_.createdAt = kvargs["createdAt"]
            else:
                self_.createdAt = kvargs["createdAt"]
        if "description" in kvargs and kvargs["description"] is not None:
            if (
                type(kvargs["description"]).__name__
                == AutomationListObject.__annotations__["description"]
            ):
                self_.description = kvargs["description"]
            else:
                self_.description = kvargs["description"]
        if "id" in kvargs and kvargs["id"] is not None:
            if (
                type(kvargs["id"]).__name__
                == AutomationListObject.__annotations__["id"]
            ):
                self_.id = kvargs["id"]
            else:
                self_.id = kvargs["id"]
        if "lastRunAt" in kvargs and kvargs["lastRunAt"] is not None:
            if (
                type(kvargs["lastRunAt"]).__name__
                == AutomationListObject.__annotations__["lastRunAt"]
            ):
                self_.lastRunAt = kvargs["lastRunAt"]
            else:
                self_.lastRunAt = kvargs["lastRunAt"]
        if "lastRunStatus" in kvargs and kvargs["lastRunStatus"] is not None:
            if (
                type(kvargs["lastRunStatus"]).__name__
                == AutomationListObject.__annotations__["lastRunStatus"]
            ):
                self_.lastRunStatus = kvargs["lastRunStatus"]
            else:
                self_.lastRunStatus = kvargs["lastRunStatus"]
        if "name" in kvargs and kvargs["name"] is not None:
            if (
                type(kvargs["name"]).__name__
                == AutomationListObject.__annotations__["name"]
            ):
                self_.name = kvargs["name"]
            else:
                self_.name = kvargs["name"]
        if "ownerId" in kvargs and kvargs["ownerId"] is not None:
            if (
                type(kvargs["ownerId"]).__name__
                == AutomationListObject.__annotations__["ownerId"]
            ):
                self_.ownerId = kvargs["ownerId"]
            else:
                self_.ownerId = kvargs["ownerId"]
        if "runMode" in kvargs and kvargs["runMode"] is not None:
            if (
                type(kvargs["runMode"]).__name__
                == AutomationListObject.__annotations__["runMode"]
            ):
                self_.runMode = kvargs["runMode"]
            else:
                self_.runMode = kvargs["runMode"]
        if "state" in kvargs and kvargs["state"] is not None:
            if (
                type(kvargs["state"]).__name__
                == AutomationListObject.__annotations__["state"]
            ):
                self_.state = kvargs["state"]
            else:
                self_.state = kvargs["state"]
        if "updatedAt" in kvargs and kvargs["updatedAt"] is not None:
            if (
                type(kvargs["updatedAt"]).__name__
                == AutomationListObject.__annotations__["updatedAt"]
            ):
                self_.updatedAt = kvargs["updatedAt"]
            else:
                self_.updatedAt = kvargs["updatedAt"]
        for k0, v in kvargs.items():
            k = k0.replace("-", "_")
            if k not in getattr(self_, "__annotations__", {}):
                self_.__setattr__(k, v)


@dataclass
class AutomationUsageObject:
    """

    Attributes
    ----------
    guid: str
      The unique identifier for the automation.
    name: str
      The name for the automation.
    ownerId: str
      The unique identifier for the user who owns the automation
    """

    guid: str = None
    name: str = None
    ownerId: str = None

    def __init__(self_, **kvargs):

        if "guid" in kvargs and kvargs["guid"] is not None:
            if (
                type(kvargs["guid"]).__name__
                == AutomationUsageObject.__annotations__["guid"]
            ):
                self_.guid = kvargs["guid"]
            else:
                self_.guid = kvargs["guid"]
        if "name" in kvargs and kvargs["name"] is not None:
            if (
                type(kvargs["name"]).__name__
                == AutomationUsageObject.__annotations__["name"]
            ):
                self_.name = kvargs["name"]
            else:
                self_.name = kvargs["name"]
        if "ownerId" in kvargs and kvargs["ownerId"] is not None:
            if (
                type(kvargs["ownerId"]).__name__
                == AutomationUsageObject.__annotations__["ownerId"]
            ):
                self_.ownerId = kvargs["ownerId"]
            else:
                self_.ownerId = kvargs["ownerId"]
        for k0, v in kvargs.items():
            k = k0.replace("-", "_")
            if k not in getattr(self_, "__annotations__", {}):
                self_.__setattr__(k, v)


@dataclass
class Links:
    """

    Attributes
    ----------
    next: PaginationLink
    prev: PaginationLink
    """

    next: PaginationLink = None
    prev: PaginationLink = None

    def __init__(self_, **kvargs):

        if "next" in kvargs and kvargs["next"] is not None:
            if type(kvargs["next"]).__name__ == Links.__annotations__["next"]:
                self_.next = kvargs["next"]
            else:
                self_.next = PaginationLink(**kvargs["next"])
        if "prev" in kvargs and kvargs["prev"] is not None:
            if type(kvargs["prev"]).__name__ == Links.__annotations__["prev"]:
                self_.prev = kvargs["prev"]
            else:
                self_.prev = PaginationLink(**kvargs["prev"])
        for k0, v in kvargs.items():
            k = k0.replace("-", "_")
            if k not in getattr(self_, "__annotations__", {}):
                self_.__setattr__(k, v)


@dataclass
class PaginationLink:
    """

    Attributes
    ----------
    href: str
      The URL to a resource request
    """

    href: str = None

    def __init__(self_, **kvargs):

        if "href" in kvargs and kvargs["href"] is not None:
            if type(kvargs["href"]).__name__ == PaginationLink.__annotations__["href"]:
                self_.href = kvargs["href"]
            else:
                self_.href = kvargs["href"]
        for k0, v in kvargs.items():
            k = k0.replace("-", "_")
            if k not in getattr(self_, "__annotations__", {}):
                self_.__setattr__(k, v)


@dataclass
class RunDetailRequestObject:
    """

    Attributes
    ----------
    context: str
      The source that triggers the automation will set the context.
    id: str
    inputs: object
    """

    context: str = None
    id: str = None
    inputs: object = None

    def __init__(self_, **kvargs):

        if "context" in kvargs and kvargs["context"] is not None:
            if (
                type(kvargs["context"]).__name__
                == RunDetailRequestObject.__annotations__["context"]
            ):
                self_.context = kvargs["context"]
            else:
                self_.context = kvargs["context"]
        if "id" in kvargs and kvargs["id"] is not None:
            if (
                type(kvargs["id"]).__name__
                == RunDetailRequestObject.__annotations__["id"]
            ):
                self_.id = kvargs["id"]
            else:
                self_.id = kvargs["id"]
        if "inputs" in kvargs and kvargs["inputs"] is not None:
            if (
                type(kvargs["inputs"]).__name__
                == RunDetailRequestObject.__annotations__["inputs"]
            ):
                self_.inputs = kvargs["inputs"]
            else:
                self_.inputs = kvargs["inputs"]
        for k0, v in kvargs.items():
            k = k0.replace("-", "_")
            if k not in getattr(self_, "__annotations__", {}):
                self_.__setattr__(k, v)


@dataclass
class RunDetailResponseObject:
    """

    Attributes
    ----------
    context: str
      The source that triggers the automation will set the context. Certain contexts impact the execution of an automation (for example, The "test_run" context will not process all results when listing items).
    createdAt: str
    error: object
    expectedInputs: object
    id: str
    inputs: object
    isArchived: bool
    isTestRun: bool
    metrics: RunDetailResponseObjectMetrics
    scheduledStartTime: str
    startTime: str
    status: str
    stopTime: str
    title: str
    updatedAt: str
    """

    context: str = None
    createdAt: str = None
    error: object = None
    expectedInputs: object = None
    id: str = None
    inputs: object = None
    isArchived: bool = None
    isTestRun: bool = None
    metrics: RunDetailResponseObjectMetrics = None
    scheduledStartTime: str = None
    startTime: str = None
    status: str = None
    stopTime: str = None
    title: str = None
    updatedAt: str = None

    def __init__(self_, **kvargs):

        if "context" in kvargs and kvargs["context"] is not None:
            if (
                type(kvargs["context"]).__name__
                == RunDetailResponseObject.__annotations__["context"]
            ):
                self_.context = kvargs["context"]
            else:
                self_.context = kvargs["context"]
        if "createdAt" in kvargs and kvargs["createdAt"] is not None:
            if (
                type(kvargs["createdAt"]).__name__
                == RunDetailResponseObject.__annotations__["createdAt"]
            ):
                self_.createdAt = kvargs["createdAt"]
            else:
                self_.createdAt = kvargs["createdAt"]
        if "error" in kvargs and kvargs["error"] is not None:
            if (
                type(kvargs["error"]).__name__
                == RunDetailResponseObject.__annotations__["error"]
            ):
                self_.error = kvargs["error"]
            else:
                self_.error = kvargs["error"]
        if "expectedInputs" in kvargs and kvargs["expectedInputs"] is not None:
            if (
                type(kvargs["expectedInputs"]).__name__
                == RunDetailResponseObject.__annotations__["expectedInputs"]
            ):
                self_.expectedInputs = kvargs["expectedInputs"]
            else:
                self_.expectedInputs = kvargs["expectedInputs"]
        if "id" in kvargs and kvargs["id"] is not None:
            if (
                type(kvargs["id"]).__name__
                == RunDetailResponseObject.__annotations__["id"]
            ):
                self_.id = kvargs["id"]
            else:
                self_.id = kvargs["id"]
        if "inputs" in kvargs and kvargs["inputs"] is not None:
            if (
                type(kvargs["inputs"]).__name__
                == RunDetailResponseObject.__annotations__["inputs"]
            ):
                self_.inputs = kvargs["inputs"]
            else:
                self_.inputs = kvargs["inputs"]
        if "isArchived" in kvargs and kvargs["isArchived"] is not None:
            if (
                type(kvargs["isArchived"]).__name__
                == RunDetailResponseObject.__annotations__["isArchived"]
            ):
                self_.isArchived = kvargs["isArchived"]
            else:
                self_.isArchived = kvargs["isArchived"]
        if "isTestRun" in kvargs and kvargs["isTestRun"] is not None:
            if (
                type(kvargs["isTestRun"]).__name__
                == RunDetailResponseObject.__annotations__["isTestRun"]
            ):
                self_.isTestRun = kvargs["isTestRun"]
            else:
                self_.isTestRun = kvargs["isTestRun"]
        if "metrics" in kvargs and kvargs["metrics"] is not None:
            if (
                type(kvargs["metrics"]).__name__
                == RunDetailResponseObject.__annotations__["metrics"]
            ):
                self_.metrics = kvargs["metrics"]
            else:
                self_.metrics = RunDetailResponseObjectMetrics(**kvargs["metrics"])
        if "scheduledStartTime" in kvargs and kvargs["scheduledStartTime"] is not None:
            if (
                type(kvargs["scheduledStartTime"]).__name__
                == RunDetailResponseObject.__annotations__["scheduledStartTime"]
            ):
                self_.scheduledStartTime = kvargs["scheduledStartTime"]
            else:
                self_.scheduledStartTime = kvargs["scheduledStartTime"]
        if "startTime" in kvargs and kvargs["startTime"] is not None:
            if (
                type(kvargs["startTime"]).__name__
                == RunDetailResponseObject.__annotations__["startTime"]
            ):
                self_.startTime = kvargs["startTime"]
            else:
                self_.startTime = kvargs["startTime"]
        if "status" in kvargs and kvargs["status"] is not None:
            if (
                type(kvargs["status"]).__name__
                == RunDetailResponseObject.__annotations__["status"]
            ):
                self_.status = kvargs["status"]
            else:
                self_.status = kvargs["status"]
        if "stopTime" in kvargs and kvargs["stopTime"] is not None:
            if (
                type(kvargs["stopTime"]).__name__
                == RunDetailResponseObject.__annotations__["stopTime"]
            ):
                self_.stopTime = kvargs["stopTime"]
            else:
                self_.stopTime = kvargs["stopTime"]
        if "title" in kvargs and kvargs["title"] is not None:
            if (
                type(kvargs["title"]).__name__
                == RunDetailResponseObject.__annotations__["title"]
            ):
                self_.title = kvargs["title"]
            else:
                self_.title = kvargs["title"]
        if "updatedAt" in kvargs and kvargs["updatedAt"] is not None:
            if (
                type(kvargs["updatedAt"]).__name__
                == RunDetailResponseObject.__annotations__["updatedAt"]
            ):
                self_.updatedAt = kvargs["updatedAt"]
            else:
                self_.updatedAt = kvargs["updatedAt"]
        for k0, v in kvargs.items():
            k = k0.replace("-", "_")
            if k not in getattr(self_, "__annotations__", {}):
                self_.__setattr__(k, v)


@dataclass
class RunDetailResponseObjectMetrics:
    """

    Attributes
    ----------
    blocks: list[RunDetailResponseObjectMetricsBlocks]
      List of blocks used during execution. Note: this list currently only contains endpointBlocks and snippetBlocks
    network: RunDetailResponseObjectMetricsNetwork
    totalApiCalls: int
      The number of API calls made.
    """

    blocks: list[RunDetailResponseObjectMetricsBlocks] = None
    network: RunDetailResponseObjectMetricsNetwork = None
    totalApiCalls: int = None

    def __init__(self_, **kvargs):

        if "blocks" in kvargs and kvargs["blocks"] is not None:
            if (
                len(kvargs["blocks"]) > 0
                and f"list[{type(kvargs['blocks'][0]).__name__}]"
                == RunDetailResponseObjectMetrics.__annotations__["blocks"]
            ):
                self_.blocks = kvargs["blocks"]
            else:
                self_.blocks = [
                    RunDetailResponseObjectMetricsBlocks(**e) for e in kvargs["blocks"]
                ]
        if "network" in kvargs and kvargs["network"] is not None:
            if (
                type(kvargs["network"]).__name__
                == RunDetailResponseObjectMetrics.__annotations__["network"]
            ):
                self_.network = kvargs["network"]
            else:
                self_.network = RunDetailResponseObjectMetricsNetwork(
                    **kvargs["network"]
                )
        if "totalApiCalls" in kvargs and kvargs["totalApiCalls"] is not None:
            if (
                type(kvargs["totalApiCalls"]).__name__
                == RunDetailResponseObjectMetrics.__annotations__["totalApiCalls"]
            ):
                self_.totalApiCalls = kvargs["totalApiCalls"]
            else:
                self_.totalApiCalls = kvargs["totalApiCalls"]
        for k0, v in kvargs.items():
            k = k0.replace("-", "_")
            if k not in getattr(self_, "__annotations__", {}):
                self_.__setattr__(k, v)


@dataclass
class RunDetailResponseObjectMetricsBlocks:
    """
    Information of the ended execution per block.

    Attributes
    ----------
    apiCalls: int
      API calls to external resources made by the current block.
    connectorId: str
      Unique identifier for the connector used within the block of the execution.
    endpointId: str
      Unique identifier for the snippet block used within the execution.
    rxBytes: int
      Total amount of received bytes sent by the current block.
    snippetId: str
      Unique identifier for the snippet block used within the execution.
    txBytes: int
      Total amount of sent bytes sent by the current block.
    type: str
      Type of the block.
    """

    apiCalls: int = None
    connectorId: str = None
    endpointId: str = None
    rxBytes: int = None
    snippetId: str = None
    txBytes: int = None
    type: str = None

    def __init__(self_, **kvargs):

        if "apiCalls" in kvargs and kvargs["apiCalls"] is not None:
            if (
                type(kvargs["apiCalls"]).__name__
                == RunDetailResponseObjectMetricsBlocks.__annotations__["apiCalls"]
            ):
                self_.apiCalls = kvargs["apiCalls"]
            else:
                self_.apiCalls = kvargs["apiCalls"]
        if "connectorId" in kvargs and kvargs["connectorId"] is not None:
            if (
                type(kvargs["connectorId"]).__name__
                == RunDetailResponseObjectMetricsBlocks.__annotations__["connectorId"]
            ):
                self_.connectorId = kvargs["connectorId"]
            else:
                self_.connectorId = kvargs["connectorId"]
        if "endpointId" in kvargs and kvargs["endpointId"] is not None:
            if (
                type(kvargs["endpointId"]).__name__
                == RunDetailResponseObjectMetricsBlocks.__annotations__["endpointId"]
            ):
                self_.endpointId = kvargs["endpointId"]
            else:
                self_.endpointId = kvargs["endpointId"]
        if "rxBytes" in kvargs and kvargs["rxBytes"] is not None:
            if (
                type(kvargs["rxBytes"]).__name__
                == RunDetailResponseObjectMetricsBlocks.__annotations__["rxBytes"]
            ):
                self_.rxBytes = kvargs["rxBytes"]
            else:
                self_.rxBytes = kvargs["rxBytes"]
        if "snippetId" in kvargs and kvargs["snippetId"] is not None:
            if (
                type(kvargs["snippetId"]).__name__
                == RunDetailResponseObjectMetricsBlocks.__annotations__["snippetId"]
            ):
                self_.snippetId = kvargs["snippetId"]
            else:
                self_.snippetId = kvargs["snippetId"]
        if "txBytes" in kvargs and kvargs["txBytes"] is not None:
            if (
                type(kvargs["txBytes"]).__name__
                == RunDetailResponseObjectMetricsBlocks.__annotations__["txBytes"]
            ):
                self_.txBytes = kvargs["txBytes"]
            else:
                self_.txBytes = kvargs["txBytes"]
        if "type" in kvargs and kvargs["type"] is not None:
            if (
                type(kvargs["type"]).__name__
                == RunDetailResponseObjectMetricsBlocks.__annotations__["type"]
            ):
                self_.type = kvargs["type"]
            else:
                self_.type = kvargs["type"]
        for k0, v in kvargs.items():
            k = k0.replace("-", "_")
            if k not in getattr(self_, "__annotations__", {}):
                self_.__setattr__(k, v)


@dataclass
class RunDetailResponseObjectMetricsNetwork:
    """

    Attributes
    ----------
    rxBytes: int
      The number of received bytes.
    txBytes: int
      The number of received bytes.
    """

    rxBytes: int = None
    txBytes: int = None

    def __init__(self_, **kvargs):

        if "rxBytes" in kvargs and kvargs["rxBytes"] is not None:
            if (
                type(kvargs["rxBytes"]).__name__
                == RunDetailResponseObjectMetricsNetwork.__annotations__["rxBytes"]
            ):
                self_.rxBytes = kvargs["rxBytes"]
            else:
                self_.rxBytes = kvargs["rxBytes"]
        if "txBytes" in kvargs and kvargs["txBytes"] is not None:
            if (
                type(kvargs["txBytes"]).__name__
                == RunDetailResponseObjectMetricsNetwork.__annotations__["txBytes"]
            ):
                self_.txBytes = kvargs["txBytes"]
            else:
                self_.txBytes = kvargs["txBytes"]
        for k0, v in kvargs.items():
            k = k0.replace("-", "_")
            if k not in getattr(self_, "__annotations__", {}):
                self_.__setattr__(k, v)


@dataclass
class RunList:
    """

    Attributes
    ----------
    data: list[RunListObject]
    links: Links
    """

    data: list[RunListObject] = None
    links: Links = None

    def __init__(self_, **kvargs):

        if "data" in kvargs and kvargs["data"] is not None:
            if (
                len(kvargs["data"]) > 0
                and f"list[{type(kvargs['data'][0]).__name__}]"
                == RunList.__annotations__["data"]
            ):
                self_.data = kvargs["data"]
            else:
                self_.data = [RunListObject(**e) for e in kvargs["data"]]
        if "links" in kvargs and kvargs["links"] is not None:
            if type(kvargs["links"]).__name__ == RunList.__annotations__["links"]:
                self_.links = kvargs["links"]
            else:
                self_.links = Links(**kvargs["links"])
        for k0, v in kvargs.items():
            k = k0.replace("-", "_")
            if k not in getattr(self_, "__annotations__", {}):
                self_.__setattr__(k, v)


@dataclass
class RunListObject:
    """

    Attributes
    ----------
    context: str
    createdAt: str
    error: object
    id: str
    isArchived: bool
    isTestRun: bool
    scheduledStartTime: str
    startTime: str
    status: str
    stopTime: str
    title: str
    updatedAt: str
    """

    context: str = None
    createdAt: str = None
    error: object = None
    id: str = None
    isArchived: bool = None
    isTestRun: bool = None
    scheduledStartTime: str = None
    startTime: str = None
    status: str = None
    stopTime: str = None
    title: str = None
    updatedAt: str = None

    def __init__(self_, **kvargs):

        if "context" in kvargs and kvargs["context"] is not None:
            if (
                type(kvargs["context"]).__name__
                == RunListObject.__annotations__["context"]
            ):
                self_.context = kvargs["context"]
            else:
                self_.context = kvargs["context"]
        if "createdAt" in kvargs and kvargs["createdAt"] is not None:
            if (
                type(kvargs["createdAt"]).__name__
                == RunListObject.__annotations__["createdAt"]
            ):
                self_.createdAt = kvargs["createdAt"]
            else:
                self_.createdAt = kvargs["createdAt"]
        if "error" in kvargs and kvargs["error"] is not None:
            if type(kvargs["error"]).__name__ == RunListObject.__annotations__["error"]:
                self_.error = kvargs["error"]
            else:
                self_.error = kvargs["error"]
        if "id" in kvargs and kvargs["id"] is not None:
            if type(kvargs["id"]).__name__ == RunListObject.__annotations__["id"]:
                self_.id = kvargs["id"]
            else:
                self_.id = kvargs["id"]
        if "isArchived" in kvargs and kvargs["isArchived"] is not None:
            if (
                type(kvargs["isArchived"]).__name__
                == RunListObject.__annotations__["isArchived"]
            ):
                self_.isArchived = kvargs["isArchived"]
            else:
                self_.isArchived = kvargs["isArchived"]
        if "isTestRun" in kvargs and kvargs["isTestRun"] is not None:
            if (
                type(kvargs["isTestRun"]).__name__
                == RunListObject.__annotations__["isTestRun"]
            ):
                self_.isTestRun = kvargs["isTestRun"]
            else:
                self_.isTestRun = kvargs["isTestRun"]
        if "scheduledStartTime" in kvargs and kvargs["scheduledStartTime"] is not None:
            if (
                type(kvargs["scheduledStartTime"]).__name__
                == RunListObject.__annotations__["scheduledStartTime"]
            ):
                self_.scheduledStartTime = kvargs["scheduledStartTime"]
            else:
                self_.scheduledStartTime = kvargs["scheduledStartTime"]
        if "startTime" in kvargs and kvargs["startTime"] is not None:
            if (
                type(kvargs["startTime"]).__name__
                == RunListObject.__annotations__["startTime"]
            ):
                self_.startTime = kvargs["startTime"]
            else:
                self_.startTime = kvargs["startTime"]
        if "status" in kvargs and kvargs["status"] is not None:
            if (
                type(kvargs["status"]).__name__
                == RunListObject.__annotations__["status"]
            ):
                self_.status = kvargs["status"]
            else:
                self_.status = kvargs["status"]
        if "stopTime" in kvargs and kvargs["stopTime"] is not None:
            if (
                type(kvargs["stopTime"]).__name__
                == RunListObject.__annotations__["stopTime"]
            ):
                self_.stopTime = kvargs["stopTime"]
            else:
                self_.stopTime = kvargs["stopTime"]
        if "title" in kvargs and kvargs["title"] is not None:
            if type(kvargs["title"]).__name__ == RunListObject.__annotations__["title"]:
                self_.title = kvargs["title"]
            else:
                self_.title = kvargs["title"]
        if "updatedAt" in kvargs and kvargs["updatedAt"] is not None:
            if (
                type(kvargs["updatedAt"]).__name__
                == RunListObject.__annotations__["updatedAt"]
            ):
                self_.updatedAt = kvargs["updatedAt"]
            else:
                self_.updatedAt = kvargs["updatedAt"]
        for k0, v in kvargs.items():
            k = k0.replace("-", "_")
            if k not in getattr(self_, "__annotations__", {}):
                self_.__setattr__(k, v)


@dataclass
class ScheduleRequestObject:
    """

    Attributes
    ----------
    interval: int
      time in seconds.
    startAt: str
    stopAt: str
    timezone: str
    """

    interval: int = None
    startAt: str = None
    stopAt: str = None
    timezone: str = None

    def __init__(self_, **kvargs):

        if "interval" in kvargs and kvargs["interval"] is not None:
            if (
                type(kvargs["interval"]).__name__
                == ScheduleRequestObject.__annotations__["interval"]
            ):
                self_.interval = kvargs["interval"]
            else:
                self_.interval = kvargs["interval"]
        if "startAt" in kvargs and kvargs["startAt"] is not None:
            if (
                type(kvargs["startAt"]).__name__
                == ScheduleRequestObject.__annotations__["startAt"]
            ):
                self_.startAt = kvargs["startAt"]
            else:
                self_.startAt = kvargs["startAt"]
        if "stopAt" in kvargs and kvargs["stopAt"] is not None:
            if (
                type(kvargs["stopAt"]).__name__
                == ScheduleRequestObject.__annotations__["stopAt"]
            ):
                self_.stopAt = kvargs["stopAt"]
            else:
                self_.stopAt = kvargs["stopAt"]
        if "timezone" in kvargs and kvargs["timezone"] is not None:
            if (
                type(kvargs["timezone"]).__name__
                == ScheduleRequestObject.__annotations__["timezone"]
            ):
                self_.timezone = kvargs["timezone"]
            else:
                self_.timezone = kvargs["timezone"]
        for k0, v in kvargs.items():
            k = k0.replace("-", "_")
            if k not in getattr(self_, "__annotations__", {}):
                self_.__setattr__(k, v)


@dataclass
class ScheduleResponseObject:
    """

    Attributes
    ----------
    id: str
    interval: int
      time in seconds.
    lastStartedAt: str
    startAt: str
    stopAt: str
    timezone: str
    """

    id: str = None
    interval: int = None
    lastStartedAt: str = None
    startAt: str = None
    stopAt: str = None
    timezone: str = None

    def __init__(self_, **kvargs):

        if "id" in kvargs and kvargs["id"] is not None:
            if (
                type(kvargs["id"]).__name__
                == ScheduleResponseObject.__annotations__["id"]
            ):
                self_.id = kvargs["id"]
            else:
                self_.id = kvargs["id"]
        if "interval" in kvargs and kvargs["interval"] is not None:
            if (
                type(kvargs["interval"]).__name__
                == ScheduleResponseObject.__annotations__["interval"]
            ):
                self_.interval = kvargs["interval"]
            else:
                self_.interval = kvargs["interval"]
        if "lastStartedAt" in kvargs and kvargs["lastStartedAt"] is not None:
            if (
                type(kvargs["lastStartedAt"]).__name__
                == ScheduleResponseObject.__annotations__["lastStartedAt"]
            ):
                self_.lastStartedAt = kvargs["lastStartedAt"]
            else:
                self_.lastStartedAt = kvargs["lastStartedAt"]
        if "startAt" in kvargs and kvargs["startAt"] is not None:
            if (
                type(kvargs["startAt"]).__name__
                == ScheduleResponseObject.__annotations__["startAt"]
            ):
                self_.startAt = kvargs["startAt"]
            else:
                self_.startAt = kvargs["startAt"]
        if "stopAt" in kvargs and kvargs["stopAt"] is not None:
            if (
                type(kvargs["stopAt"]).__name__
                == ScheduleResponseObject.__annotations__["stopAt"]
            ):
                self_.stopAt = kvargs["stopAt"]
            else:
                self_.stopAt = kvargs["stopAt"]
        if "timezone" in kvargs and kvargs["timezone"] is not None:
            if (
                type(kvargs["timezone"]).__name__
                == ScheduleResponseObject.__annotations__["timezone"]
            ):
                self_.timezone = kvargs["timezone"]
            else:
                self_.timezone = kvargs["timezone"]
        for k0, v in kvargs.items():
            k = k0.replace("-", "_")
            if k not in getattr(self_, "__annotations__", {}):
                self_.__setattr__(k, v)


@dataclass
class SettingsObject:
    """

    Attributes
    ----------
    automationsEnabled: bool
    """

    automationsEnabled: bool = None

    def __init__(self_, **kvargs):

        if "automationsEnabled" in kvargs and kvargs["automationsEnabled"] is not None:
            if (
                type(kvargs["automationsEnabled"]).__name__
                == SettingsObject.__annotations__["automationsEnabled"]
            ):
                self_.automationsEnabled = kvargs["automationsEnabled"]
            else:
                self_.automationsEnabled = kvargs["automationsEnabled"]
        for k0, v in kvargs.items():
            k = k0.replace("-", "_")
            if k not in getattr(self_, "__annotations__", {}):
                self_.__setattr__(k, v)


@dataclass
class UsageList:
    """

    Attributes
    ----------
    data: list[UsageObject]
    """

    data: list[UsageObject] = None

    def __init__(self_, **kvargs):

        if "data" in kvargs and kvargs["data"] is not None:
            if (
                len(kvargs["data"]) > 0
                and f"list[{type(kvargs['data'][0]).__name__}]"
                == UsageList.__annotations__["data"]
            ):
                self_.data = kvargs["data"]
            else:
                self_.data = [UsageObject(**e) for e in kvargs["data"]]
        for k0, v in kvargs.items():
            k = k0.replace("-", "_")
            if k not in getattr(self_, "__annotations__", {}):
                self_.__setattr__(k, v)


@dataclass
class UsageObject:
    """

    Attributes
    ----------
    automation: AutomationUsageObject
    date: str
      The date for the metric
    name: str
      The name for the metric
    value: int
      The value count of the metric
    """

    automation: AutomationUsageObject = None
    date: str = None
    name: str = None
    value: int = None

    def __init__(self_, **kvargs):

        if "automation" in kvargs and kvargs["automation"] is not None:
            if (
                type(kvargs["automation"]).__name__
                == UsageObject.__annotations__["automation"]
            ):
                self_.automation = kvargs["automation"]
            else:
                self_.automation = AutomationUsageObject(**kvargs["automation"])
        if "date" in kvargs and kvargs["date"] is not None:
            if type(kvargs["date"]).__name__ == UsageObject.__annotations__["date"]:
                self_.date = kvargs["date"]
            else:
                self_.date = kvargs["date"]
        if "name" in kvargs and kvargs["name"] is not None:
            if type(kvargs["name"]).__name__ == UsageObject.__annotations__["name"]:
                self_.name = kvargs["name"]
            else:
                self_.name = kvargs["name"]
        if "value" in kvargs and kvargs["value"] is not None:
            if type(kvargs["value"]).__name__ == UsageObject.__annotations__["value"]:
                self_.value = kvargs["value"]
            else:
                self_.value = kvargs["value"]
        for k0, v in kvargs.items():
            k = k0.replace("-", "_")
            if k not in getattr(self_, "__annotations__", {}):
                self_.__setattr__(k, v)


class Automations:
    def __init__(self, config: Config) -> None:
        self.config = config
        self.auth = Auth(config)

    def get_settings(self) -> SettingsObject:
        """
        Retrieves the automation settings.

        Parameters
        ----------
        """
        response = self.auth.rest(
            path="/automations/settings",
            method="GET",
            params={},
            data=None,
        )
        obj = SettingsObject(**response.json())
        obj.auth = self.auth
        return obj

    def set_settings(self, data: SettingsObject) -> SettingsObject:
        """
        Update the automation settings.

        Parameters
        ----------
        data: SettingsObject

        """
        if data is not None:
            try:
                data = asdict(data)
            except:
                data = data
        response = self.auth.rest(
            path="/automations/settings",
            method="PUT",
            params={},
            data=data,
        )
        obj = SettingsObject(**response.json())
        obj.auth = self.auth
        return obj

    def get_usages(self, filter: str, breakdown: str = None) -> UsageList:
        """
        Retrieves all automation usage for a tenant
        Limitations:
         * Maximum 1000 metrics per API call

        Parameters
        ----------
        filter: str
          Indicates how the metrics should be filtered
        breakdown: str = None
          If specified, result will be broken apart for each automation
        """
        query_params = {}
        if filter is not None:
            query_params["filter"] = filter
        if breakdown is not None:
            query_params["breakdown"] = breakdown
        response = self.auth.rest(
            path="/automations/usage",
            method="GET",
            params=query_params,
            data=None,
        )
        obj = UsageList(**response.json())
        obj.auth = self.auth
        return obj

    def export(self, automationId: str, runId: str) -> object:
        """
        Export a run.
        Creates a temporary pre-signed AWS S3 URL to download the raw logs of a completed run. This URL is valid for 5 minutes.

        Parameters
        ----------
        automationId: str
          The unique identifier for the automation.
        runId: str
          The unique identifier for the run.
        """
        response = self.auth.rest(
            path="/automations/{automationId}/runs/{runId}/actions/export".replace(
                "{automationId}", automationId
            ).replace("{runId}", runId),
            method="POST",
            params={},
            data=None,
        )
        return response.json()

    def retry(self, automationId: str, runId: str) -> None:
        """
        Retry a run.
        This action allows you to retry a specific run by creating a new run using the same inputs.

        Parameters
        ----------
        automationId: str
          The unique identifier for the automation.
        runId: str
          The unique identifier for the run.
        """
        self.auth.rest(
            path="/automations/{automationId}/runs/{runId}/actions/retry".replace(
                "{automationId}", automationId
            ).replace("{runId}", runId),
            method="POST",
            params={},
            data=None,
        )

    def stop(self, automationId: str, runId: str) -> None:
        """
        Stop a running automation.
        Force a run to be stopped immediately.

        Parameters
        ----------
        automationId: str
          The unique identifier for the automation.
        runId: str
          The unique identifier for the run.
        """
        self.auth.rest(
            path="/automations/{automationId}/runs/{runId}/actions/stop".replace(
                "{automationId}", automationId
            ).replace("{runId}", runId),
            method="POST",
            params={},
            data=None,
        )

    def get_run(self, automationId: str, runId: str) -> RunDetailResponseObject:
        """
        Retrieves a run

        Parameters
        ----------
        automationId: str
          The unique identifier for the automation.
        runId: str
          The unique identifier for the run.
        """
        response = self.auth.rest(
            path="/automations/{automationId}/runs/{runId}".replace(
                "{automationId}", automationId
            ).replace("{runId}", runId),
            method="GET",
            params={},
            data=None,
        )
        obj = RunDetailResponseObject(**response.json())
        obj.auth = self.auth
        return obj

    def get(self, id: str) -> AutomationDetailResponseObject:
        """
        Retrieves an automation

        Parameters
        ----------
        id: str
          The unique identifier for the automation.
        """
        response = self.auth.rest(
            path="/automations/{id}".replace("{id}", id),
            method="GET",
            params={},
            data=None,
        )
        obj = AutomationDetailResponseObject(**response.json())
        obj.auth = self.auth
        return obj

    def get_automations(
        self,
        filter: str = None,
        limit: int = 100,
        sort: str = "id",
        max_items: int = 100,
    ) -> ListableResource[AutomationListObject]:
        """
        Retrieves a list of automations

        Parameters
        ----------
        filter: str = None
          Allowed filters: name, runMode, lastRunStatus, ownerId
        limit: int = 100
          The number of automations to retrieve.
        sort: str = "id"
          The field to sort by, with +- prefix indicating sort order. (?query=-name => sort on the name field using descending order)
        """
        query_params = {}
        if filter is not None:
            query_params["filter"] = filter
        if limit is not None:
            query_params["limit"] = limit
        if sort is not None:
            query_params["sort"] = sort
        response = self.auth.rest(
            path="/automations",
            method="GET",
            params=query_params,
            data=None,
        )
        return ListableResource(
            response=response.json(),
            cls=AutomationListObject,
            auth=self.auth,
            path="/automations",
            max_items=max_items,
            query_params=query_params,
        )

    def create(
        self, data: AutomationDetailRequestObject
    ) -> AutomationDetailResponseObject:
        """
        Create an automation

        Parameters
        ----------
        data: AutomationDetailRequestObject
          Automation object to create
        """
        if data is not None:
            try:
                data = asdict(data)
            except:
                data = data
        response = self.auth.rest(
            path="/automations",
            method="POST",
            params={},
            data=data,
        )
        obj = AutomationDetailResponseObject(**response.json())
        obj.auth = self.auth
        return obj
