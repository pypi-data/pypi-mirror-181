...
class BaseProvider(ABC):
    ...
    def _validate_df(self, df: DataFrame) -> DataFrame:
        ...
        if set(df.columns) != set(self.columns.keys()):
            ...
            raise ValueError(
                f'DataFrame column content from {type(self).__name__}.get_data() does not match the provider spec.\n'
    ...
    def _enforce_column_types(self, df):
    ...
    @staticmethod
    def _csv_chunk_iterator(dfs: Iterator[DataFrame]) -> Iterator[str]:
        for df in dfs:
        ...
        async def provider_data(request: Request):
            ...
            try:
                ...
                if isinstance(data, pd.DataFrame):
                    ...
                    raise TypeError(
                        f'{type(self).__name__}.get_data did not return/yield a DataFrame. Was {type(data).__name__})'
                    )

                dfs = map(self._enforce_column_types, map(self._validate_df, data))
                return StreamingResponse(self._csv_chunk_iterator(dfs))
