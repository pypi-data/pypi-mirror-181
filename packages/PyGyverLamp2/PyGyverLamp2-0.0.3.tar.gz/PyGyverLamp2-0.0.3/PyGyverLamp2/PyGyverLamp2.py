...
from enums import Codes, TypeControls
...
class Lamp:
    def __init__(self, key: str, ip: str, port: int, group_id: int, json_settings_path: str = 'settings.json',
                 ...
                 server_port: int = 14523, server_debug: bool = False, timeout_sync: int = 10, log_data_request: bool = False,
    ...
    def set_auto_sync_settings(self, state: bool):
        self.__send_request(f'{Codes.set_auto_sync_settings.value},{int(state)}')

    def sync_settings(self, auto_sync: bool = False, wait_sync: bool = True, timeout: int = None):
        self.__send_request(f'{Codes.sync_settings.value},{int(auto_sync)}')
        if wait_sync:
    ...
    def random_effects(self, count: int = 3, *date, **kwargs):
        ...
        max_brightness, min_brightness = [max_brightness, min_brightness] if max_brightness > min_brightness else [min_brightness, max_brightness]
    ...
    def __get_name_control_by_code(self, code):
        for type_control in TypeControls:
            if type_control.value == code:
                return type_control.name
    ...
    def __send_request(self, *args, **kwargs):
        ...
        if kwargs.get('skip') is None and (self.__enable_task_list and current_time - self.__last_request_time < 550):
        ...
        if self.__log_data_request:
            print(f'{self.__get_name_control_by_code(message.split(",")[1])} - {message}')
    ...
    def __task_list_processing(self):
        while True:
            ...
            if len(self.__task_list) > 0 and (
                    current_time - self.__last_request_time >= 550 or self.__check_multi_requests(current_time)):
    ...
    def __now_date(self):
        day = datetime.today()
        return f'{datetime.today().isoweekday()},{day.hour},{day.minute},{day.second + 1}'

    def __start_server(self):
        app = _FlaskAppWrapper(name=self.server_name, host=self.server_host, port=self.server_port, debug=self.server_debug)
        app.add_endpoint(endpoint='/', endpoint_name='', handler=self.__sync_response)
        app.run()
