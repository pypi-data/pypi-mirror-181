...
from tao_byom.layers.custom_layers import GroupConv, ZeroPadding1D_NCW, ZeroPadding1D_NCW_TF2
import tensorflow as tf

if tf.__version__[0] == '1':
    # ZeroPadding1D are different for TF1 and TF2
    zero_padding = ZeroPadding1D_NCW
    FRAMEWORK = 'TF1'
else:
    zero_padding = ZeroPadding1D_NCW_TF2
    FRAMEWORK = 'TF2'
    ...
    if len(W.shape) == 5:  # 3D conv
        ...
        if auto_pad == "NOTSET":
            ...
            if padding:
                ...
                if FRAMEWORK == "TF1":
                    input_height, input_width = input_0.shape[2].value, input_0.shape[3].value
                else:
                    input_height, input_width = input_0.shape[2], input_0.shape[3]

                out_height = (input_height + strides[0] - 1) // strides[0]
                out_width = (input_width + strides[1] - 1) // strides[1]
                filter_size_height = (height - 1) * dilation + 1
                filter_size_width = (width - 1) * dilation + 1

                height_padding = (out_height - 1) * strides[0] + filter_size_height - input_height
                width_padding = (out_width - 1) * strides[1] + filter_size_width - input_width
