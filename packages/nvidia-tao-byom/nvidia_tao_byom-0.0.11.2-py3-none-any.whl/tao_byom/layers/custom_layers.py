...
if tf.__version__[0] == '1':
    ...
    zero_pad = keras.layers.convolutional._ZeroPadding
else:
    from keras.engine.input_spec import InputSpec
    zero_pad = keras.layers.Layer
...
class ImageResizeLayer(keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        output = tf.compat.v1.image.resize_images(inputs, self.output_dim, self.mode, align_corners=(self.coordinate_transformation_mode == "align_corners"))
...
class ZeroPadding1D_NCW(zero_pad):
...
class ZeroPadding1D_NCW_TF2(zero_pad):
    """Zero-padding layer for 1D input (e.g. temporal sequence).

    Default ZeroPadding1D only accepts `channels_last`.
    Ref: https://github.com/keras-team/keras/blob/master/keras/layers/reshaping/zero_padding1d.py
    ...
    def call(self, inputs):
        """Zero Pad."""
        pattern = [[0, 0], [self.padding[0], self.padding[1]], [0, 0]]
        return tf.compat.v1.pad(inputs, pattern)
