...
def tao_byom_converter(onnx_model, input_names,
    ...
    """
    # Throw an error msg if system python version is not 3.6.* for TF1 or 3.8.* for TF2
    if sys.version_info.major != 3 or sys.version_info.minor not in [6, 8]:
        raise RuntimeError('Please use Python version 3.6 TAO BYOM for TF1. '
                           'If you wish to use TF2 workflow, please set Python version to 3.8. '
                           'This is required for loading BYOM model in the TAO container.')

    if sys.version_info.minor == 6 and tf.__version__[0] == '2':
        raise RuntimeError('Please install TF2 if you are in Python 3.8. Only Python 3.6 is allowed for TF1.')

    # Only patch for TF1
    if tf.__version__[0] == '1':
        import tao_byom.third_party.tensorflow_backend as TFB   # noqa pylint: disable=c0415
        import tao_byom.third_party.nn_ops as NOP  # noqa pylint: disable=c0415
