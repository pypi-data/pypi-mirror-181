...
def main():
    ...
    parser.add_argument('cover', type=str, help='A dictionary whose keys are the names of mobile operators,'
                                                ' and the values are the sets of regions they cover')
    parser.add_argument('regions', type=str, help='The set of all regions that need to be covered')
    parser.add_argument('-t', '--test', type=bool, help='test mode')
    ...
    if test:
        print('TEST MODE ON')
        pytest.main(['-q', 'OP/project/package/tests/test.py'])
        return
    else:
        ...
        greedy(phone_operator_regions, regions)
