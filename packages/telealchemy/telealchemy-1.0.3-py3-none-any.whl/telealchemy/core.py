...
class AlchemyCoreSession(AlchemySession):
    def _load_session(self) -> None:
        ...
        rows = self.engine.execute(
            select([t.c.dc_id, t.c.server_address, t.c.port, t.c.auth_key]).where(
    ...
    def get_update_state(self, entity_id: int) -> Optional[updates.State]:
        ...
        rows = self.engine.execute(
            select([t]).where(
                and_(t.c.session_id == self.session_id, t.c.entity_id == entity_id)
            )
        )
        try:
    ...
    def _update_session_table(self) -> None:
        with self.engine.begin() as conn:
            conn.execute(
                self.Session.__table__.delete().where(
                    self.Session.session_id == self.session_id
                )
            )
            conn.execute(
                self.Session.__table__.insert(),
                session_id=self.session_id,
                dc_id=self._dc_id,
    ...
    def delete(self) -> None:
        with self.engine.begin() as conn:
            ...
            conn.execute(
                self.Entity.__table__.delete().where(
                    self.Entity.__table__.c.session_id == self.session_id
                )
            )
            conn.execute(
                self.SentFile.__table__.delete().where(
                    self.SentFile.__table__.c.session_id == self.session_id
                )
            )
            conn.execute(
                self.UpdateState.__table__.delete().where(
    ...
    def get_entity_rows_by_username(self, key: str) -> Optional[Tuple[int, int]]:
        return self._get_entity_rows_by_condition(
            self.Entity.__table__.c.username == key
        )
    ...
    def _get_entity_rows_by_condition(self, condition) -> Optional[Tuple[int, int]]:
        ...
        rows = self.engine.execute(
            select([t.c.id, t.c.hash]).where(
                and_(t.c.session_id == self.session_id, condition)
            )
        )
        try:
            return next(rows)
