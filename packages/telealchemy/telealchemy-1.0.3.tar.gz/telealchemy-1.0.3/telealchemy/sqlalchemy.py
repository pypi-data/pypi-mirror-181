...
class AlchemySessionContainer:
    def __init__(
        ...
        if manage_tables:
            ...
            if (
                sql.inspect(self.db_engine).has_table(self.Version.__tablename__)
                is False
            ):
    ...
    def has_session(self, session_id: str) -> bool:
        if self.core_mode:
            ...
            rows = self.db_engine.execute(
                select([func.count(t.c.auth_key)]).where(
                    and_(t.c.session_id == session_id, t.c.auth_key != b"")
                )
            )
            try:
                (count,) = next(rows)
            ...
            return (
                self.Session.query.filter(self.Session.session_id == session_id).count()
                > 0
            )
