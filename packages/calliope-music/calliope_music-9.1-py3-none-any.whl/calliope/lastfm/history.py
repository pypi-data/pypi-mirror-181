...
class ListenHistory(ListenHistoryProvider):
    def __init__(self, username=None, server='last.fm', retry_on_error=True,
                 cachedir=None):
...
def load(username: str, server: str=None, retry_on_error: bool=True,
         cachedir: pathlib.Path=None) -> ListenHistory:
    """Load the listen history database for `user`."""

    history = ListenHistory(username=username, server=server, retry_on_error=retry_on_error, cachedir=cachedir)
    return history
