...
from offline_model_builder.user_profile.constants import \
    ...
    CUSTOMER_MODIFIED_ON, GENDER, LABEL, CENTROID, PROPERTIES, IS_PAYTV, IS_PAY_TV, MINIBATCH_KMEANS_FEATURE, STATUS
...
class CentroidGenerator(InterCentroidDistance):
    ...
    def compute_centroids(
        ...
        centroids = self.find_all_centroids(
            ...
            self.data[MINIBATCH_KMEANS_FEATURE].to_numpy(),
        ...
        centroids_df[MINIBATCH_KMEANS_FEATURE] = list(centroids.keys())
        centroids_df[MINIBATCH_KMEANS_FEATURE] = list(map(int, centroids_df[MINIBATCH_KMEANS_FEATURE]))
    ...
    def write_centroid_properties_to_s3(self, centroid_properties, centroids_df, resource, s3_bucket_name,
        ...
        ConnectS3().write_csv_to_s3(bucket_name=s3_bucket_name,
                                    object_name=s3_object_name + status + MINIBATCH_KMEANS_FEATURE + "_centroids.csv",
                                    df_to_upload=centroid_properties,
                                    resource=resource)
        ConnectS3().write_csv_to_s3(bucket_name=s3_bucket_name,
                                    object_name=s3_object_name + status + MINIBATCH_KMEANS_FEATURE + "_centroids_feature.csv",
                                    df_to_upload=centroids_df,
                                    resource=resource)
        ConnectS3().write_csv_to_s3(bucket_name=s3_bucket_name,
                                    object_name=s3_object_name + status + MINIBATCH_KMEANS_FEATURE + "_users_clustering_feature.csv",
