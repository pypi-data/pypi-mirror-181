...
class ContentApi(object):
    ...
    def create_attachment(self,attachment: Attachment,page_id:str,file_name:str,content,content_type:str):
        path = f'{BASE_PATH}/{page_id}/child/attachment'
        attachments = self.__api_client.get(path,query_params={"filename": file_name})
        results = attachments.json()['results']
        if results:
            path = path + "/" + results[0]["id"] + "/data"
        files={"file": (file_name, content, content_type)}
        self.__api_client.post(path,body=attachment,files=files)
