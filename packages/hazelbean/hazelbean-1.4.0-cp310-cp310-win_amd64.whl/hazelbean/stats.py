...
class RegressionFrame(object):
    ...
    def initialize_df_from_equation(self, equation, current_bounding_box, existing_df=None,):
        ...
        # NOTE: I need a new initialize_df_from_equation that doesn't assume everything is a GLOBAL_aligned_input
        # Ultimately I want to be able to mix and match global and local sources
        is_global = False
        if is_global:
        ...
        for c, variable_transform in enumerate(self.equation_dict['variable_transforms']):
            if data_cur is None:
                ...
                if first_time:
                    data_cur = data
            elif n_new_cols > 0 and len(df) != 0:
                pass
                # data_cur = np.concatenate((data_cur, data), axis=1)  # Funny newaxis is because needed to pass a 2d array to match data
            ...
            if variable_transform == 'intercept':
                if 'intercept' not in df.columns:
                    ...
                    current_array = np.ones(data_cur.shape[0])
                    data_cur = np.concatenate((data_cur, current_array.flatten()[:, np.newaxis]), axis=1)  # Funny newaxis is because needed to pass a 2d array to match data
                    added_column_index += 1
                ...
                if parsed_variable_name not in df.columns:
                    ...
                    current_array = hb.load_geotiff_chunk_by_bb(correct_aligned_inputs[parsed_input_name].path,
                                                                current_bounding_box,
                                                                stride_rate=actual_stride_rate)
                ...
                #
                #     current_array = hb.load_geotiff_chunk_by_bb(correct_aligned_inputss[parsed_input_name].path,
                #                                                 current_bounding_box,
                #                                                 stride_rate=actual_stride_rate)
                #     current_array = np.where(np.isin(current_array, values_to_make_dummy), 1, 0)
                #
