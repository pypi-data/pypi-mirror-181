...
def calculate_zone_to_chunk_list_lookup_dict(ndarray[DTYPEINT32_t, ndim=2] input_array):
    ...
    cdef output_r_array = np.zeros((n_values, max_n_in_zone), dtype=np.int)
    cdef output_c_array = np.zeros((n_values, max_n_in_zone), dtype=np.int)

    cdef row_counter = np.zeros(max_n_in_zone, dtype=np.int)
