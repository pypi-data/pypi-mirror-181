...
def create_cascade(files, key=-1, sep=os.path.sep):
    ...
    fps = sorted(fps, key=lambda file: [sep.join(file.split(sep)[:-1]), file.split(sep)[-1]])
    ordered = []
    def loop_dir(dir):
        nonlocal ordered
        ...
        for __dir in pend:
            ...
            if found:
                ...
                ordered[-1].append((sep.join(__dir), 0))
    loop_dir(fps[0].split(sep)[0])
    return ordered


def format_cascade(cascade, key=-1, sep=os.path.sep, f_sep=" "):
    ...
    formatted = ""
    cols = list(zip(*cascade))[:-1]
    cols.pop(key)
    cols = [len(max(_, key=len)) for _ in cols]
    template = ""
    for col in cols:
        if template:
            template += f_sep
