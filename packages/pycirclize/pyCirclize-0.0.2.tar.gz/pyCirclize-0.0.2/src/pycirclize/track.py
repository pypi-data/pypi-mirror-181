...
class Track:
    ...
    def bar(
        ...
        *,
        align: str = "center",
        facecolor: str | list[str] = "tab:blue",
        **kwargs,
    ) -> None:
        """Plot bar
        ...
        bottom : float, optional
            Bar bottom limit value
        top : float | None, optional
            Bar top limit value. If None, `max(height)` is set.
        align : str, optional
            Bar alignment type (`center` or `edge`)
        facecolor : str | list[str], optional
            Bar facecolor(s)
        **kwargs : dict, optional
            Patch properties (e.g. `fc="tomato", ec="black", lw=0.5, hatch="//"`)
            <https://matplotlib.org/stable/api/_as_gen/matplotlib.patches.Patch.html>
        """
        ...
        top = max(height) if top is None else top
        self._check_value_min_max(height, bottom, top)
        height = [self._y_to_r(v, bottom, top) for v in height]
        height = np.array(height) - min(self.r_plot_lim)
        ...
        for idx, (rad, h) in enumerate(zip(x, height)):
            # Set radian & r position
            if align == "center":
                radr = (rad - (width / 2), min(self.r_plot_lim))
            elif align == "edge":
                radr = (rad, min(self.r_plot_lim))
            else:
                raise ValueError(f"{align=} is invalid ('center' or 'edge').")
            # Set facecolor(s)
            if isinstance(facecolor, (list, tuple)):
                kwargs.update(dict(facecolor=facecolor[idx]))
            else:
                kwargs.update(dict(facecolor=facecolor))
            # Add bar rectangle patch
    ...
    def hist(
        ...
        rad = list(map(self.x_to_rad, x))
        bins_rad = self.x_to_rad(bins)

        def plot_hist(ax: PolarAxes) -> None:
            ax.hist(rad, bins_rad)

        self._plot_funcs.append(plot_hist)
    ...
    def heatmap(
        ...
        *,
        ...
        start : float | None, optional
            ...
            If None, track start position is set.
        end : float | None, optional
            End position for heatmap plot (x coordinate).
            If None, track end position is set.
    ...
    def tree(
        ...
        *,
        ...
        show_leaf_label: bool = False,
        show_innode_label: bool = False,
        label_size: float = 8,
        label_margin: float = 0.5,
        label_formatter: Callable[[Clade], str] | None = None,
        nodes_color: list[tuple[list[str], str]] = [],
        line_kws: dict[str, Any] = {},
        text_kws: dict[str, Any] = {},
    ) -> None:
        """Plot tree
        ...
        show_leaf_label : bool, optional
            If True, show tree leaf node label.
        show_innode_label : bool, optional
            If True, show tree internal node label.
        label_size : float, optional
            Label size
        label_margin : float, optional
            Label margin size
        ...
        for node in tree.get_nonterminals():
            ...
            for child_node in node.clades:
                ...
                self._simpleline((parent_x, child_x), (parent_r, parent_r), **line_kws)
                self._simpleline((child_x, child_x), (parent_r, child_r), **line_kws)
        # Plot label if required
        for node in tree.find_clades():
            if node.is_terminal() and show_leaf_label:
                orientation = "vertical"
            elif not node.is_terminal() and show_innode_label:
            ...
            r = r + label_margin if outer else r - label_margin
            rad = self.x_to_rad(x)
            params = utils.plot.get_label_params_by_rad(rad, orientation, outer)
            text_kws.update({**params, **dict(size=label_size)})
            self.text(label, x, r, orientation=orientation, **text_kws)
