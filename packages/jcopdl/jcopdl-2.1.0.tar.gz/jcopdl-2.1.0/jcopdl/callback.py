...
from jcopdl.utils import IN_NOTEBOOK
from jcopdl.utils.exception import NotLoggedError, InvalidScaleValue, InvalidSenseValue, ModelError
...
class Callback:
    ...
    if callback.early_stopping("test_cost"):
        break
    """
    def __init__(self, model, model_config=None, metadata=None, checkpoint_every=50, early_stop_patience=5, max_epoch=None, outdir="model"):
        self.outdir = Path(outdir)   
        self.outdir.mkdir(parents=True, exist_ok=True)
        
        self._check_value(model, model_config, metadata, max_epoch)
    ...
    def early_stopping(self, sense, monitor, load_best_when_stop=True, plot=True, figsize="auto"):
        """
        ...
        if stop:
            ckpt_path = self.outdir / f'model_checkpoint.pth'
    ...
    def add_plot(self, metrics, scale="linear"):
        ...
        """
        if scale not in ["linear", "semilogy"]:
            raise InvalidScaleValue('Available scale {"linear", "semilogy"}')
    ...
    def _save(self, fname):
        torch.save(self.model, self.outdir / f'model_{fname}.pth')
        try:
            pd.DataFrame(self.logs).to_csv(self.outdir / "logs.csv", index=None)
        except:
            torch.save(self.logs, self.outdir / "logs.pth")

    def _check_value(self, model, model_config, metadata, max_epoch):
        ...
        if (model_config is not None) and isinstance(model_config, dict):
            torch.save(model_config, self.outdir / "configs.pth")
