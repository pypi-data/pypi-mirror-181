...
class MolcasExtractor(RegexExtractor):
    ...
    def __init__(self, prog, *args, **kwargs):
        ...
        if prog == "single_aniso":  # exception in header naming convention
            self.header = \
                self.sep_line + \
                r"\&SINGLE_ANISO(_OPEN)?\s+" + self.skip + \
                self.sep_line
        else:
            self.header = \
                self.sep_line + \
                r"\&{}\s+".format(prog.upper()) + self.skip + \
                self.sep_line
        ...
        footer = (r"\-\-\- Module {} spent (?:(?{}\d+) hours? )?"
                  r"(?:(?{}\d+) minutes? )?(?{}\d+) seconds? \-\-\-\s+")

        self.footer = footer.format(prog, ":", ":", ":")
        self.timing = footer.format(prog, "P<hrs>", "P<mins>", "P<secs>")
...
class ExtractTiming(MolcasExtractor):
    ...
    def format_data(self, data, secs=None, mins=None, hrs=None, **kwargs):
        return np.array([
            int(secs or 0) +
            int(mins or 0) * 60 +
            int(hrs or 0) * 3600])
