...
class PlayerHome(gym.Env):
    ...
    def score(self):
        ...
        """
        kpis = {"std_demand": [np.std(self.demand_profile)], "max_demand": [np.max(self.demand_profile)], "l2_dev_avg": [np.linalg.norm(np.subtract(self.demand_profile, np.mean(self.demand_profile)))]}
