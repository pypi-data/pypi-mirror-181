...
def sleep_random_seconds(*thresholds: "int | float") -> "float":
    """
    ...
    max_second = thresholds[-1]
    if len(thresholds) == 1:
        min_second = 0
    elif len(thresholds) == 2:
        min_second = thresholds[0]
    else:
        error("number of parameter 'thresholds' must is one or two", TypeError)
