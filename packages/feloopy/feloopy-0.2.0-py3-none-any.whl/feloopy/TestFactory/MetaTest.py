...
class Implement:
    ...
    def __init__(self, ModelFunction, Times=10):
        '''
        ...
        * Times (Integer): The number of times in which the heuristic optimization algorithm is implemented to deduce the (near) optimal solution. 
        '''
        ...
        match self.InterfaceName:
            ...
            case 'mealpy':
                ...
                self.BestAgent, self.BestReward = MealpySolutionGenerator.GenerateSolution(
                    self.ModelObject, self.Fitness, self.ToTalVariableCounter, self.ObjectivesDirections, self.ObjectiveBeingOptimized, self.Times)
...
def instance(X):
    m = feloopy('heuristic', 'TSP', 'feloopy', X)
    x = m.bvar('x', [range(8),range(8)], VarBound=[0, 1])
    m.obj(x(0,0)+x(0,1))
    m.sol(['max'], 'DE', {'S': 1000, 'T': 10})
    return m[X]
