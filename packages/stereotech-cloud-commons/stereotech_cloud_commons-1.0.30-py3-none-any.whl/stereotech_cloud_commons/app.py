...
class StereotechCloud:
    ...
    def register_static_file_handler(self,
        ...
        if target:
            params = {'path': file_path, "width": width, "height": height}
            self.mutable_router.add_handler(pattern, target, params)
        else:
...
class ImageFileHandler(AuthorizedFileHandler):
    """
    Handler for images from the directory
    """

    async def get(self, path: str, include_body: bool =True) -> None:
        """
        Return image from the directory by path
        Optional request args width, height
        """
        ...
        if width and height:
            ...
            pic.save(buff, format="JPEG", quality=100)
        else:
