...
def read_imports(src_fspath):
    ...
    for line in imports:
        if len(line) > 0 and line[0] == '#':
            continue
        # import_fmt = import [module_1] (as [alias_1]), ... , [module_i] (as [alias_i])
        S1 = line[7:]
        # print(S1)
...
# import [module_name] as [module_alias] => [module_name] in range(module_space_map)
# module_space_map: [ module_name in src_pkg_name ] => [ module_name in dst_pkg_name ]
...
# module_transform: [ dst_module_name ] => list[ src_rel_fspath ]
# result: [ src_module_name ] => [ dst_module_name ], prepending pkg_names
# check that result is a bijection
def compute_module_correspondence( src_pkg_name, dst_pkg_name, module_transform ):
    ...
    for dst_module_name in module_transform:
        for rel_path in module_transform[dst_module_name]:
            ...
            # case: transform is not injective
            ...
            # case: result[ result_values[dst_mod] ] = result[ src_mod ] = dst_mod
...
# src_pkg_name = '__py_utils'
# dst_pkg_name = 'dev_utils'
# src_base = '/home/algorithmspath/sdev/hb_api_typon/api'
# dst_base = '/home/algorithmspath/py_local/packages'
# module_mapping = dev_utils_mapping.module_transform
