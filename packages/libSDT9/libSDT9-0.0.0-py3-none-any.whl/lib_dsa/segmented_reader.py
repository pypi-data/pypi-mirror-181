...
# return bytes_read
# reads >= max( num_bytes, buffer_size )
...
# file_format:
# [entry1_size] [entry1] ... [entry_i_size] [entry_i]
...
class forward_reader():
    ...
    def has_next(self):
        ...
        # read entry_size, len( entry_size ) = 8
        if len( self.buffer ) < self.buffer_index + 8:
            ...
            # case: size could not be read
            if len(buf_seg) < bytes_to_read:
                self.iter_complete = True
                return False
            # appending self.buffer segment
    ...
    # return initial_index, entry_size
    def get_next(self):
    ...
    # extracted_value = self.buffer[ self.initial_index, self.initial_index + self.entry_size ]
