...
class MainWindow(YkWindow):
    ...
    def batch_all_conditions(self):
        ...
        for input_values in dynamic_for(data_for):
            ...
            if result.success(no_error_as_success=True):
                ...
                logger.debug(f"输入参数为：{res_para_description} = {output_values}")
    ...
    def solve(self, power=348.75, heat_flow=0.0, heat_flow2=0.0, t_heat_flow2=0.0, t_outlet=0.0):
        """
        求解指定参数下的热耗
        ...
        :return:
        """
        result = self._solve(power, heat_flow, heat_flow2, t_heat_flow2, t_outlet)
        logger.debug(result)
        return result
    ...
    def _solve(self, power=348.75, heat_flow=0, heat_flow2=0, t_heat_flow2=0, t_outlet=0):
        """
        每一个请求都是一个新线程，每一个线程都要单独初始化Ebsilon
        :param args:
        :return:
        """
        return_dict = {"power": power,
                       "f1": heat_flow,
                       "f2": heat_flow2,
                       "t2": t_heat_flow2,
                       "t_outlet": t_outlet, }
        ...
        # ebs.show_window()
        try:
            model.set_value("Controller_1", "SCV", float(power))
            model.set_value("Measuring_point", "MEASM", float(heat_flow))
            model.set_value("Boundary_value_1", "M", float(heat_flow2))
            model.set_value("Boundary_value_1", "T", float(t_heat_flow2))
            model.set_value("condenser_pump_T_1", "MEASM", float(t_outlet))

            result = model.simulate_new()

            _1 = model.get_value("Logic_4", "Q")
            _2 = model.get_value("Electric", "Q")
            汽机热耗率 = _1 / _2 * 3600

            _3 = model.get_value("Efficiency_meter", "ETA")
            _4 = model.get_value("Generator", "QREAL")
            _5 = model.get_value("steam_77", "Q")
            _6 = model.get_value("water_47", "Q")
            _7 = model.get_value("steam_80", "Q")
            _8 = model.get_value("water_49", "Q")
            汽机热耗率2 = (3.6 / _3 * _4 - 3.6 * _5 + 3.6 * _6 - 3.6 * _7 + 3.6 * _8) / _4 * 1000
            ...
            if result.success(no_error_as_success=True):
                ...
                logger.warning(f"{power=},{heat_flow=},{heat_flow2=},{t_heat_flow2=},"
                               f"{t_outlet=}, {model=}")
                # input()
                return_dict.update({"hr1": 0.0, "hr2": 0.0, "model": model})
        except (AttributeError, NameError):
            return_dict = {}
        return return_dict
