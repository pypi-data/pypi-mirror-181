...
class MainWindow(YkWindow, QMainWindow):
    ...
    def output_log(self, msg, level="DEBUG"):
        """
        在日志面板输出消息
        """
        log_editor: YkEditor = self.ui.log_editor

        log_editor.append(str(msg) + "\n")
        logger.debug(msg)

    def output_console(self, msg):
        self.ui.terminal.append(msg)
    ...
    def open(self, proj=None):
        ...
        self.add_content_tab(tab_name=proj_name)
    ...
    def calculate(self):
        ...
        :return:
        """
        components = self.content_widget.scene.items_scene_id
        epochs = 10
        exit_msg = "计算失败！"
        error_info = {}

        for scene_id, com in components.items():
        ...
        for i in range(1, epochs):
            total_num = len(components) - 3
            calculated = {}
            exit_epochs = False
            for scene_id, com in dict(components.items()).items():  # 不加dict，调试经常出现无法解析整形的错误
                if com.__class__.__name__ in ["SceneGridItem", "SceneFlagItem", "CoordinateItem"]:
                    continue  # 场景的辅助组件不参与计算
                ...
                if msg is None:
                    self.output_log(f"组件{scene_id}的calculate方法未返回计算结果信息，请检查组件代码！")
                    return
                ...
                if msg == "done":
                    ...
                    error_info.update({scene_id: msgs})
                    self.output_log(error_info)  # 输出错误原因
                    exit_epochs = True
                    exit_msg = "计算失败！"
                    break
                else:
                    com.calculated = False
                    if error_info.get(scene_id) == msg:  # 当前错误信息与上一次相同，则退出计算
                        self.output_log(error_info)
                        exit_epochs = True
                        exit_msg = "计算失败！"
                        break
                    error_info.update({scene_id: msg})  # 更新当前组件的错误信息
    ...
    def show_active_tool_window(self):
        # if self.ui.ActiveToolWindow.isHidden():
        #     self.ui.ActiveToolWindow.show()
        # if not self.ui.ActiveToolWindow.isVisible():
        #     self.ui.ActiveToolWindow.show()
        if self.ui.dockWidget_ATW.isHidden():
            self.ui.dockWidget_ATW.show()

    def display_info(self, scene_id, tab=None):
        """
                ...
                if len(子参数项) >= 5:
                    ...
                    if isinstance(var4, str):
                        ...
                        if var4.get("size") is not None:
                            size = var4.get("size")
                item = YkItem(f"{name}[{symbol}]", value, unit, size=size)
                if disabled:
    ...
    def info_tab_changed(self, idx):
        ...
        self.display_info(scene_id=scene_id, tab=tab_name)
