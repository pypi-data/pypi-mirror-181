...
class DataFitterNet(torch.nn.Module):
    def __init__(self, settings1, trial=None):
        ...
        self.dataset: DataSetFitting = None  # 数据集的信息，主要需要使用数据集中的归一化与反归一化的信息，但不会保存dataframe数据
    ...
    def prediction(self, x_input):
        ...
        self.eval()
        y = self.net(x_input)
        return standard_reverse(y, mean=self.mean[self.y_title], std=self.std[self.y_title])
