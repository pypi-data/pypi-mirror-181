...
from yangke.common.QtImporter import QtGui, QDir, QStringListModel, Qt, QModelIndex, QRectF, QPointF, QRect, QSize, \
    ...
    QGraphicsSceneContextMenuEvent, QMenu, QAction, QApplication, QGraphicsLineItem, QLabel
...
def points_contains(points, point, tolerance=6):
    ...
    :param point: 点对象，一般为点击事件发生的鼠标的坐标
...
class PopUp(QWidget):
    def __init__(self, parent, pos: QPointF):
        """
        :param parent: view对象
        :param pos: 场景坐标
        """
    ...
    def append_para(self, var, val, unit):
        if isinstance(unit, Unit):
            unit = unit.unit
        self.layout().addWidget(QLabel(f"f{var:10}{val:10.3}{unit:10}"))
...
class YkGraphicsItem(QGraphicsItem):
    ...
    def hoverEnterEvent(self, event: QGraphicsSceneHoverEvent) -> None:
        popup = PopUp(None, event.screenPos())
        p = self.get("P")
        if p is not None:
            if not isinstance(p, Value):
                p = Value(p, "MPa")
            popup.append_para("P", p.value, p.unit)
    ...
    def spread_values(self, step=None, symbols=None):
        ...
        values = {}
        for s in symbols:
            values.update({s: self.get(s, need_unit=True)})
        ...
        if hasattr(self, "connect_items"):  # 说明是管道组件，管道组件自己处理自己的传值方法
            ...
            for pid, p in self.ports.items():
                ...
                p.values.update(step=step, values=values, set_none=False)
...
class YkGraphicsScene(QGraphicsScene):
    ...
    def __init__(self):
        ...
        self.main_window: YkWindow = None
    ...
    def keyPressEvent(self, event: QtGui.QKeyEvent) -> None:
        ...
        if event.key() == Qt.Key_Delete:  # 如果是删除图元，则不再执行图元的按键事件
            item = self.focusItem()  # 获取到当前激活的图元
            if item is None:  # 当不选择任何图元按delete键时，item为None
                return
            if isinstance(item, PipeItem):
                ...
                for idx, port in item.ports.items():
                    ...
                    if stream_line is not None:
                        _cis = copy.deepcopy(stream_line.connect_items)
                        for _item in _cis:
                            _item: ConnectItem
                            if _item.item_sid == item.scene_id:
                                stream_line.connect_items.remove(_item)
    ...
    def mousePressEvent(self, event: QGraphicsSceneMouseEvent) -> None:
        ...
        item_: YkGraphicsItem = self.itemAt(event.scenePos(), QTransform())
        if event.button() == Qt.LeftButton:
            if isinstance(item_, SceneGridItem) or isinstance(item_, CoordinateItem):  # 点击的位置没有其他组件
                if self.state == "常规":
                    ...
                    if idx != 0:  # 如果组件列表中选中任何组件，则添加组件到场景中
                        try:
                            item: YkGraphicsItem = self.main_window.all_comps[f"Comp{idx}"]()
    ...
    def get_main_window(self, widget=None):
        ...
        if self.main_window is not None:
            ...
            while widget.parentWidget() is not None:
                if isinstance(widget.parentWidget(), YkWindow):
                    self.main_window = widget.parentWidget()
...
class YkGraphicsView(QGraphicsView):
    ...
    def get_main_window(self) -> YkWindow | QWidget:
        if self.main_window is not None:
            return self.main_window
        else:
            widget = self
...
def over_defined(set_var: list):
    ...
    already_set = None
    for sett in set_var:
        set_vars = set(sett.keys())
        if already_set is None:
            already_set = set_vars
        else:
            inter = already_set & set_vars
            if len(inter) > 0:
                return list(inter)
            else:
                already_set = already_set | set_vars

    if "P" in already_set and "T" in already_set and "H" in already_set:
        return ["焓"]
    return None
...
class PipeItem(YkGraphicsItem):
    ...
    def mousePressEvent(self, event: QGraphicsSceneMouseEvent) -> None:
        ...
        if event.button() == Qt.LeftButton:
            idx = points_contains(self.control_points, event.pos(), tolerance=self.pen_width)
    ...
    def calculate(self, step=None):
        ...
        for connect_item in self.connect_items:
            ...
            self.values.update(step=step, values=port.values, set_none=False)  # to_result必须为False，否则会丢失之前的值

        if step == 1:  # 只在第一个迭代步对过定义进行判断
            _ = over_defined(set_var)
            if _ is not None:
                return f"Error: 场景ID为{self.scene_id}的组件过定义！{_} --- break"
        ...
        # 其次检测是否焓值过定义
        suc, msg = validate_water_parameters(p=P, t=T, h=H)
        if suc:
            ...
        else:
            return f"Error: {msg}"
        ...
        if P is not None and T is not None:
            H = get_h_by_pt(P, T)
            self.values.update({"H": Value(H, "kJ/kg")}, step=step)
        elif P is not None and H is not None:
            T = get_t_by_hp(H, P)
            self.values.update({"T": Value(T, "℃")}, step=step)
        elif T is not None and H is not None:
            P = get_p_by_th(T, H)
            self.values.update({"P": Value(P, "MPa")}, step=step)
