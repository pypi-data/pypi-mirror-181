...
def _recursive_render(t_base, o_base, name, context, env):
    ...
    if match_list:
        ...
        if key == 'blueprints':
            ...
            out_key = '__seed'
            # seeds can be accessed at context level, which means it can be used in different views
            # there is another way to access seed, that is blueprint->view->seed, we often use it generate backend logic
            ...
            out_names = [t_name.replace(syntax, v['name']) for v in out_values if not v.get('suffix')]
        elif key == 'models':
            out_key = '__model'
