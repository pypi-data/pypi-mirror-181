...
def git_clone(repo_type, repo_url, repo_dir:Path, checkout):
    repo_dir = repo_dir.parent
    try:
        subprocess.check_output([repo_type, 'clone', repo_url], cwd=repo_dir.__str__(), stderr=subprocess.STDOUT,)
...
def dulwich_clone(repo_type, repo_url, repo_dir:Path, checkout):
...
def clone(
    ...
    logger.debug(f"clone_to_dir is {clone_to_dir}")
    # identify the repo_type
    repo_type, repo_url = identify_repo(repo_url)
    # check that the appropriate VCS for the repo_type is installed
    if not is_vcs_installed(repo_type):
        msg = f"'{repo_type}' is not installed."
        clone_op = dulwich_clone
    else:
        clone_op = git_clone
    # clone_op = dulwich_clone
    ...
    if repo_type == 'git':
        ...
        repo_dir = clone_to_dir.joinpath(repo_name).resolve()
    if repo_type == 'hg':
        # repo_dir = os.path.normpath(os.path.join(clone_to_dir, repo_name))
        repo_dir = clone_to_dir.joinpath(repo_name).resolve()
    ...
    if repo_dir.is_dir():
        ...
        clone = True
    if clone:
        clone_op(repo_type=repo_type, repo_url=repo_url, repo_dir=repo_dir, checkout=checkout)
    return repo_dir