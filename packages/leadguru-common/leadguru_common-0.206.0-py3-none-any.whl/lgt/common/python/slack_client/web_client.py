...
def __get_dedicated_bot_credentials(user: UserModel,
    ...
    bot = next(iter([bot for bot in dedicated_bots
                     if bot.slack_url.strip("/").lower().replace("http://",
                                                                 "https://") == slack_url.replace("http://",
                                                                                                  "https://") and not bot.invalid_creds]),
               None)
...
class SlackMessageConvertService:
    @staticmethod
    def from_slack_response(user_email, bot_name, bot_token, dic, cookies={}):
...
class SlackFilesClient:
    ...
    @staticmethod
    def get_file_url(user_email, bot_name, bot_token, file_id, url_private_download, mimetype, cookies={}):
