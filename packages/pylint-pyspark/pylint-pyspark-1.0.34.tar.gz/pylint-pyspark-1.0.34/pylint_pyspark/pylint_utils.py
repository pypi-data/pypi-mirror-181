...
def select_contains_alias_call(expression):
    if not hasattr (expression.value, "func") or not hasattr(expression.value.func, "attrname"):
        return False
...
def get_length(arg):
    ...
    if isinstance(arg, astroid.nodes.Subscript):
        ...
        return total_length
    if isinstance(arg, astroid.nodes.JoinedStr) or isinstance(arg, astroid.nodes.FormattedValue):
