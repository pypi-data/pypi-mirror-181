...
class OmniSpectrum():
    ...
    def spect(self, direction):
        ...
        self.v[direction] = sp.sv_truemax
...
def csv2docx(path, output, baseline_correction: bool = False, pool=None):
    doc=Document()
    for root,dirs,files in os.walk(path):
        for file in files:
            if file.split('.')[-1] == 'csv':
                ...
                if (not os.path.exists(os.path.join(path, file).split('.')[-2] + '_acc.jpg')):
                    ...
                    fyp(e, n, u, os.path.join(path,file).split('.')[-2] + '_sa.jpg', method='a')
