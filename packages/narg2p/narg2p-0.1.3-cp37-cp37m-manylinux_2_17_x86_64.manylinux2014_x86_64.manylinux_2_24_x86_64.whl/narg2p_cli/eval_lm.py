...
import torch
import narg2p
from narg2p import checkpoint_utils, distributed_utils, options, tasks, utils
from narg2p.dataclass.utils import convert_namespace_to_omegaconf
from narg2p.logging import progress_bar
from narg2p.logging.meters import StopwatchMeter
...
logger = logging.getLogger("narg2p_cli.eval_lm")


def eval_lm(
    models: List[narg2p.models.FairseqModel],
    source_dictionary: narg2p.data.Dictionary,
    batch_iterator: Iterable,
    post_process: Optional[str] = None,
    output_word_probs: bool = False,
    output_word_stats: bool = False,
    target_dictionary: Optional[narg2p.data.Dictionary] = None,
    ...
    """
    Args:
        models (List[~narg2p.models.FairseqModel]): list of models to
            evaluate. Models are essentially `nn.Module` instances, but
            must be compatible with narg2p's `SequenceScorer`.
        source_dictionary (~narg2p.data.Dictionary): dictionary for
        ...
        post_process (Optional[str]): post-process text by removing BPE,
            ...
            narg2p.data.utils.post_process, although not all options
        ...
        target_dictionary (Optional[~narg2p.data.Dictionary]): output
