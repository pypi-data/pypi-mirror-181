...
from narg2p.dataclass.configs import (
    CheckpointConfig,
    CommonConfig,
    CommonEvalConfig,
    DatasetConfig,
...
from narg2p.utils import csv_str_list, eval_bool, eval_str_dict, eval_str_list  # noqa
...
def parse_args_and_arch(
    ...
    from narg2p.models import ARCH_MODEL_REGISTRY, ARCH_CONFIG_REGISTRY, MODEL_REGISTRY
    ...
    if hasattr(args, "task"):
        from narg2p.tasks import TASK_REGISTRY

        TASK_REGISTRY[args.task].add_args(parser)
    if getattr(args, "use_bmuf", False):
        # hack to support extra args for block distributed data parallelism
        from narg2p.optim.bmuf import FairseqBMUF

        FairseqBMUF.add_args(parser)

    # Add *-specific args to parser.
    from narg2p.registry import REGISTRIES
...
def get_parser(desc, default_task="translation"):
    ...
    # Task definitions can be found under narg2p/tasks/
    from narg2p.tasks import TASK_REGISTRY
...
def add_model_args(parser):
    ...
    # Model definitions can be found under narg2p/models/
    #
    ...
    from narg2p.models import ARCH_MODEL_REGISTRY
