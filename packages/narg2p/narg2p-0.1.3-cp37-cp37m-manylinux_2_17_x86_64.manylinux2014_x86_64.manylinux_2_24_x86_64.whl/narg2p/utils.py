...
from narg2p.modules.multihead_attention import MultiheadAttention
...
class FileContentsAction(argparse.Action):
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        from narg2p.file_io import PathManager
...
def load_ensemble_for_inference(filenames, task, model_arg_overrides=None):
    from narg2p import checkpoint_utils
...
def import_user_module(args):
    ...
    if module_path is not None:
        ...
        if not os.path.exists(module_path) and not os.path.isfile(os.path.dirname(module_path)):
            narg2p_rel_path = os.path.join(os.path.dirname(__file__), args.user_dir)
            if os.path.exists(narg2p_rel_path):
                module_path = narg2p_rel_path
            else:
                narg2p_rel_path = os.path.join(
...
def get_perplexity(loss, round=2, base=2):
    from narg2p.logging.meters import safe_round
...
def get_activation_fn(activation: str) -> Callable:
    ...
    from narg2p.modules import gelu, gelu_accurate
...
def tpu_data_loader(itr):
    ...
    from narg2p.data import iterators
