...
class RoundRobinZipDatasets(FairseqDataset):
    """Zip multiple :class:`~narg2p.data.FairseqDataset` instances together.

    Shorter datasets are repeated in a round-robin fashion to match the length
    of the longest one.

    Args:
        datasets (Dict[~narg2p.data.FairseqDataset]): a dictionary of
            :class:`~narg2p.data.FairseqDataset` instances.
