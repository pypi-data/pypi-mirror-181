...
from narg2p.data import (
    ...
    FairseqDataset,
    ResamplingDataset,
    data_utils as narg2p_data_utils,
)
from narg2p.data.audio.audio_utils import get_fbank, get_waveform
from narg2p.data.audio.feature_transforms import CompositeAudioFeatureTransform
...
class S2TDataConfig(object):
    ...
    def vocab_filename(self):
        """narg2p vocabulary file under data root"""
    ...
    def pre_tokenizer(self) -> Dict:
        ...
        Tokenizers are defined in `narg2p.data.encoders.*`"""
...
class SpeechToTextDataset(FairseqDataset):
    ...
    def collater(self, samples: List[Tuple[int, torch.Tensor, torch.Tensor]]) -> Dict:
        ...
        ntokens = None
        if self.tgt_texts is not None:
            target = narg2p_data_utils.collate_tokens(
            ...
            prev_output_tokens = narg2p_data_utils.collate_tokens(
                [t for _, _, t in samples],
                self.tgt_dict.pad(),
                self.tgt_dict.eos(),
                left_pad=False,
