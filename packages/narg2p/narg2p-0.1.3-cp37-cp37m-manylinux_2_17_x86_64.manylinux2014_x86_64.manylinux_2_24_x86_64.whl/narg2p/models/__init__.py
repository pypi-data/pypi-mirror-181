...
from narg2p.dataclass.utils import merge_with_parent, populate_dataclass
from hydra.core.config_store import ConfigStore

from .composite_encoder import CompositeEncoder
from .distributed_narg2p_model import DistributedFairseqModel
from .narg2p_decoder import FairseqDecoder
from .narg2p_encoder import FairseqEncoder
from .narg2p_incremental_decoder import FairseqIncrementalDecoder
from .narg2p_model import (
    BaseFairseqModel,
...
def register_model(name, dataclass=None):
    """
    New model types can be added to narg2p with the :func:`register_model`
    function decorator.
    ...
    def register_model_cls(cls):
        ...
        if dataclass is not None:
            ...
            node = dataclass()
            node._name = name
            cs.store(name=name, group="model", node=node, provider="narg2p")
...
def register_model_architecture(model_name, arch_name):
    """
    New model architectures can be added to narg2p with the
...
for file in os.listdir(models_dir):
    ...
    if (
        ...
        module = importlib.import_module("narg2p.models." + model_name)
