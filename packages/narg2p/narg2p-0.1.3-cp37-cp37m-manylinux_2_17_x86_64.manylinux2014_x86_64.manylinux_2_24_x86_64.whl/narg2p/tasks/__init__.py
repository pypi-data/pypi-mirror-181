...
from .narg2p_task import FairseqTask, LegacyFairseqTask  # noqa
...
def register_task(name, dataclass=None):
    """
    New tasks can be added to narg2p with the
    :func:`~narg2p.tasks.register_task` function decorator.
    ...
    .. note::
        ...
        All Tasks must implement the :class:`~narg2p.tasks.FairseqTask`
        interface.
    ...
    def register_task_cls(cls):
        ...
        if dataclass is not None:
            ...
            node = dataclass()
            node._name = name
            cs.store(name=name, group="task", node=node, provider="narg2p")
...
for file in os.listdir(tasks_dir):
    ...
    if (
        ...
        module = importlib.import_module("narg2p.tasks." + task_name)
