...
import torch
from narg2p import metrics, search, tokenizer, utils
from narg2p.data import Dictionary, FairseqDataset, data_utils, encoders, iterators
from narg2p.dataclass import FairseqDataclass
from narg2p.dataclass.utils import gen_parser_from_dataclass
...
class FairseqTask(object):
    ...
    def dataset(self, split):
        ...
        Returns:
            a :class:`~narg2p.data.FairseqDataset` corresponding to *split*
        """
        from narg2p.data import FairseqDataset
    ...
    def filter_indices_by_size(
        ...
        Args:
            ...
            dataset (~narg2p.data.FairseqDataset): dataset to batch
    ...
    def get_batch_iterator(
        ...
        Args:
            ...
            disable_iterator_cache (bool, optional): don't cache the
                ...
                (default: False).
        Returns:
            ~narg2p.iterators.EpochBatchIterator: a batched iterator over the
                given dataset split
        """
    ...
    def build_model(self, cfg: FairseqDataclass):
        """
        Build the :class:`~narg2p.models.BaseFairseqModel` instance for this
        task.
        ...
        Returns:
            a :class:`~narg2p.models.BaseFairseqModel` instance
        """
        from narg2p import models, quantization_utils
    ...
    def build_criterion(self, cfg: DictConfig):
        """
        Build the :class:`~narg2p.criterions.FairseqCriterion` instance for
        this task.
        ...
        Returns:
            a :class:`~narg2p.criterions.FairseqCriterion` instance
        """
        from narg2p import criterions
    ...
    def build_generator(
        ...
        if getattr(args, "score_reference", False):
            from narg2p.sequence_scorer import SequenceScorer
        ...
        from narg2p.sequence_generator import (
            SequenceGenerator,
            SequenceGeneratorWithAlignment,
        )
        try:
            from narg2p.fb_sequence_generator import FBSequenceGenerator
    ...
    def train_step(
        ...
        Args:
            sample (dict): the mini-batch. The format is defined by the
                :class:`~narg2p.data.FairseqDataset`.
            model (~narg2p.models.BaseFairseqModel): the model
            criterion (~narg2p.criterions.FairseqCriterion): the criterion
            optimizer (~narg2p.optim.FairseqOptimizer): the optimizer
    ...
    def source_dictionary(self):
        """Return the source :class:`~narg2p.data.Dictionary` (if applicable
        for this task)."""
        raise NotImplementedError

    @property
    def target_dictionary(self):
        """Return the target :class:`~narg2p.data.Dictionary` (if applicable
        for this task)."""
        raise NotImplementedError
