...
class LogicJobBroker:
    ...
    def buy(self, symbol: BrokerSymbol, amount=None, amountQuote=None,
        ...
        self.manager.saveInfo("trader", "buy",
                              {"symbol": symbol.symbol, "amount": amount,
                               "amountQuote": amountQuote, "price": price, "group": group})

        return False

    def sell(self, symbol: BrokerSymbol, amount=None, amountQuote=None,
             price=None, leverage=None, group=None):
        symbol = self.fillSymbolInfo(symbol)
        if symbol is None or isinstance(symbol, str) or symbol.symbol is None:
            raise WrongArgumentError("Please use a symbol object to use any brokerage  method")
        self.manager.saveInfo("trader", "sell",
                              {"symbol": symbol.symbol, "amountQuote": amountQuote,
                               "amount": amount, "price": price, "group": group})
        return False

    def printDebug(self):
        return None

    def long(self, symbol: BrokerSymbol, amount: float = None, amountQuote=None,
             price=None, leverage=None, group=None):
        if symbol is None or isinstance(symbol, str) or symbol.symbol is None:
            raise WrongArgumentError("Please use a symbol object to use any brokerage  method")
        self.manager.saveInfo("trader", "long",
                              {"symbol": symbol.symbol,"amount": amount,
                               "amountQuote": amountQuote,  "price": price,
                               "group": group})
        return True

    def short(self, symbol: BrokerSymbol, amount: float= None, amountQuote=None,
              price=None, leverage=None, group=None):
        if symbol is None or isinstance(symbol, str) or symbol.symbol is None:
            raise WrongArgumentError("Please use a symbol object to use any brokerage  method")
        self.manager.saveInfo("trader", "short", {"symbol": symbol.symbol,
