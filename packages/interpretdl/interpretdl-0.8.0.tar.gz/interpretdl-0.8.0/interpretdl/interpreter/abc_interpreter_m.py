...
class Interpreter(ABC):
    ...
    def __init__(self, model: callable, device: str, **kwargs):
        ...
        self.env = kwargs['env'] if 'env' in kwargs else 'pp'

        assert self.device[:3] in ['cpu', 'gpu']
        assert self.env in ['pp', 'pt', 'tf']
...
class InputGradientInterpreter(Interpreter):
    ...
    def _build_predict_fn(self, rebuild: bool = False, gradient_of: str = 'probability'):
        ...
        if self.predict_fn is None or rebuild:
            ...
            def predict_fn_pp(inputs, label=None):
                ...
                assert label is None or \
                    (isinstance(label, (list, np.ndarray)) and len(label) == inputs.shape[0])

                import paddle
                if isinstance(inputs, tuple):
                    # for nlp or multiple inputs.
                    tensor_inputs = []
                    for inp in inputs:
                ...
                if gradient_of == 'loss':
                    ...
                    # logits or probas
                    label = np.array(label).reshape((num_samples, ))
                    labels_onehot = paddle.nn.functional.one_hot(paddle.to_tensor(label), num_classes=num_classes)
                ...
                return gradients, label, probas
                
            def predict_fn_pt(inputs, labels=None):
                """predict_fn for input gradients based interpreters,
                    for image classification models only.
                ...
                if isinstance(inputs, tuple):
                    ...
                    tensor_inputs.requires_grad = True
                    if self.device[:3] == 'gpu':
                ...
                if gradient_of == 'loss':
                    ...
                    labels_onehot = F.one_hot(torch.from_numpy(labels), num_classes=num_classes)
                    if gradient_of == 'logit':
                ...
                # loss.backward()
                # gradients = tensor_inputs[0].grad
                gradients = torch.autograd.grad(loss, tensor_inputs[0])
                ...
                return gradients, labels.cpu().numpy(), probas.cpu().numpy()
            
            if self.env == 'pp':
                self.predict_fn = predict_fn_pp
            elif self.env == 'pt':
                self.predict_fn = predict_fn_pt
            else:
                print("Use Paddle implementation by default.")
                self.predict_fn = predict_fn_pp
...
class TransformerInterpreter(Interpreter):
    ...
    def _build_predict_fn(
        ...
        if self.predict_fn is None or rebuild:
            ...
            def predict_fn_pp(inputs, label=None, scale: float or None=None):
                """predict_fn for input gradients based interpreters,
                    for image classification models only.
            ...
            def predict_fn_pt(inputs, label=None, scale: float or None=None):
                """predict_fn for input gradients based interpreters,
                    for image classification models only.
                ...
                block_projs = []
                hooks = []
                for n, v in self.model.named_modules():
                    if attn_map_name is not None and re.match(attn_map_name, n):
                        h = v.register_forward_hook(block_attn_hook)
                        hooks.append(h)
                    elif scale is not None and embedding_name is not None and re.match(embedding_name, n):
                        h = v.register_forward_hook(hook)
                        hooks.append(h)
                    elif attn_proj_name is not None and re.match(attn_proj_name, n):
                        block_projs.append(v.weight)
                    elif attn_v_name is not None and re.match(attn_v_name, n):
                        h = v.register_forward_pre_hook(block_input_hook)
                        hooks.append(h)
                        h = v.register_forward_hook(block_value_hook)
                        hooks.append(h)
                ######################## apply hooks in the forward pass ########################
                ...
                if gradient_of == 'loss':
                    ...
                    # logits or probas
                    label = label.reshape((num_samples, ))
                    label_onehot = F.one_hot(label, num_classes=num_classes)
                    if gradient_of == 'logit':
                        loss = torch.sum(logits * label_onehot, dim=-1)
                    else:
                        loss = torch.sum(probas * label_onehot, dim=1)

                loss.backward()
                block_attns_grads = []
                for i, attn in enumerate(block_attns):
                    grad = attn.grad.cpu().numpy()
                    block_attns_grads.append(grad)
                    block_attns[i] = attn.detach().cpu().numpy()
                
                for i, inp in enumerate(block_inputs):
                    block_inputs[i] = inp[0].detach().cpu().numpy()
                ...
                for i, value in enumerate(block_values):
                    ...
                    d_value = value.shape[-1]
                    if d_inp == d_value:
                        block_values[i] = value[0].cpu().numpy()
                    elif d_inp * 3 == d_value:
                        b, s, _ = value.shape
                        value = value.reshape((b, s, 3, -1))  # 3 == [q,k,v], b == 1.
                        block_values[i] = value[0, :, 2].detach().cpu().numpy()
                    else:
                        raise ValueError("Report this issue to InterpretDL.")
                
                for i, proj in enumerate(block_projs):
                    block_projs[i] = proj.detach().cpu().numpy()                
                
                return block_attns, block_attns_grads, block_inputs, block_values, block_projs, \
                    probas.detach().cpu().numpy(), label.detach().cpu().numpy()
