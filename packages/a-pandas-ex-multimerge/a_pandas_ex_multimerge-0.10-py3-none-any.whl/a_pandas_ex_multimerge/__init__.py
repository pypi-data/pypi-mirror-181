...
def multimerge(
    ...
    dataframes: list,
    on_index: bool = False,
    columns_to_merge: Union[None, list] = None,
    print_exception: bool = True,
    disable: bool = False,
    *args,
    **kwargs
) -> pd.DataFrame:
    df2x, isser = series_to_dataframe(df)
    dataframes.insert(0, df2x)
    dataframes = dataframes.copy()
    if columns_to_merge is not None or on_index is False:
        if columns_to_merge is None:
            columns_to_merge = list(
                reduce(
                    lambda x, y: ignore_exceptions_decorator(
                        FlexiblePartialOwnName(
                            set.union, "", True, lambda: set(x), lambda: set(y)
                        ),
                        exception_value=x,
        ...
        xax = reduce(
            lambda x, y: ignore_exceptions_decorator(
                FlexiblePartialOwnName(
                    pd.merge, "", True, x, y, *args, on=columns_to_merge, **kwargs
                ),
                exception_value=x,
                print_exception=print_exception,
                disable=disable,
...
def pd_add_multimerge():
    DataFrame.ds_multimerge = multimerge
    Series.ds_multimerge = multimerge
