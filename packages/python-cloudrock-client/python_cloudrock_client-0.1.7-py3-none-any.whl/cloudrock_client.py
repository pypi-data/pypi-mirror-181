...
class CloudrockClientException(Exception):
    pass


class ObjectDoesNotExist(CloudrockClientException):
    """The requested object does not exist"""

    pass


class MultipleObjectsReturned(CloudrockClientException):
    """The query returned multiple objects when only one was expected."""

    pass


class ValidationError(CloudrockClientException):
    """An error while validating data."""

    pass


class TimeoutError(CloudrockClientException):
    """Thrown when a command does not complete in enough time."""

    pass


class InvalidStateError(CloudrockClientException):
...
class ComponentUsage:
    # TODO: rename to 'component_type' after https://cloudrock.atlassian.net/browse/WAL-4259 is done
    type: str
    amount: int
    description: str = ""
...
class CloudrockClient(object):
    ...
    def __init__(self, api_url, access_token):
        """
        Initializes a Cloudrock client
        :param api_url: a fully qualified URL to the Cloudrock API. Example: https://cloudrock.example.com:8000/api
    ...
    def _make_request(self, method, url, valid_states, retry_count=3, **kwargs):
        if retry_count == 0:
            raise CloudrockClientException(
        ...
        try:
            ...
            raise CloudrockClientException(str(error))
        ...
        if response.status_code not in valid_states:
            ...
            raise CloudrockClientException(error)
    ...
    def _get_resource(self, endpoint, value, extra=None):
        ...
        :param endpoint: CloudrockClient.Endpoint attribute.
        :param value: name or uuid of the resource
        :return: a resource as a dictionary.
        :raises: CloudrockClientException if resource could not be received or response failed.
        """
        if not value:
            raise CloudrockClientException("Empty ID is not allowed.")
        if is_uuid(value):
    ...
    def _get_tenant(self, name, project=None):
        """
        Find OpenStack tenant resource in Cloudrock database.
        :param name: OpenStack name or UUID.
        :param project: Cloudrock project name or UUID.
        :return: OpenStack tenant as Cloudrock resource.
        """
        extra = None
        if project:
    ...
    def create_security_group(
        ...
        """
        Creates OpenStack security group via Cloudrock API from passed parameters.

        :param tenant: uuid or name of the tenant to use.
        :param name: name of the security group.
        :param rules: list of rules to add the security group.
        :param project: name of the Cloudrock project where OpenStack tenant is located.
    ...
    def get_marketplace_resource(self, resource_uuid):
        return self._get_resource(
            CloudrockClient.Endpoints.MarketplaceResources, resource_uuid
        )
    ...
    def create_snapshot(
        ...
        """
        Creates OpenStack snapshot via Cloudrock API from passed parameters.
    ...
    def create_marketplace_order(
        ...
        """
        Create order with one item in Cloudrock Marketplace.
    ...
    def get_order(self, order_uuid):
        return self._get_resource(CloudrockClient.Endpoints.MarketplaceOrder, order_uuid)
    ...
    def get_order_item(self, order_item_uuid):
        return self._get_resource(
            CloudrockClient.Endpoints.MarketplaceOrderItem, order_item_uuid
        )
...
def cloudrock_full_argument_spec(**kwargs):
    spec = dict(
...
def cloudrock_resource_argument_spec(**kwargs):
    spec = dict(
    ...
    spec.update(cloudrock_full_argument_spec(**kwargs))
    return spec


def cloudrock_client_from_module(module):
    return CloudrockClient(module.params["api_url"], module.params["access_token"])
