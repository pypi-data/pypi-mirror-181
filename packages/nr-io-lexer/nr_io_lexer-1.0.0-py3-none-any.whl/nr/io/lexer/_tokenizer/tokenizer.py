...
class ProxyToken(t.Generic[T, U]):
    ...
    def _token(self) -> Token[T, U]:
        ...
        if self._transformed_token is not None and self._transformed_token[0] is self.tokenizer.current:
    ...
    def save(self) -> "TokenizerState[T, U]":
        return self.tokenizer.state

    def load(self, pos: "TokenizerState[T, U]") -> None:
...
class Tokenizer(t.Generic[T, U]):
    ...
    def state(self) -> TokenizerState[T, U]:
        ...
        return TokenizerState(
            self.scanner.pos, self._current, self.skipped.copy(), self.ignored.copy(), self._skip_rule_once
        )
    ...
    def _do_next(self, select: t.Optional[t.Set[T]], expect: t.Optional[t.Set[T]]) -> Token[T, U]:  # NOSONAR
        token, skippable = self._extract_token(
            lambda t: (select is None and not self.ignored.get(t, False)) or (select is not None and t in select)
        )
        if token is None and select is not None:
            ...
            # the "expect" variable.
            token, skippable = self._extract_token(
                lambda t: select is not None and t not in select and not self.ignored.get(t, False)
            )
            if token is not None and skippable:
                if self.debug & Debug.EXTRACT:
                    self.log.debug(
                        'No selected token matched, but extract another skippable token "%s". '
            ...
            if self.debug & Debug.EXTRACT:
                self.log.debug(
                    'Extracted skippable token "%s". Skip and continue recursively\n\t\ttoken: %r', token.type, token
                )
            return self._do_next(select, expect)
    ...
    def _extract_token(self, filter: t.Callable[[T], bool]) -> t.Tuple[t.Optional[Token[T, U]], bool]:
        if not self.scanner:
            return Token(self.rules.sentinel.type, self.rules.sentinel.value, self.scanner.pos, True), False
