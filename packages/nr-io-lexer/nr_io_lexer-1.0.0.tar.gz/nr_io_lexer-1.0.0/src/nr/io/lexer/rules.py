...
def regex(pattern: str, *, at_line_start_only: bool = False) -> TokenExtractor["t.Match[str]"]:
    """
    Creates a tokenizer rule that matches a regular expression and returns the #t.Match object
    ...
    *at_line_start_only* argument to `True`. The regex caret (`^`) control character will not work
    ...
    def _impl(scanner: "Scanner") -> t.Optional["t.Match[str]"]:
...
def regex_extract(
    pattern: str, group: t.Union[str, int] = 0, *, at_line_start_only: bool = False
) -> TokenExtractor[str]:
    """
...
def string_literal(
    ...
    def _impl(scanner: "Scanner") -> t.Optional[str]:
        prefix = (scanner.getmatch(r"[" + re.escape(accepted_prefixes) + r"]+") or "") if accepted_prefixes else ""
