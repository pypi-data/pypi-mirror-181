...
if __name__ == "__main__":
    ...
    root = "test data for refine (tilted detector)/"

    mask = snpl.image.NpzImage(root+"Mask_EmptyBeam_ManuallyEdited.npz").get_layer("mask")
    geom = sgt.geometry()
    geom.load_specs(root+"Geometry_AgBh_CeO2.txt")
    geom.mask = mask
    geom.refresh_q()
    ...
    intensity = pilatus.ReadTiff(root+"AgBh.tif").astype(np.float64)

    qtargets = [Q_AgBh_1*6.0, Q_AgBh_1*7.0, Q_AgBh_1*8.0]
    ...
    snpl.savefig("test_opt_image_tilted.png", dpi=300)
    snpl.clf()
