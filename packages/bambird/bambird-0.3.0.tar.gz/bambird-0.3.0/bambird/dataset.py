...
def query_download_xc(
    ...
    NUM_FILES = params['PARAMS_XC']['NUM_FILES']

    if os.path.isfile(csv_fullfilename) == False :
        params['PARAMS_XC']['NUM_FILES'] = None
        ...
        df_dataset = pd.read_csv(csv_fullfilename, sep=';', index = False)
        
        if verbose:
            print((("The metadata file {} already exits\n") +
                  ("with {} audio recordings")).format(csv_fullfilename,
                                                      len(df_dataset)))
        ...
        if increment :
            ...
            df_dataset = df_dataset[df_dataset['fullfilename']==None]
            
            # if no limit in the number of files
            if NUM_FILES is not None :
                # test if the number of files is greater than the maximum number of
                # resquested files
                if len(df_dataset) > NUM_FILES : 
                    df_dataset = df_dataset.sample(n = NUM_FILES,
                                                   random_state = random_seed)
            
            df, csv = download_xc (df_dataset      =df_dataset,
                                        rootdir         =rootdir, 
                                        dataset_name    =dataset_name, 
        ...
        # if we                 
            ...
            if NUM_FILES is not None :
                ...
                # resquested files
                if len(df_dataset) > NUM_FILES : 
                    df = df_dataset.sample(n = NUM_FILES,
                                           random_state = random_seed)
