...
class SyntheticDataBench:
    ...
    def __init__(
        ...
        if test_df is not None:
            ...
            self.test_df = data.sample(frac=test_size, random_state=self.random_state)
    ...
    def compute_distance_to_closest_records(
        original: pd.DataFrame, synthetic: pd.DataFrame, n_test: int
    ) -> pd.Series:
        """
        ...
        return pd.Series(
            [
                np.linalg.norm(original - synthetic.iloc[i], ord=1, axis=1).min()
    ...
    def compute_discriminator_predictions(
        ...
        processed = SyntheticDataBench.preprocess_data(original, train_synthetic)
        preprocessor = processed["preprocessor"]
        column_names = processed["column_names"]
        original = processed["data"]
        train_synthetic = processed["other"]
        ...
        test = pd.DataFrame(
            test if isinstance(test, np.ndarray) else test.toarray(),
            columns=column_names,
        )
        test[label_col_name] = 1

        test_synthetic = preprocessor.transform(test_synthetic)
        test_synthetic = pd.DataFrame(
            test_synthetic
            if isinstance(test_synthetic, np.ndarray)
            else test_synthetic.toarray(),
            columns=column_names,
        )
        test_synthetic[label_col_name] = 0

        combined_test = pd.concat([test, test_synthetic])
        X_test = combined_test.drop(label_col_name, axis=1)
        ...
        return dict(y_test=y_test, y_preds=y_preds)

    def get_dcr(self):
        """Get the DCR values for this experiment."""
        return self.compute_distance_to_closest_records(
            self.train_df, self.synth_train_df, self.n_test
        )

    def get_ml_efficiency(self, model: sklearn.base.BaseEstimator):
        """Get the ML efficiency for this experiment."""

        performance_set = self.measure_ml_efficiency(
            model=model,
            train=self.train_df,
    ...
    def compute_sensitivity_metric(
        ...
        if qt_interval <= 1:
            return (synth_min <= np.quantile(test_min, qt_max)).mean()
        else:
