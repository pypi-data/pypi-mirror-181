...
class InvoiceConfiguration(BaseModel):
    ...
    def first_or_default(cls):
        return cls.objects.first() if cls.objects.exists() else cls(currency="", currency_symbol="")
...
class Invoice(BaseModel):
    ...
    def csv_filename(self):
        return f"{slugify(self.invoice_number)}_{slugify(self.project_details.name)}.csv"

    def csv_export_http_response(self):
        from NEMO_billing.invoices.invoice_generator import CSVInvoiceGenerator

        csv_generator = CSVInvoiceGenerator()
        csv_generator.render_invoice(self)
        return csv_generator.document

    def filename_for_zip(self, extension=None):
        ext = extension or os.path.splitext(self.filename())[1]
        return export_invoice_filename(self) + ext
    ...
    def tool_usage_details(self, core_facility: CoreFacility):
        return self.invoice_details().filter(
            core_facility=core_facility, item_type=BillableItemType.TOOL_USAGE.value
        ).order_by("start")

    def area_access_details(self, core_facility: CoreFacility):
        return self.invoice_details().filter(
            core_facility=core_facility, item_type=BillableItemType.AREA_ACCESS.value
        ).order_by("start")

    def staff_charge_details(self, core_facility: CoreFacility):
        return self.invoice_details().filter(
            core_facility=core_facility, item_type=BillableItemType.STAFF_CHARGE.value
        )

    def consumable_withdrawal_details(self, core_facility: CoreFacility):
        return self.invoice_details().filter(
            core_facility=core_facility, item_type=BillableItemType.CONSUMABLE.value
        ).order_by("start")

    def training_details(self, core_facility: CoreFacility):
        return self.invoice_details().filter(
            core_facility=core_facility, item_type=BillableItemType.TRAINING.value
        ).order_by("start")

    def missed_reservation_details(self, core_facility: CoreFacility):
        return self.invoice_details().filter(
            core_facility=core_facility, item_type=BillableItemType.MISSED_RESERVATION.value
        ).order_by("start")

    def custom_charges_details(self, core_facility: CoreFacility):
        return self.invoice_details().filter(
            core_facility=core_facility, item_type=BillableItemType.CUSTOM_CHARGE.value
        ).order_by("start")
