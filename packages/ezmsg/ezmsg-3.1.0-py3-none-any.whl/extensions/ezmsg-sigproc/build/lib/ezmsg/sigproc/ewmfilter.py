...
class EWMState(ez.State):
    last_signal: Optional[TimeSeriesMessage] = None
    buffer_queue: "asyncio.Queue[ TimeSeriesMessage ]" = field(default_factory=asyncio.Queue)
...
class EWM(ez.Unit):
    ...
    async def sync_output(self) -> AsyncGenerator:
        ...
        while True:
            ...
            yield (self.OUTPUT_SIGNAL, replace(signal,
                                               data=standardized[-block_len:, ...]
                                               ))
