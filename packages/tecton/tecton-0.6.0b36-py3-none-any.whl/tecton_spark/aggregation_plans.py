...
def FirstNAgg(timestamp: str, col: str, n: int) -> Column:
    ...
    udf_name = f"tecton_first_{n}"
    sc._jvm.com.tecton.udfs.spark3.FirstNRegister().register(n, udf_name, False)
    return expr(f"{udf_name}({timestamp},{col}).values")


def _make_first_n_partial(time_key: str, n: int) -> Callable:
    def _first_n_partial(col: str) -> List[Column]:
        return [FirstNAgg(time_key, col, n)]

    return _first_n_partial


def _make_first_n_full(n: int) -> Callable:
    def _first_n_full(cols: List[Column], window: WindowSpec) -> Column:
        return functions.slice(functions.flatten(functions.collect_list(cols[0]).over(window)), 1, n)

    return _first_n_full
...
AGGREGATION_PLANS = {
    ...
    afpb.AGGREGATION_FUNCTION_FIRST_NON_DISTINCT_N: lambda time_key, params, is_continuous: AggregationPlan(
        partial_aggregation_transform=_make_first_n_partial(time_key, params.first_n.n),
        full_aggregation_transform=_make_first_n_full(params.first_n.n),
        materialized_column_prefixes=[
        ...
        feature_server_transform=afpb.AGGREGATION_FUNCTION_FIRST_NON_DISTINCT_N,
    ),
}
