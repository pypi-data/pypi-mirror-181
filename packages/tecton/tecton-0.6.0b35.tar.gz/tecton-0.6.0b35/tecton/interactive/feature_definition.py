...
class FeatureDefinition(Fco):
    ...
    def _should_infer_timestamp_of_spine(self, timestamp_key: Optional[str]):
        return timestamp_key is None

    def _point_in_time_join(
        self, spine: TectonDataFrame, timestamp_key: Optional[str], from_source: bool
    ) -> TectonDataFrame:
        ...
        if self._proto.HasField("on_demand_feature_view"):
            utils.validate_spine_dataframe(
                spine.to_spark(), timestamp_key, list(self._request_context.arg_to_schema.keys())
            )
        ...
        return TectonDataFrame._create(
            builder.build_spine_join_querytree(
                dac,
                UserSpecifiedDataNode(spine).as_ref(),
                timestamp_key,
                from_source,
            )
        )
    ...
    def _get_historical_features(
        ...
        if has_point_in_time_join_params:
            ...
            if spine is not None and self._should_infer_timestamp_of_spine(timestamp_key):
