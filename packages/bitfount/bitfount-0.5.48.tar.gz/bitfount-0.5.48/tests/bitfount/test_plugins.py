...
class TestAlgorithmPlugins:
    ...
    def test_plugin_with_error_gets_ignored(
        ...
        assert caplog.records[0].levelname == "ERROR"
        assert caplog.records[0].message == (
            f"Error importing module {dummy_module_name}"
        )
...
class TestProtocolPlugins:
    ...
    def test_abstract_protocol_plugin_gets_ignored(
        ...
        assert caplog.records[1].levelname == "WARNING"
        assert caplog.records[1].message == (
            f"Found class {dummy_protocol_plugin_name} in module {dummy_module_name} which did not fully implement BaseProtocolFactory. "  # noqa: B950
            "Skipping."
        )
