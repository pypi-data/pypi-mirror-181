...
from bitfount.federated.transport.protos.messages_pb2 import BitfountTask, BitfountTasks
...
class _MessageService:
    ...
    def metadata(self) -> List[Tuple[str, str]]:
        ...
        if api_keys := self._session.api_keys:
            return [
                # Only the first portion of the API key is used
                ("x-api-key-id", api_keys.access_key_id.split(":")[0]),
                ("x-api-key", api_keys.access_key.split(":")[0]),
            ]

        return [("token", cast(str, self._session.access_token))]
        ...
        for pod_identifier, aes_encrypted_task in tasks_per_pod.items():
            ...
            if len(aes_encrypted_task) > _SMALL_MESSAGE_UPPER_LIMIT_SIZE_BYTES:
                ...
                (
                    ...
                    self._session._client_id, task_size, None, pod_name
                )
